		...
		uint8 level)
{
	...
	if (level >= 68) SetTaximaskNode(213); //Shattered Sun Staging Area
}
		...
		uint32 team)
{
	...
	if (m_TaxiDestinations.empty()) return true;

	// Check integrity
	if (m_TaxiDestinations.size() < 2) return false;
	...
	if (!sObjectMgr->GetTaxiMountDisplayId(GetTaxiSource(), team, true)) return false;
...
{
	if (m_TaxiDestinations.empty()) return "";
...
uint32 PlayerTaxi::GetCurrentTaxiPath() const
{
	...
	sObjectMgr->GetTaxiPath(m_TaxiDestinations [0], m_TaxiDestinations [1],
			path, cost);
...
bool TradeData::HasItem(uint64 item_guid) const
{
	for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i)
		if (m_items [i] == item_guid) return true;
...
void TradeData::SetItem(TradeSlots slot, Item* item)
{
	...
	if (m_items [slot] == itemGuid) return;
	...
	if (slot == TRADE_SLOT_NONTRADED) GetTraderData()->SetSpell(0);
...
void TradeData::SetSpell(uint32 spell_id, Item* castItem /*= NULL*/)
{
	...
	if (m_spell == spell_id && m_spellCastItem == itemGuid) return;
...
{
	if (forTarget) m_trader->GetSession()->SendUpdateTrade(true); // player state for trader
	else m_player->GetSession()->SendUpdateTrade(false); // player state for player
}
...
void TradeData::SetAccepted(bool state, bool crosssend /*= false*/)
{
	...
	if (!state)
	{
		if (crosssend) m_trader->GetSession()->SendTradeStatus(
				TRADE_STATUS_BACK_TO_TRADE);
		else m_player->GetSession()->SendTradeStatus(
				TRADE_STATUS_BACK_TO_TRADE);
	}
}
...
KillRewarder::KillRewarder(Player* killer, Unit* victim, bool isBattleGround) :
		// 1. Initialize internal variables to default values.
		_killer(killer), _victim(victim), _isBattleGround(isBattleGround), _isPvP(
				false), _group(killer->GetGroup()), _groupRate(1.0f), _maxLevel(
				0), _maxNotGrayMember(NULL), _count(0), _sumLevel(0), _isFullXP(
				false), _xp(0)
{
	// mark the credit as pvp if victim is player
	if (victim->GetTypeId() == TYPEID_PLAYER) _isPvP = true;
	// or if its owned by player and its not a vehicle
	else if (IS_PLAYER_GUID(victim->GetCharmerOrOwnerGUID())) _isPvP =
			!victim->IsVehicle();
...
inline void KillRewarder::_InitGroupData()
{
	...
	{
		// 2. In case when player is in group, initialize variables necessary for group calculations:
		for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr =
				itr->next())
			if (Player* member = itr->getSource()) if (member->isAlive()
					&& member->IsAtGroupRewardDistance(_victim))
			{
				...
				_sumLevel += lvl;
				// 2.3. _maxLevel - maximum level of alive group member within reward distance;
				if (_maxLevel < lvl) _maxLevel = lvl;
				// 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
				//      for whom victim is not gray;
				uint32 grayLevel = Trinity::XP::GetGrayLevel(lvl);
				if (_victim->getLevel() > grayLevel
						&& (!_maxNotGrayMember
								|| _maxNotGrayMember->getLevel() < lvl)) _maxNotGrayMember =
						member;
			}
		// 2.5. _isFullXP - flag identifying that for all group members victim is not gray,
		//      so 100% XP will be rewarded (50% otherwise).
		_isFullXP = _maxNotGrayMember
				&& (_maxLevel == _maxNotGrayMember->getLevel());
	}
	else _count = 1;
}
...
inline void KillRewarder::_InitXP(Player* player)
{
	...
	if (_isBattleGround || (!_isPvP && !_killer->GetVehicle())) _xp =
			Trinity::XP::Gain(player, _victim);
}

inline void KillRewarder::_RewardHonor(Player* player)
{
	// Rewarded player must be alive.
	if (player->isAlive()) player->RewardHonor(_victim, _count, -1, true);
}
...
inline void KillRewarder::_RewardXP(Player* player, float rate)
{
	...
	if (_group)
	{
		...
		if (_maxNotGrayMember && player->isAlive()
				&& _maxNotGrayMember->getLevel() >= player->getLevel()) xp =
				_isFullXP ? uint32(xp * rate) : // Reward FULL XP if all group members are not gray.
	...
	if (xp)
	{
		...
		Unit::AuraEffectList const& auras = player->GetAuraEffectsByType(
...
{
	// 4.4. Give kill credit (player must not be in group, or he must be alive or without corpse).
	if (!_group || player->isAlive() || !player->GetCorpse()) if (_victim->GetTypeId()
			== TYPEID_UNIT) player->KilledMonster(
			_victim->ToCreature()->GetCreatureInfo(), _victim->GetGUID());
}
...
void KillRewarder::_RewardPlayer(Player* player, bool isDungeon)
{
	...
	if (!_isBattleGround)
	{
		...
		if (_victim->GetTypeId() == TYPEID_PLAYER) player->KilledPlayerCredit();
	}
	...
	{
		const float rate =
				_group ? _groupRate * float(player->getLevel()) / _sumLevel : // Group rate depends on summary level.
		...
		if (!_isBattleGround)
		{
			...
			if (player->GetGuildId() != 0 && _victim->GetTypeId() == TYPEID_UNIT
					&& _victim->ToCreature()->IsDungeonBoss()
					&& player->GetGroup()
					&& player->GetGroup()->IsGuildGroup(player->GetGuildId()))
			{
				uint32 guildRep = uint32(_xp / 450);
				if (guildRep < 1) guildRep = 1;
				player->GetReputationMgr().ModifyReputation(
						sFactionStore.LookupEntry(1168), guildRep);
			}
		}
	}
}
...
void KillRewarder::_RewardGroup()
{
	if (_maxLevel)
	{
		...
		{
			const bool isDungeon = !_isPvP
					&& sMapStore.LookupEntry(_killer->GetMapId())->IsDungeon();
			if (!_isBattleGround)
			{
				// 3.1.2. Alter group rate if group is in raid (not for battlegrounds).
				const bool isRaid = !_isPvP
			...
			for (GroupReference* itr = _group->GetFirstMember(); itr != NULL;
					itr = itr->next())
				if (Player* member = itr->getSource()) if (member->IsAtGroupRewardDistance(
						_victim)) _RewardPlayer(member, isDungeon);
		}
	}
}
...
void KillRewarder::Reward()
{
	...
	// 5. Credit instance encounter.
	if (Creature* victim = _victim->ToCreature()) if (victim->IsDungeonBoss()) if (InstanceScript* instance = _victim->GetInstanceScript()) instance->UpdateEncounterState(
			ENCOUNTER_CREDIT_KILL_CREATURE, _victim->GetEntry(), _victim);
}
...
void Player::CleanupsBeforeDelete(bool finalCleanup)
{
	...
	if (m_transport) m_transport->RemovePassenger(this);
		...
		uint8 hairColor, uint8 facialHair, uint8 /*outfitId*/)
{
	...
	InitDisplayIds();
	if (sWorld->getIntConfig(CONFIG_GAME_TYPE) == REALM_TYPE_PVP || sWorld->getIntConfig(
			CONFIG_GAME_TYPE) == REALM_TYPE_RPPVP)
	{
	...
	if (oEntry)
	{
		for (int j = 0; j < MAX_OUTFIT_ITEMS; ++j)
		{
					...
					&& iProto->SubClass == ITEM_SUBCLASS_FOOD)
			{
				...
				if (iProto->GetMaxStackSize() < count) count =
						iProto->GetMaxStackSize();
			}
	...
	for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; i++)
	{
		if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i))
		{
			...
			else
			{
				...
				if (msg == EQUIP_ERR_OK) SetAmmo(pItem->GetEntry());
			}
		}
	}
...
bool Player::StoreNewItemInBestSlots(uint32 titem_id, uint32 titem_amount)
{
	...
	if (titem_amount == 0) return true; // equipped
		...
		uint32 CurrentValue, int32 Regen)
{
	...
	{
		if (int(CurrentValue) != DISABLED_MIRROR_TIMER) StopMirrorTimer(Type);
		return;
	}
...
{
	if (!isAlive() || isGameMaster()) return 0;

	// Absorb, resist some environmental damage type
	uint32 absorb = 0;
	uint32 resist = 0;
	if (type == DAMAGE_LAVA) CalcAbsorbResist(this, SPELL_SCHOOL_MASK_FIRE,
			DIRECT_DAMAGE, damage, &absorb, &resist);
	else if (type == DAMAGE_SLIME) CalcAbsorbResist(this,
			SPELL_SCHOOL_MASK_NATURE, DIRECT_DAMAGE, damage, &absorb, &resist);
...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
	switch (timer)
	{
		...
		case BREATH_TIMER:
		{
			if (!isAlive() || HasAuraType(SPELL_AURA_WATER_BREATHING)
					|| GetSession()->GetSecurity()
							>= AccountTypes(
									sWorld->getIntConfig(
											CONFIG_DISABLE_BREATHING))) return DISABLED_MIRROR_TIMER;
		...
		case FIRE_TIMER:
		{
			if (!isAlive()) return DISABLED_MIRROR_TIMER;
...
void Player::UpdateMirrorTimers()
{
	...
	if (m_MirrorTimerFlags) m_MirrorTimerFlagsLast = ~m_MirrorTimerFlags;
}

void Player::HandleDrowning(uint32 time_diff)
{
	if (!m_MirrorTimerFlags) return;
	...
	else if (m_MirrorTimer [BREATH_TIMER] != DISABLED_MIRROR_TIMER) // Regen timer
	{
		...
		if (m_MirrorTimer [BREATH_TIMER] >= UnderWaterTime || !isAlive()) StopMirrorTimer(BREATH_TIMER);
		else if (m_MirrorTimerFlagsLast & UNDERWATER_INWATER) SendMirrorTimer(
	...
	if (m_MirrorTimerFlags & UNDERWARER_INDARKWATER)
	{
		...
		else
		{
			...
			else if (!(m_MirrorTimerFlagsLast & UNDERWARER_INDARKWATER)) SendMirrorTimer(
					FATIGUE_TIMER, getMaxTimer(FATIGUE_TIMER),
					m_MirrorTimer [FATIGUE_TIMER], -1);
		}
	}
	else if (m_MirrorTimer [FATIGUE_TIMER] != DISABLED_MIRROR_TIMER) // Regen timer
	{
		...
		if (m_MirrorTimer [FATIGUE_TIMER] >= DarkWaterTime || !isAlive()) StopMirrorTimer(FATIGUE_TIMER);
		else if (m_MirrorTimerFlagsLast & UNDERWARER_INDARKWATER) SendMirrorTimer(
				FATIGUE_TIMER, DarkWaterTime, m_MirrorTimer [FATIGUE_TIMER],
				10);
	}
	...
	{
		// Breath timer not activated - activate it
		if (m_MirrorTimer [FIRE_TIMER] == DISABLED_MIRROR_TIMER) m_MirrorTimer [FIRE_TIMER] =
		...
		else
		{
			...
			if (m_MirrorTimer [FIRE_TIMER] < 0)
			{
				...
				if (m_MirrorTimerFlags & UNDERWATER_INLAVA) EnvironmentalDamage(
						DAMAGE_LAVA, damage);
				// need to skip Slime damage in Undercity,
				// maybe someone can find better way to handle environmental damage
				else if (m_zoneUpdateId != 1497 && m_zoneUpdateId != 1
						&& m_zoneUpdateId != 3968) EnvironmentalDamage(
						DAMAGE_SLIME, damage);
			}
		}
	}
	else m_MirrorTimer [FIRE_TIMER] = DISABLED_MIRROR_TIMER;
...
{
	if (value >= 23000) return DRUNKEN_SMASHED;
	if (value >= 12800) return DRUNKEN_DRUNK;
	if (value & 0xFFFE) return DRUNKEN_TIPSY;
...
void Player::SetDrunkValue(uint16 newDrunkenValue, uint32 itemId)
{
	...
	if (!newDrunkenValue && !HasAuraType(SPELL_AURA_MOD_FAKE_INEBRIATE)) m_invisibilityDetect.AddFlag(
			INVISIBILITY_DRUNK);
	else m_invisibilityDetect.DelFlag(INVISIBILITY_DRUNK);
	...
	if (newDrunkenState == oldDrunkenState) return;
...
void Player::Update(uint32 p_time)
{
			...
			itr != m_globalCooldowns.end(); ++itr)
	{
		if (itr->second)
		{
			if (itr->second > p_time) itr->second -= p_time;
			else itr->second = 0;
		}
	}
	...
	if (isCharmed()) if (Unit *charmer = GetCharmer()) if (charmer->GetTypeId()
			== TYPEID_UNIT && charmer->isAlive()) UpdateCharmedAI();

	// Update items that have just a limited lifetime
	if (now > m_Last_tick) UpdateItemDuration(uint32(now - m_Last_tick));
	...
	// check every second
	if (now > m_Last_tick + 1) UpdateSoulboundTradeItems();
		...
		if (Unit *pVictim = getVictim())
		{
			...
			if (isAttackReady(BASE_ATTACK))
			{
				...
				else
				{
					...
					if (haveOffhandWeapon()) if (getAttackTimer(
							OFF_ATTACK) < ATTACK_DISPLAY_DELAY) setAttackTimer(
							OFF_ATTACK, ATTACK_DISPLAY_DELAY);
			...
			{
				if (!IsWithinMeleeRange(pVictim)) setAttackTimer(OFF_ATTACK,
						100);
				else if (!HasInArc(2 * M_PI / 3, pVictim)) setAttackTimer(
						OFF_ATTACK, 100);
				else
				{
					// prevent base and off attack in same time, delay attack at 0.2 sec
					if (getAttackTimer(BASE_ATTACK) < ATTACK_DISPLAY_DELAY) setAttackTimer(
							BASE_ATTACK, ATTACK_DISPLAY_DELAY);
	...
	{
		if (p_time >= m_weaponChangeTimer) m_weaponChangeTimer = 0;
		else m_weaponChangeTimer -= p_time;
	}
	...
	if (m_zoneUpdateTimer > 0)
	{
		if (p_time >= m_zoneUpdateTimer)
		{
			...
			if (m_zoneUpdateId != newzone) UpdateZone(newzone, newarea); // also update area
			else
			{
				// use area updates as well
				// needed for free far all arenas for example
				if (m_areaUpdateId != newarea) UpdateArea(newarea);

				m_zoneUpdateTimer = ZONE_UPDATE_INTERVAL;
			}
		}
		else m_zoneUpdateTimer -= p_time;
	}

	if (m_timeSyncTimer > 0)
	{
		if (p_time >= m_timeSyncTimer) SendTimeSync();
		else m_timeSyncTimer -= p_time;
	}
	...
	if (m_deathState == JUST_DIED) KillPlayer();
	...
	if (m_drunk)
	{
		...
		if (m_drunkTimer > 10 * IN_MILLISECONDS) HandleSobering();
	}
	...
	if (!_instanceResetTimes.empty())
	{
		...
		{
			if (itr->second < now) _instanceResetTimes.erase(itr++);
			else ++itr;
		}
	}
	...
	if (IsHasDelayedTeleport() && isAlive()) TeleportTo(m_teleport_dest,
			m_teleport_options);

	if (getLevel() >= 80) RemoveOrAddMasterySpells();
}
...
void Player::setDeathState(DeathState s)
{
	...
	if (s == JUST_DIED)
	{
		...
		// passive spell
		if (!ressSpellId) ressSpellId = GetResurrectionSpellId();
		GetAchievementMgr().UpdateAchievementCriteria(
	...
	if (s == JUST_DIED && cur && ressSpellId) SetUInt32Value(
			PLAYER_SELF_RES_SPELL, ressSpellId);
...
bool Player::BuildEnumData(QueryResult result, WorldPacket * p_data)
{
	...
	*p_data << fields [12].GetFloat(); // z
	...
	if (uint32 guildId = fields[13].GetUInt32()) *p_data
			<< uint64(MAKE_NEW_GUID(guildId, 0, HIGHGUID_GUILD));
	else *p_data << uint64(0);

	uint32 char_flags = 0;
	uint32 playerFlags = fields [14].GetUInt32();
	uint32 atLoginFlags = fields [15].GetUInt32();
	if (playerFlags & PLAYER_FLAGS_HIDE_HELM) char_flags |=
			CHARACTER_FLAG_HIDE_HELM;
	if (playerFlags & PLAYER_FLAGS_HIDE_CLOAK) char_flags |=
			CHARACTER_FLAG_HIDE_CLOAK;
	if (playerFlags & PLAYER_FLAGS_GHOST) char_flags |= CHARACTER_FLAG_GHOST;
	if (atLoginFlags & AT_LOGIN_RENAME) char_flags |= CHARACTER_FLAG_RENAME;
	if (fields [20].GetUInt32()) char_flags |= CHARACTER_FLAG_LOCKED_BY_BILLING;
	if (sWorld->getBoolConfig(CONFIG_DECLINED_NAMES_USED))
	{
		if (!fields [21].GetString().empty()) char_flags |=
	...
	*p_data << uint32(char_flags); // character flags
	...
	if (atLoginFlags & AT_LOGIN_CUSTOMIZE) *p_data
			<< uint32(CHAR_CUSTOMIZE_FLAG_CUSTOMIZE);
	else if (atLoginFlags & AT_LOGIN_CHANGE_FACTION) *p_data
			<< uint32(CHAR_CUSTOMIZE_FLAG_FACTION);
	else if (atLoginFlags & AT_LOGIN_CHANGE_RACE) *p_data
			<< uint32(CHAR_CUSTOMIZE_FLAG_RACE);
	else *p_data << uint32(CHAR_CUSTOMIZE_FLAG_NONE);
...
bool Player::ToggleAFK()
{
	...
	if (state && InBattleground() && !InArena()) LeaveBattleground();
	if (Guild * pGuild = sObjectMgr->GetGuildById(GetGuildId())) pGuild->OnPlayerStatusChange(
			this, GUILD_MEMBER_FLAG_AFK, state);
...
bool Player::ToggleDND()
{
	...
	if (Guild * pGuild = sObjectMgr->GetGuildById(GetGuildId())) pGuild->OnPlayerStatusChange(
			this, GUILD_MEMBER_FLAG_DND, state);
	return state;
}
...
uint8 Player::chatTag() const
{
	...
	// 0x2 - dnd
	// 0x1 - afk
	if (isGMChat()) return 4;
	else if (isDND()) return 3;
	if (isAFK()) return 1;
	else return 0;
}
		...
		float orientation, uint32 options)
{
	...
	if (!InBattleground() && mEntry->IsBattlegroundOrArena()) return false;
	...
	sLog->outDebug(LOG_FILTER_MAPS, "Player %s is being teleported to map %u",
			GetName(), mapid);
	...
	if (m_transport)
	{
		if (options & TELE_TO_NOT_LEAVE_TRANSPORT) AddUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT);
		else
		{
			m_transport->RemovePassenger(this);
			m_transport = NULL;
	...
	if (duel && GetMapId() != mapid
			&& GetMap()->GetGameObject(GetUInt64Value(PLAYER_DUEL_ARBITER))) DuelComplete(DUEL_FLED);
...
if(		IsHasDelayedTeleport())
		...
		if (!(options & TELE_TO_NOT_UNSUMMON_PET))
		{
			...
			if (pet
					&& !pet->IsWithinDist3d(x, y, z,
							GetMap()->GetVisibilityRange())) UnsummonPetTemporaryIfAny();
		}

		if (!(options & TELE_TO_NOT_LEAVE_COMBAT)) CombatStop();
	...
	else
	{
		if (getClass() == CLASS_DEATH_KNIGHT && GetMapId() == 609
				&& !isGameMaster() && !HasSpell(50977)) return false;
		...
		if (!sMapMgr->CanPlayerEnter(mapid, this, false)) return false;
...
if(			IsHasDelayedTeleport())
			...
			if (Battleground const* bg = GetBattleground())
			{
				...
				if (bg->GetMapId() != mapid) LeaveBattleground(false); // don't teleport to entry point
			}
			...
			if (mEntry->IsBattleArena())
			{
				...
				RemoveArenaAuras();
				if (pet) pet->RemoveArenaAuras();
			}

			// remove pet on map change
			if (pet) UnsummonPetTemporaryIfAny();
			...
			if (!(options & TELE_TO_SPELL)) if (IsNonMeleeSpellCasted(true)) InterruptNonMeleeSpells(
					true);
			...
			if (!GetSession()->PlayerLogout())
			{
				...
				data << uint32(mapid);
				if (m_transport) data << m_transport->GetEntry() << GetMapId();
			...
			if (!GetSession()->PlayerLogout())
			{
				...
				data << uint32(mapid);
				if (m_transport) data
						<< m_movementInfo.t_pos.PositionXYZOStream();
				else data << m_teleport_dest.PositionXYZOStream();
...
{
	if (m_DelayedOperations == 0) return;

	if (m_DelayedOperations & DELAYED_RESURRECT_PLAYER)
	{
		ResurrectPlayer(0.0f, false);

		if (GetMaxHealth() > m_resurrectHealth) SetHealth(m_resurrectHealth);
		else SetFullHealth();

		if (GetMaxPower(POWER_MANA) > m_resurrectMana) SetPower(POWER_MANA,
				m_resurrectMana);
	...
	if (m_DelayedOperations & DELAYED_SPELL_CAST_DESERTER) CastSpell(this,
			26013, true); // Deserter
...
void Player::AddToWorld()
{
	...
	for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i)
		if (m_items [i]) m_items [i]->AddToWorld();
}
...
void Player::RemoveFromWorld()
{
	...
	for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; ++i)
	{
		if (m_items [i]) m_items [i]->RemoveFromWorld();
	}
...
void Player::RegenerateAll()
{
	...
	if (getClass() == CLASS_PALADIN) m_holyPowerRegenTimerCount += m_regenTimer;
	...
	if (getClass() == CLASS_DEATH_KNIGHT) for (uint32 i = 0; i < MAX_RUNES; i +=
			2)
	{
		uint32 cd1 = GetRuneCooldown(i);
		uint32 cd2 = GetRuneCooldown(i + 1);

		if (cd1 && (!cd2 || cd1 <= cd2)) SetRuneCooldown(i,
				(cd1 > m_regenTimer) ? cd1 - m_regenTimer : 0);
		else if (cd2) SetRuneCooldown(i + 1,
	...
	if (m_regenTimerCount >= 2000)
	{
		...
		if (getClass() == CLASS_DEATH_KNIGHT) Regenerate(POWER_RUNIC_POWER);
		if (getClass() == CLASS_HUNTER) Regenerate(POWER_FOCUS);
...
void Player::Regenerate(Powers power)
{
	...
	if (HasAuraTypeWithValue(SPELL_AURA_PREVENT_REGENERATE_POWER, power)) return;
	...
	switch (power)
	{
		case POWER_MANA:
		{
			...
			if (getLevel() < 15) ManaIncreaseRate = sWorld->getRate(
					RATE_POWER_MANA) * (2.066f - (getLevel() * 0.066f));
			...
			else addvalue += GetFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER)
					* ManaIncreaseRate * 0.001f * m_regenTimer * haste;
		}
			break;
		...
		case POWER_ENERGY: // Regenerate energy (rogue)
			addvalue += 0.01f * m_regenTimer * haste * sWorld->getRate(
					RATE_POWER_ENERGY);
			break;
	...
	if (power != POWER_MANA)
	{
		...
		for (AuraEffectList::const_iterator i = ModPowerRegenPCTAuras.begin();
			...
			if (Powers((*i)->GetMiscValue()) == power) addvalue *=
					((*i)->GetAmount() + 100) / 100.0f;

		// Butchery requires combat for this effect
		if (power != POWER_RUNIC_POWER || isInCombat()) addvalue +=
	...
	else
	{
		...
		else m_powerFraction [power] = addvalue - integerValue;
	}
	if (m_regenTimerCount >= 2000 || m_holyPowerRegenTimerCount >= 10000) SetPower(
			power, curValue);
	else UpdateUInt32Value(UNIT_FIELD_POWER1 + power, curValue);
}
...
void Player::RegenerateHealth()
{
	...
	if (getLevel() < 15) HealthIncreaseRate = sWorld->getRate(RATE_HEALTH)
			* (2.066f - (getLevel() * 0.066f));

	float addvalue = 0.0f;

	// polymorphed case
	if (IsPolymorphed()) addvalue = (float) GetMaxHealth() / 3;
	...
	else if (!isInCombat() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
	{
		...
		else if (HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT)) addvalue *=
				GetTotalAuraModifier(SPELL_AURA_MOD_REGEN_DURING_COMBAT)
						/ 100.0f;

		if (!IsStandState()) addvalue *= 1.5;
	}
...
{
	SetHealth(GetMaxHealth());switch (getPowerType())
	{
		case POWER_MANA:
		SetPower(POWER_MANA, GetMaxPower(POWER_MANA));
		break;
...
Creature* Player::GetNPCIfCanInteractWith(uint64 guid, uint32 npcflagmask)
{
	...
	if (!isAlive()
			&& !(creature->GetCreatureInfo()->type_flags
					& CREATURE_TYPEFLAGS_GHOST)) return NULL;

	// alive or spirit healer
	if (!creature->isAlive()
			&& !(creature->GetCreatureInfo()->type_flags
					& CREATURE_TYPEFLAGS_DEAD_INTERACT)) return NULL;

	// appropriate npc type
	if (npcflagmask && !creature->HasFlag(UNIT_NPC_FLAGS, npcflagmask)) return NULL;

	// not allow interaction under control, but allow with own pets
	if (creature->GetCharmerGUID()) return NULL;

	// not enemy
	if (creature->IsHostileTo(this)) return NULL;

	// not unfriendly
	if (FactionTemplateEntry const* factionTemplate = sFactionTemplateStore.LookupEntry(creature->getFaction())) if (factionTemplate->faction) if (FactionEntry const* faction = sFactionStore.LookupEntry(factionTemplate->faction)) if (faction->reputationListID
			>= 0 && GetReputationMgr().GetRank(faction) <= REP_UNFRIENDLY) return NULL;

	// not too far
	if (!creature->IsWithinDistInMap(this, INTERACTION_DISTANCE)) return NULL;
		...
		GameobjectTypes type) const
{
	if (GameObject *go = GetMap()->GetGameObject(guid))
	{
		if (go->GetGoType() == type)
		{
			...
			if (go->IsWithinDistInMap(this, maxdist)) return go;
...
void Player::SetGameMaster(bool on)
{
	if (on)
	{
		...
		if (sWorld->getIntConfig(CONFIG_GM_DEV_TAG_ENABLE) == 1)
		{
			if (sWorld->getIntConfig(CONFIG_GM_DEV_TAG_LEVEL)
					== GetSession()->GetSecurity()) SetFlag(PLAYER_FLAGS,
					PLAYER_FLAGS_DEVELOPER);
			else SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM);
		}
		else SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM);
	...
	else
	{
		...
		if (!phases.empty()) for (AuraEffectList::const_iterator itr =
				phases.begin(); itr != phases.end(); ++itr)
			newPhase |= (*itr)->GetMiscValue();

		if (!newPhase) newPhase = PHASEMASK_NORMAL;

		m_ExtraFlags &= ~PLAYER_EXTRA_GM_ON;
		setFactionForRace(getRace());RemoveFlag
		...
		if (sWorld->IsFFAPvPRealm()) SetByteFlag(UNIT_FIELD_BYTES_2, 1,
...
void Player::GiveXP(uint32 xp, Unit *victim, float group_rate)
{
	...
	if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_NO_XP_GAIN)) return;

	if (victim && victim->GetTypeId() == TYPEID_UNIT
			&& !victim->ToCreature()->hasLootRecipient()) return;
	...
	if (HasAura(78631)) favored_exp_guild_low = 0.05f;
	if (HasAura(78632)) favored_exp_guild_high = 0.1f;
	if ((HasAura(32096) || HasAura(32098))
			&& (zone == 3483 || zone == 3562 || zone == 3836 || zone == 3713
					|| zone == 3714)) favored_exp_mult = 0.05f; // Thrallmar's Favor and Honor Hold's Favor
	xp = uint32(
			xp
					* (1 + favored_exp_mult + favored_exp_guild_low
							+ favored_exp_guild_high));
	// Favored experience increase END

	// XP to money conversion processed in Player::RewardQuest
	if (level >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) return;

	uint32 bonus_xp = 0;
	bool recruitAFriend = GetsRecruitAFriendBonus(true);

	// RaF does NOT stack with rested experience
	if (recruitAFriend) bonus_xp = 2 * xp; // xp + bonus_xp must add up to 3 * xp for RaF; calculation for quests done client-side
	else bonus_xp = victim ? GetXPRestBonus(xp) : 0; // XP resting bonus
	...
	while (newXP >= nextLvlXP && level < sWorld->getIntConfig(
			CONFIG_MAX_PLAYER_LEVEL))
	{
		newXP -= nextLvlXP;

		if (level < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) GiveLevel(
				level + 1);
...
void Player::GiveLevel(uint8 level)
{
	...
	if (Guild * pGuild = sObjectMgr->GetGuildById(this->GetGuildId())) pGuild->UpdateMemberData(
			this, GUILD_MEMBER_DATA_LEVEL, level);
	...
	SetPower(POWER_MANA, GetMaxPower(POWER_MANA));
	SetPower(POWER_ENERGY, GetMaxPower(POWER_ENERGY));
	if (GetPower(POWER_RAGE) > GetMaxPower(POWER_RAGE)) SetPower(POWER_RAGE,
	...
	if (Pet* pet = GetPet()) pet->SynchronizeLevelWithOwner();
...
void Player::InitTalentForLevel()
{
	...
	else
	{
		...
		{
			if (GetSession()->GetSecurity() < SEC_ADMINISTRATOR) resetTalents(
					true);
			else SetFreeTalentPoints(0);
		}
		// else update amount of free points
		else SetFreeTalentPoints(talentPointsForLevel - m_usedTalentCount);
	}

	if (!GetSession()->PlayerLoading()) SendTalentsInfoData(false); // update at client
}
...
void Player::SendInitialSpells()
{
			...
			itr != m_spells.end(); ++itr)
	{
		...
		if (!itr->second->active || itr->second->disabled) continue;
		...
		uint32 item_count)
{
	...
	if (mailError == MAIL_ERR_EQUIP_ERROR) data << (uint32) equipError;
	...
	for (PlayerMails::iterator itr = m_mail.begin(); itr != m_mail.end(); ++itr)
	{
		if ((*itr)->deliver_time > cTime)
		{
			if (!m_nextMailDelivereTime
					|| m_nextMailDelivereTime > (*itr)->deliver_time) m_nextMailDelivereTime =
					(*itr)->deliver_time;
		}
		else if (((*itr)->checked & MAIL_CHECK_MASK_READ) == 0) ++unReadMails;
	}
}
...
void Player::AddNewMailDeliverTime(time_t deliver_time)
{
	...
	{
		if (!m_nextMailDelivereTime || m_nextMailDelivereTime > deliver_time) m_nextMailDelivereTime =
				deliver_time;
	}
}
...
bool Player::AddTalent(uint32 spell_id, uint8 spec, bool learning)
{
	...
	if (itr != m_talents [spec]->end()) itr->second->state =
		...
		bool dependent, bool disabled)
{
	...
	if (itr != m_spells.end() && itr->second->state == PLAYERSPELL_TEMPORARY) RemoveTemporarySpell(
			spell_id);
	else if (itr != m_spells.end())
	{
		...
		{
			itr->second->dependent = dependent;
			if (itr->second->state != PLAYERSPELL_NEW) itr->second->state =
					PLAYERSPELL_CHANGED;
			dependent_set = true;
		}
				...
				&& !itr->second->disabled)
		{
			...
			itr->second->state = PLAYERSPELL_UNCHANGED;
			else if (itr->second->state != PLAYERSPELL_NEW) itr->second->state =
					PLAYERSPELL_CHANGED;

			if (active)
			{
				if (IsPassiveSpell(spell_id)
						&& IsNeedCastPassiveSpellAtLearn(spellInfo)) CastSpell(
						this, spell_id, true);
			}
			else if (IsInWorld())
			{
		...
		else switch (itr->second->state)
		{
			...
			default: // known not saved yet spell (new or modified)
			{
				...
				if (!IsInWorld() && !learning && !dependent_set) itr->second->state =
						PLAYERSPELL_UNCHANGED;
	...
	if (!disabled_case) // skip new spell adding if spell already known (disabled spells case)
	{
				...
				if (sSpellMgr->IsRankSpellDueToSpell(spellInfo, itr2->first))
				{
					if (itr2->second->active)
					{
						if (sSpellMgr->IsHighRankOfSpell(spell_id, itr2->first))
						{
							...
							if (itr2->second->state != PLAYERSPELL_NEW) itr2->second->state =
									PLAYERSPELL_CHANGED;
								...
								spell_id))
						{
							...
							if (newspell->state != PLAYERSPELL_NEW) newspell->state =
									PLAYERSPELL_CHANGED;
						}
					}
				}
		...
		if (newspell->disabled) return false;
	}
	...
	{
		if (IsNeedCastPassiveSpellAtLearn(spellInfo)) CastSpell(this, spell_id,
				true);
	}
	...
	{
		if (sSpellMgr->IsPrimaryProfessionFirstRankSpell(spell_id)) SetFreePrimaryProfessions(
				freeProfs - 1);
	}
	...
	if (spellLearnSkill)
	{
		...
		if (skill_value < spellLearnSkill->value) skill_value =
				spellLearnSkill->value;

		uint32 new_skill_max_value =
				spellLearnSkill->maxvalue == 0 ?
						maxskill : spellLearnSkill->maxvalue;

		if (skill_max_value < new_skill_max_value) skill_max_value =
				new_skill_max_value;
	...
	else
	{
				...
				_spell_idx != skill_bounds.second; ++_spell_idx)
		{
			...
			if (!Has310Flyer(false) && pSkill->id == SKILL_MOUNTS) for (uint8 i =
					0; i < MAX_SPELL_EFFECTS; ++i)
				if (spellInfo->EffectApplyAuraName [i]
						== SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED
						&& SpellMgr::CalculateSpellEffectAmount(spellInfo, i)
								== 310) SetHas310Flyer(true);

			if (HasSkill(pSkill->id)) continue;
							...
							&& _spell_idx->second->max_value == 0))
			{
						...
						_spell_idx->second->racemask != 0))
				{
					case SKILL_RANGE_LANGUAGE:
						SetSkill(pSkill->id, GetSkillStep(pSkill->id), 300,
								300);
						break;
...
void Player::RemoveTemporarySpell(uint32 spellId)
{
	...
	if (itr->second->state != PLAYERSPELL_TEMPORARY) return;
	delete itr->second;
	m_spells.erase(itr);
}
...
void Player::learnSpell(uint32 spell_id, bool dependent)
{
	...
	if (disabled)
	{
		...
		if (node)
		{
			...
			if (iter != m_spells.end() && iter->second->disabled) learnSpell(
					node->next, false);
		}
				...
				++itr2)
		{
			...
			if (iter2 != m_spells.end() && iter2->second->disabled) learnSpell(
					itr2->second, false);
		}
	}
}
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
	...
	if (itr->second->state == PLAYERSPELL_REMOVED
			...
			|| itr->second->state == PLAYERSPELL_TEMPORARY) return;

	// unlearn non talent higher ranks (recursive)
	if (SpellChainNode const* node = sSpellMgr->GetSpellChainNode(spell_id))
	{
		if (HasSpell(node->next) && !GetTalentSpellPos(node->next)) removeSpell(
	...
	if (itr == m_spells.end()) return; // already unleared
	...
	else
	{
		...
		else itr->second->state = PLAYERSPELL_REMOVED;
	}
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
		if (PetAura const *petSpell = sSpellMgr->GetPetAura(spell_id, i)) RemovePetAura(
				petSpell);
	...
	{
		if (talentCosts < m_usedTalentCount) m_usedTalentCount -= talentCosts;
	...
	if (sSpellMgr->IsPrimaryProfessionFirstRankSpell(spell_id))
	{
		...
		if (freeProfs <= sWorld->getIntConfig(CONFIG_MAX_PRIMARY_TRADE_SKILL)) SetFreePrimaryProfessions(
				freeProfs);
	}
	...
	if (spellLearnSkill)
	{
		...
		else
		{
			...
			else // set to prev. skill setting values
			{
				...
				if (skill_value > prevSkill->value) skill_value =
						prevSkill->value;
							...
							&& _spell_idx->second->max_value == 0))
			{
				...
				if ((pSkill->categoryId == SKILL_CATEGORY_SECONDARY
						...
						&& (IsProfessionSkill(pSkill->id)
								|| _spell_idx->second->racemask != 0)) continue;
				...
				for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
					if (pSpellInfo->EffectApplyAuraName [i]
							...
							&& SpellMgr::CalculateSpellEffectAmount(pSpellInfo,
									i) == 310) Has310Flyer(true, spell_id); // with true as first argument its also used to set/remove the flag
			}
		}
	}
				...
				&& sSpellMgr->GetSpellRank(spellInfo->Id) != 0)
		{
			...
			if (prev_itr != m_spells.end())
			{
				if (prev_itr->second->dependent != cur_dependent)
				{
					...
					if (prev_itr->second->state != PLAYERSPELL_NEW) prev_itr->second->state =
							PLAYERSPELL_CHANGED;
				}
	...
	if (spell_id == 46917 && m_canTitanGrip) SetCanTitanGrip(false);
	if (spell_id == 674 && m_canDualWield) SetCanDualWield(false);

	if (sWorld->getBoolConfig(CONFIG_OFFHAND_CHECK_AT_SPELL_UNLEARN)) AutoUnequipOffhandIfNeed();
...
{
	if (!checkAllSpells) return m_ExtraFlags & PLAYER_EXTRA_HAS_310_FLYER;
	else
	{
		...
		for (PlayerSpellMap::iterator itr = m_spells.begin();
				itr != m_spells.end(); ++itr)
		{
			if (itr->first == excludeSpellId) continue;
			...
			for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first;
					_spell_idx != bounds.second; ++_spell_idx)
			{
				if (_spell_idx->second->skillId != SKILL_MOUNTS) break; // We can break because mount spells belong only to one skillline (at least 310 flyers do)
...
void Player::RemoveSpellCooldown(uint32 spell_id, bool update /* = false */)
{
	...
	if (update) SendClearCooldown(spell_id, this);
}
...
void Player::RemoveCategoryCooldown(uint32 cat)
{
	SpellCategoryStore::const_iterator i_scstore = sSpellCategoryStore.find(
			cat);
	if (i_scstore != sSpellCategoryStore.end()) for (SpellCategorySet::const_iterator i_scset =
...
void Player::RemoveSpellCategoryCooldown(uint32 cat, bool update /* = false */)
{
	...
	if (ct == sSpellCategoryStore.end()) return;

	const SpellCategorySet& ct_set = ct->second;
	for (SpellCooldowns::const_iterator i = m_spellCooldowns.begin();
			i != m_spellCooldowns.end();)
	{
		if (ct_set.find(i->first) != ct_set.end()) RemoveSpellCooldown(
				(i++)->first, update);
		else ++i;
	}
}
...
void Player::RemoveArenaSpellCooldowns(bool removeActivePetCooldowns)
{
	...
	// pet cooldowns
	if (removeActivePetCooldowns) if (Pet *pet = GetPet())
	{
		// notify player
...
void Player::_SaveSpellCooldowns(SQLTransaction& trans)
{
	...
	{
		if (itr->second.end <= curTime) m_spellCooldowns.erase(itr++);
	...
	if (!first_round) trans->Append(ss.str().c_str());
}

uint32 Player::resetTalentsCost() const
{
	// The first time reset costs 1 gold
	if (m_resetTalentsCost < 1 * GOLD) return 1 * GOLD;
	// then 5 gold
	else if (m_resetTalentsCost < 5 * GOLD) return 5 * GOLD;
	// After that it increases in increments of 5 gold
	else if (m_resetTalentsCost < 10 * GOLD) return 10 * GOLD;
	else
	{
		...
		else
		{
			...
			if (new_cost > 50 * GOLD) new_cost = 50 * GOLD;
			return new_cost;
		}
	}
}
...
bool Player::resetTalents(bool no_cost)
{
	...
	if (HasAtLoginFlag(AT_LOGIN_RESET_TALENTS)) RemoveAtLoginFlag(
			AT_LOGIN_RESET_TALENTS, true);
	...
	for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i)
	{
		...
		for (int8 rank = MAX_TALENT_RANK - 1; rank >= 0; --rank)
		{
			...
			if (const SpellEntry *_spellEntry = sSpellStore.LookupEntry(talentInfo->RankID[rank])) for (uint8 i =
					0; i < MAX_SPELL_EFFECTS; ++i) // search through the SpellEntry for valid trigger spells
				if (_spellEntry->EffectTriggerSpell [i] > 0
						&& _spellEntry->Effect [i] == SPELL_EFFECT_LEARN_SPELL) removeSpell(
						_spellEntry->EffectTriggerSpell [i], true); // and remove any spells that the talent teaches
			// if this talent rank can be found in the PlayerTalentMap, mark the talent as removed so it gets deleted
			PlayerTalentMap::iterator plrTalent =
					m_talents [m_activeSpec]->find(talentInfo->RankID [rank]);
			if (plrTalent != m_talents [m_activeSpec]->end()) plrTalent->second->state =
					PLAYERSPELL_REMOVED;
		}
	}
	...
	for (uint32 i = 0; i < sTalentTreePrimarySpellsStore.GetNumRows(); ++i)
	{
		...
		if (!talentInfo
				|| talentInfo->TalentTabID != GetTalentBranchSpec(m_activeSpec)) continue;
...
{
	for (PlayerMails::iterator itr = m_mail.begin(); itr != m_mail.end(); ++itr)
		if ((*itr)->messageID == id) return (*itr);
...
{
	if (target == this) Object::_SetCreateBits(updateMask, target);
	else
	{
		for (uint16 index = 0; index < PLAYER_END_NOT_SELF; index++)
			if (GetUInt32Value(index) != 0 && updateVisualBits.GetBit(index)) updateMask->SetBit(
					index);
	}
}
...
{
	if (target == this) Object::_SetUpdateBits(updateMask, target);
	else
	{
...
{
	if (!trainer_spell) return TRAINER_SPELL_RED;

	bool hasSpell = true;
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
	{
		if (!trainer_spell->learnedSpell [i]) continue;
	...
	// known spell
	if (hasSpell) return TRAINER_SPELL_GRAY;

	// check skill requirement
	if (trainer_spell->reqSkill
			&& GetBaseSkillValue(trainer_spell->reqSkill)
					< trainer_spell->reqSkillValue) return TRAINER_SPELL_RED;

	// check level requirement
	if (getLevel() < trainer_spell->reqLevel) return TRAINER_SPELL_RED;
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
	{
		...
		if (!IsSpellFitByClassAndRace(trainer_spell->learnedSpell [i])) return TRAINER_SPELL_RED;

		if (SpellChainNode const* spell_chain = sSpellMgr->GetSpellChainNode(trainer_spell->learnedSpell[i]))
		{
			// check prev.rank requirement
			if (spell_chain->prev && !HasSpell(spell_chain->prev)) return TRAINER_SPELL_RED;
		}
				...
				itr != spellsRequired.second; ++itr)
		{
			...
			if (!HasSpell(itr->second)) return TRAINER_SPELL_RED;
		}
	}
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
	{
		...
		if ((sSpellMgr->IsPrimaryProfessionFirstRankSpell(
				trainer_spell->learnedSpell [i]))
				&& (GetFreePrimaryProfessionPoints() == 0)) return TRAINER_SPELL_GREEN_DISABLED;
	}
		...
		bool updateRealmChars, bool deleteFinally)
{
	...
	if (accountId == 0) updateRealmChars = false;
	...
	if (deleteFinally || Player::GetLevelFromDB(playerguid) < charDelete_minLvl) charDelete_method =
			CHAR_DELETE_REMOVE;
	...
	if (uint32 guildId = GetGuildIdFromDB(playerguid)) if (Guild * pGuild = sObjectMgr->GetGuildById(guildId)) pGuild->DeleteMember(
			guid);
	...
	if (resultGroup) if (Group * group = sObjectMgr->GetGroupByGUID(
			(*resultGroup)[0].GetUInt32())) RemoveFromGroup(group,
			playerguid);
	...
	switch (charDelete_method)
	{
		...
		case CHAR_DELETE_REMOVE:
		{
			...
			if (resultMail)
			{
				do
				{
					...
					if (mailTemplateId) draft = MailDraft(mailTemplateId,
					...
					if (has_items)
					{
						...
						PreparedQueryResult resultItems =
			...
			if (QueryResult resultFriends = CharacterDatabase.PQuery("SELECT DISTINCT guid FROM character_social WHERE friend = '%u'", guid))
			{
				do
				{
							...
							MAKE_NEW_GUID((*resultFriends)[0].GetUInt32(), 0, HIGHGUID_PLAYER)))
					{
						...
						{
							pFriend->GetSocial()->RemoveFromSocialList(guid,
									false);
							sSocialMgr->SendFriendStatus(pFriend,
									FRIEND_REMOVED, guid, false);
						}
					}
				}
			...
			trans->PAppend(
					"DELETE FROM character_account_data WHERE guid = '%u'",
					guid);
			trans->PAppend(
					"DELETE FROM character_declinedname WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_action WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_aura WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_gifts WHERE guid = '%u'",
					guid);
			...
			trans->PAppend("DELETE FROM character_inventory WHERE guid = '%u'",
					guid);
			trans->PAppend(
					"DELETE FROM character_queststatus WHERE guid = '%u'",
					guid);
			trans->PAppend(
			...
			trans->PAppend("DELETE FROM character_reputation WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_spell WHERE guid = '%u'",
					guid);
			trans->PAppend(
					"DELETE FROM character_spell_cooldown WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM gm_tickets WHERE playerGuid = '%u'",
					guid);
			...
			trans->PAppend("DELETE FROM mail_items WHERE receiver = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_pet WHERE owner = '%u'",
					guid);
			trans->PAppend(
					"DELETE FROM character_pet_declinedname WHERE owner = '%u'",
					guid);
			trans->PAppend(
					"DELETE FROM character_achievement WHERE guid = '%u' " // NOTE: These achievements have flags & 256 in DBC.
			...
			trans->PAppend(
					...
					guid);
			trans->PAppend(
					"DELETE FROM character_equipmentsets WHERE guid = '%u'",
					guid);
			trans->PAppend(
			...
			trans->PAppend(
					...
					guid);
			trans->PAppend("DELETE FROM character_talent WHERE guid = '%u'",
					guid);
			trans->PAppend("DELETE FROM character_skills WHERE guid = '%u'",
					guid);
		...
		default:
			sLog->outError(
					"Player::DeleteFromDB: Unsupported delete method: %u.",
					charDelete_method);
	}
...
void Player::BuildPlayerRepop()
{
	...
	if (getRace() == RACE_NIGHTELF) CastSpell(this, 20584, true);
	...
	if (!GetSession()->isLogingOut()) SetMovement(MOVE_UNROOT);
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	if (getRace() == RACE_NIGHTELF) RemoveAurasDueToSpell(20584); // speed bonuses
	...
	if (InBattleground())
	{
		if (Battleground* bg = GetBattleground()) bg->HandlePlayerResurrect(
				this);
	}
...
bool Player::FallGround(uint8 FallMode)
{
	...
	if (getDeathState() == DEAD_FALLING && FallMode == 0) return false;

	float x, y, z;
	GetPosition(x, y, z);
	float ground_Z = GetMap()->GetHeight(x, y, z);
	float z_diff = 0.0f;
	if ((z_diff = fabs(ground_Z - z)) < 0.1f) return false;
	...
	if (FallMode == 2 && z_diff >= 14.57f)
	{
		uint32 damage = std::min(
				GetMaxHealth(),
				(uint32)((0.018f * z_diff - 0.2426f) * GetMaxHealth() * sWorld
						->getRate(RATE_DAMAGE_FALL)));
		if (damage > 0) EnvironmentalDamage(DAMAGE_FALL, damage);
	}
	else if (FallMode == 0) Unit::setDeathState(DEAD_FALLING);
	return true;
}
...
void Player::CreateCorpse()
{
	...
	if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM)) flags |=
			CORPSE_FLAG_HIDE_HELM;
	if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_CLOAK)) flags |=
			CORPSE_FLAG_HIDE_CLOAK;
	if (InBattleground() && !InArena()) flags |= CORPSE_FLAG_LOOTABLE; // to be able to remove insignia
	...
	if (!GetMap()->IsBattlegroundOrArena()) corpse->SaveToDB();
...
{
	if (sObjectAccessor->ConvertCorpseForPlayer(GetGUID())) if (!GetSession()->PlayerLogoutWithSave()) // at logout we will already store the player
...
void Player::DurabilityLossAll(double percent, bool inventory)
{
	for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; i++)
		if (Item *pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) DurabilityLoss(
				pItem, percent);
	...
	if (inventory)
	{
		...
		for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END;
				i++)
			if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
					0; j < pBag->GetBagSize(); j++)
				if (Item* pItem = GetItemByPos(i, j)) DurabilityLoss(pItem,
						percent);
	}
}
...
{
	for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; i++)
		if (Item *pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) DurabilityPointsLoss(
				pItem, points);
	...
	if (inventory)
	{
		...
		for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END;
			...
			if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
					0; j < pBag->GetBagSize(); j++)
				if (Item* pItem = GetItemByPos(i, j)) DurabilityPointsLoss(
						pItem, points);
	}
}
...
void Player::DurabilityPointsLoss(Item* item, int32 points)
{
	...
	else if (pNewDurability > pMaxDurability) pNewDurability = pMaxDurability;

	if (pOldDurability != pNewDurability)
	{
		// modify item stats _before_ Durability set to 0 to pass _ApplyItemMods internal check
		if (pNewDurability == 0 && pOldDurability > 0 && item->IsEquipped()) _ApplyItemMods(
...
{
	if (Item *pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) DurabilityPointsLoss(
			pItem, 1);
}
		...
		bool guildBank)
{
	...
	uint32 TotalCost = 0;
	if (!item) return TotalCost;

	uint32 maxDurability = item->GetUInt32Value(ITEM_FIELD_MAXDURABILITY);
	if (!maxDurability) return TotalCost;
	...
	if (IsEquipmentPos(pos) && !curDurability) _ApplyItemMods(item, pos & 255,
			true);
	return TotalCost;
}
...
void Player::RepopAtGraveyard()
{
	...
	if (Battleground *bg = GetBattleground()) ClosestGrave =
			bg->GetClosestGraveYard(this);
	else ClosestGrave = sObjectMgr->GetClosestGraveYard(GetPositionX(),
	...
	else if (GetPositionZ() < -500.0f) TeleportTo(m_homebindMapId, m_homebindX,
			m_homebindY, m_homebindZ, GetOrientation());
}
		...
		AreaTableEntry const* zone)
{
	...
	{
		if (zone->flags & AREA_FLAG_ARENA_INSTANCE) return false;

		if ((channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
				&& !(zone->flags & AREA_FLAG_CAPITAL)) return false;
	}
...
void Player::CleanupChannels()
{
	while (!m_channels.empty())
	{
		...
		if (ChannelMgr* cMgr = channelMgr(GetTeam())) cMgr->LeftChannel(
				ch->GetName()); // deleted channel if empty
	}
...
{
	if (GetSession()->PlayerLoading() && !IsBeingTeleportedFar()) return; // The client handles it automatically after loading, but not after teleporting
	...
	for (uint32 i = 0; i < sChatChannelsStore.GetNumRows(); ++i)
	{
		...
		{
			if (!(channel->flags & CHANNEL_DBC_FLAG_ZONE_DEP)) continue; // Not zone dependent, don't handle it here

			if ((channel->flags & CHANNEL_DBC_FLAG_GUILD_REQ) && GetGuildId()) continue; // Should not join to these channels automatically
			...
			if (CanJoinConstantChannelInZone(channel, current_zone))
			{
				...
				{
					if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY
							&& usedChannel) continue; // Already on the channel, as city channel names are not changing

					char new_channel_name_buf [100];
					char const* currentNameExt;

					if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY) currentNameExt =
							sObjectMgr->GetArkCoreStringForDBCLocale(
									LANG_CHANNEL_CITY);
					else currentNameExt = current_zone_name.c_str();
					...
					if (usedChannel)
					{
						...
						else joinChannel = NULL;
					}
				}
				else joinChannel = cMgr->GetJoinChannel(channel->pattern,
						channel->ChannelID);
			}
			else removeChannel = usedChannel;

			if (joinChannel) joinChannel->Join(GetGUID(), ""); // Changed Channel: ... or Joined Channel: ...
		...
		float amount, bool apply)
{
	...
	switch (modType)
	{
		case FLAT_MOD:
			...
			break;
		case PCT_MOD:
			if (amount <= -100.0f) amount = -200.0f;
...
float Player::GetBaseModValue(BaseModGroup modGroup, BaseModType modType) const
{
	...
	if (modType == PCT_MOD && m_auraBaseMod [modGroup] [PCT_MOD] <= 0.0f) return 0.0f;
...
float Player::GetTotalBaseModValue(BaseModGroup modGroup) const
{
	...
	if (m_auraBaseMod [modGroup] [PCT_MOD] <= 0.0f) return 0.0f;
...
float Player::GetMeleeCritFromAgility()
{
	...
	if (critBase == NULL || critRatio == NULL) return 0.0f;
...
float Player::GetDodgeFromAgility()
{
	...
	GtChanceToMeleeCritEntry const *dodgeRatio =
			sGtChanceToMeleeCritStore.LookupEntry(
					(pclass - 1) * GT_MAX_LEVEL + level - 1);
	if (dodgeRatio == NULL || pclass > MAX_CLASSES) return 0.0f;
...
float Player::GetRatingCoefficient(CombatRating cr) const
{
	...
	if (Rating == NULL) return 1.0f; // By default use minimum coefficient (not must be called)
...
void Player::UpdateRating(CombatRating cr)
{
	...
	for (AuraEffectList::const_iterator i = modRatingFromStat.begin();
		...
		if ((*i)->GetMiscValue() & (1 << cr)) amount += int32(
	...
	switch (cr)
	{
		...
		case CR_CRIT_MELEE:
			...
			break;
		case CR_CRIT_RANGED:
			if (affectStats) UpdateCritPercentage(RANGED_ATTACK);
			break;
		case CR_CRIT_SPELL:
			if (affectStats) UpdateAllSpellCritChances();
			break;
		...
		case CR_ARMOR_PENETRATION:
			if (affectStats) UpdateArmorPenetration(amount);
			break;
		default:
			break;
	}
...
void Player::SetRegularAttackTime()
{
	for (uint8 i = 0; i < MAX_ATTACK; ++i)
	{
		...
		if (tmpitem && !tmpitem->IsBroken())
		{
			...
			if (proto->Delay) attackTime = proto->Delay;
		}
...
bool Player::UpdateSkill(uint32 skill_id, uint32 step)
{
	...
	if (skill_id == SKILL_FIST_WEAPONS) skill_id = SKILL_UNARMED;

	SkillStatusMap::iterator itr = mSkillStatus.find(skill_id);
	if (itr == mSkillStatus.end() || itr->second.uState == SKILL_DELETED) return false;
	...
	if (value < max)
	{
		...
		if (itr->second.uState != SKILL_NEW) itr->second.uState = SKILL_CHANGED;
...
{
	if (SkillValue >= GrayLevel) return sWorld->getIntConfig(
			CONFIG_SKILL_CHANCE_GREY) * 10;
	if (SkillValue >= GreenLevel) return sWorld->getIntConfig(
			CONFIG_SKILL_CHANCE_GREEN) * 10;
	if (SkillValue >= YellowLevel) return sWorld->getIntConfig(
			CONFIG_SKILL_CHANCE_YELLOW) * 10;
...
bool Player::UpdateCraftSkill(uint32 spellid)
{
			...
			_spell_idx != bounds.second; ++_spell_idx)
	{
		if (_spell_idx->second->skillId)
		{
			...
			{
				if (uint32 discoveredSpell = GetSkillDiscoverySpell(_spell_idx->second->skillId, spellid, this)) learnSpell(
		...
		uint32 RedLevel, uint32 Multiplicator)
{
	...
	switch (SkillId)
	{
		...
		case SKILL_SKINNING:
			if (sWorld->getIntConfig(CONFIG_SKILL_CHANCE_SKINNING_STEPS) == 0) return UpdateSkillPro(
					SkillId,
					SkillGainChance(SkillValue, RedLevel + 100, RedLevel + 50,
							RedLevel + 25) * Multiplicator,
					gathering_skill_gain);
			else return UpdateSkillPro(
					SkillId,
					(SkillGainChance(SkillValue, RedLevel + 100, RedLevel + 50,
							RedLevel + 25) * Multiplicator)
							>> (SkillValue / sWorld->getIntConfig(
									CONFIG_SKILL_CHANCE_SKINNING_STEPS)),
					gathering_skill_gain);
		case SKILL_MINING:
			if (sWorld->getIntConfig(CONFIG_SKILL_CHANCE_MINING_STEPS) == 0) return UpdateSkillPro(
					SkillId,
					SkillGainChance(SkillValue, RedLevel + 100, RedLevel + 50,
							RedLevel + 25) * Multiplicator,
					gathering_skill_gain);
...
bool Player::UpdateFishingSkill()
{
	...
	if (SkillValue <= 115) chance = 100.0f;
	else if (SkillValue <= 150) chance = 50.0f;
	else if (SkillValue <= 170) chance = 33.33f;
	else if (SkillValue <= 190) chance = 25.0f;
	else if (SkillValue <= 215) chance = 20.0f;
	else if (SkillValue <= 235) chance = 16.67f;
	else if (SkillValue <= 260) chance = 14.29f;
	else if (SkillValue <= 280) chance = 12.5f;
	else if (SkillValue <= 325) chance = 11.11f;
	else if (SkillValue <= 365) chance = 10.0f;
	else if (SkillValue <= 450) chance = 9.1f;
	else if (SkillValue <= 500) chance = 11.11f;
	else chance = 10.0f;
...
static const size_t bonusSkillLevelsSize = sizeof(bonusSkillLevels)
		/ sizeof(uint32);
...
bool Player::UpdateSkillPro(uint16 SkillId, int32 Chance, uint32 step)
{
	...
	if (!MaxValue || !SkillValue || SkillValue >= MaxValue) return false;
...
void Player::UpdateWeaponSkill(WeaponAttackType attType)
{
	...
	if (victim && victim->GetTypeId() == TYPEID_PLAYER) return;

	if (IsInFeralForm()) return;  // always maximized SKILL_FERAL_COMBAT in fact

	if (GetShapeshiftForm() == FORM_TREE) return; // use weapon but not skill up

	if (victim && victim->GetTypeId() == TYPEID_UNIT
			&& (victim->ToCreature()->GetCreatureInfo()->flags_extra
					& CREATURE_FLAG_EXTRA_NO_SKILLGAIN)) return;

	uint32 weapon_skill_gain = sWorld->getIntConfig(CONFIG_SKILL_GAIN_WEAPON);

	Item* tmpitem = GetWeaponForAttack(attType, true);
	if (!tmpitem && attType == BASE_ATTACK) UpdateSkill(SKILL_UNARMED,
			weapon_skill_gain);
	else if (tmpitem
			&& tmpitem->GetProto()->SubClass
					!= ITEM_SUBCLASS_WEAPON_FISHING_POLE) UpdateSkill(
			tmpitem->GetSkill(), weapon_skill_gain);
		...
		bool defence)
{
	...
	if (moblevel < greylevel) return;

	if (moblevel > plevel + 5) moblevel = plevel + 5;

	uint8 lvldif = moblevel - greylevel;
	if (lvldif < 3) lvldif = 3;

	uint32 skilldif = 5 * plevel
			- (defence ?
					GetBaseDefenseSkillValue() :
					GetBaseWeaponSkillValue(attType));
	if (skilldif <= 0) return;

	float chance = float(3 * lvldif * skilldif) / plevel;
	if (!defence) if (getClass() == CLASS_WARRIOR || getClass() == CLASS_ROGUE) chance +=
	...
	{
		if (defence) UpdateDefense();
		else UpdateWeaponSkill(attType);
	}
	else return;
}
...
void Player::ModifySkillBonus(uint32 skillid, int32 val, bool talent)
{
	...
	if (itr == mSkillStatus.end() || itr->second.uState == SKILL_DELETED) return;
...
void Player::UpdateSkillsForLevel()
{
	...
	{
		if (itr->second.uState == SKILL_DELETED) continue;

		uint32 pskill = itr->first;
		SkillLineEntry const *pSkill = sSkillLineStore.LookupEntry(pskill);
		if (!pSkill) continue;

		if (GetSkillRangeType(pSkill, false) != SKILL_RANGE_LEVEL) continue;
		...
		if (max != 1)
		{
			...
			if (!sWorld->getBoolConfig(CONFIG_USE_OLD_SKILL_SYSTEM))
			{
				...
				if (itr->second.uState != SKILL_NEW) itr->second.uState =
						SKILL_CHANGED;
			}
...
void Player::UpdateSkillsToMaxSkillsForLevel()
{
			...
			itr != mSkillStatus.end(); ++itr)
	{
		...
		uint32 pskill = itr->first;
		if (IsProfessionOrRidingSkill(pskill)) continue;
		uint32 valueIndex = PLAYER_SKILL_VALUE_INDEX(itr->second.pos);
		uint32 data = GetUInt32Value(valueIndex);
		uint32 max = SKILL_MAX(data);
		...
		if (pskill == SKILL_DEFENSE) UpdateDefenseBonusesMod();
	}
}
...
void Player::SetSkill(uint16 id, uint16 step, uint16 newVal, uint16 maxVal)
{
	...
	if (itr != mSkillStatus.end() && itr->second.uState != SKILL_DELETED)
	{
		...
		if (newVal)
		{
			...
			if (newVal < currVal) UpdateSkillEnchantments(id, currVal, newVal);
			// update step
		...
		else //remove
		{
			...
			if (itr->second.uState != SKILL_NEW) itr->second.uState =
					SKILL_DELETED;
			else mSkillStatus.erase(itr);

			// remove all spells that related to this skill
			for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
				if (SkillLineAbilityEntry const *pAbility = sSkillLineAbilityStore.LookupEntry(j)) if (pAbility->skillId
						== id) removeSpell(
						sSpellMgr->GetFirstSpellInChain(pAbility->spellId));
		}
	}
	else if (newVal) //add
	{
		...
		for (int i = 0; i < PLAYER_MAX_SKILLS; ++i)
			if (!GetUInt32Value(PLAYER_SKILL_INDEX(i)))
			{
				...
				for (AuraEffectList::const_iterator j = mModSkill.begin();
						j != mModSkill.end(); ++j)
					if ((*j)->GetMiscValue() == int32(id)) (*j)->HandleEffect(
							this, 0, true);
	...
	if (state != 2)
	{
		for (uint8 button = 0; button < MAX_ACTION_BUTTONS; ++button)
		{
			...
			if (itr != m_actionButtons.end()
					&& itr->second.uState != ACTIONBUTTON_DELETED) data
					<< uint32(itr->second.packedData);
			else data << uint32(0);
		}
	}
...
{
	if (!IsActionButtonDataValid(button, action, type)) return NULL;
...
void Player::removeActionButton(uint8 button)
{
	...
	if (buttonItr == m_actionButtons.end()
			|| buttonItr->second.uState == ACTIONBUTTON_DELETED) return;

	if (buttonItr->second.uState == ACTIONBUTTON_NEW) m_actionButtons.erase(
			buttonItr); // new and not saved
	else buttonItr->second.uState = ACTIONBUTTON_DELETED; // saved, will deleted at next save
...
ActionButton const* Player::GetActionButton(uint8 button)
{
	ActionButtonList::iterator buttonItr = m_actionButtons.find(button);
	if (buttonItr == m_actionButtons.end()
			|| buttonItr->second.uState == ACTIONBUTTON_DELETED) return NULL;
...
bool Player::SetPosition(float x, float y, float z, float orientation,
		bool teleport)
{
	if (!Unit::SetPosition(x, y, z, orientation, teleport)) return false;
	...
	// group update
	if (GetGroup()) SetGroupUpdateFlag(GROUP_UPDATE_FLAG_POSITION);
	...
	if (GetTrader() && !IsWithinDistInMap(GetTrader(), INTERACTION_DISTANCE)) GetSession()->SendCancelTrade();
...
{
	if (self) GetSession()->SendPacket(data);
...
{
	if (skipped_rcvr != this) GetSession()->SendPacket(data);
...
{
	if (m_session) m_session->SendPacket(data);
}
...
void Player::CheckAreaExploreAndOutdoor()
{
	...
	if (!m_AreaID) m_AreaID = GetAreaId();
	if (m_AreaID != GetAreaId()) m_AreaID = GetAreaId();

	bool isOutdoor;
	uint16 areaFlag = GetBaseMap()->GetAreaFlag(GetPositionX(), GetPositionY(),
			GetPositionZ(), &isOutdoor);

	if (sWorld->getBoolConfig(CONFIG_VMAP_INDOOR_CHECK) && !isOutdoor) RemoveAurasWithAttribute(SPELL_ATTR0_OUTDOORS_ONLY);
...
void Player::RewardOnKill(Unit *victim, float rate)
{
	...
	if (victim->ToCreature()->IsReputationGainDisabled()) return;

	RewardOnKillEntry const* Rew = sObjectMgr->GetRewardOnKillEntry(
			victim->ToCreature()->GetCreatureInfo()->Entry);
		...
		if (map && map->IsDungeon())
		{
			...
			if (dungeonLevel)
			{
				InstanceTemplate const *instance =
						sObjectMgr->GetInstanceTemplate(map->GetId());
				if (instance)
				{
					AccessRequirement const *pAccessRequirement =
							sObjectMgr->GetAccessRequirement(map->GetId(),
									((InstanceMap*) map)->GetDifficulty());
					if (pAccessRequirement)
					{
						if (!map->IsRaid()
								&& pAccessRequirement->levelMin >= dungeonLevel) ChampioningFaction =
								GetChampioningFaction();
					}
				}
			}
			else ChampioningFaction = GetChampioningFaction();
		}
	}
	...
	if ((HasAura(32096) || HasAura(32098))
			&& (zone == 3483 || zone == 3562 || zone == 3836 || zone == 3713
					|| zone == 3714)) favored_rep_mult = 0.25; // Thrallmar's Favor and Honor Hold's Favor
	else if (HasAura(30754)
			&& (Rew->repfaction1 == 609 || Rew->repfaction2 == 609)
			&& !ChampioningFaction) favored_rep_mult = 0.25; // Cenarion Favor
	...
	{
		int32 donerep1 = CalculateReputationGain(victim->getLevel(),
				Rew->repvalue1,
				ChampioningFaction ? ChampioningFaction : Rew->repfaction1,
				false);
		donerep1 = int32(donerep1 * (rate + favored_rep_mult));

		if (recruitAFriend) donerep1 = int32(
				donerep1
						* (1 + sWorld->getRate(
		...
		uint32 current_reputation_rank1 = GetReputationMgr().GetRank(
				factionEntry1);
		if (factionEntry1
				&& current_reputation_rank1 <= Rew->reputation_max_cap1) GetReputationMgr().ModifyReputation(
				factionEntry1, donerep1);
	}

	if (Rew->repfaction2 && (!Rew->team_dependent || team == HORDE))
	{
		int32 donerep2 = CalculateReputationGain(victim->getLevel(),
				Rew->repvalue2,
				ChampioningFaction ? ChampioningFaction : Rew->repfaction2,
				false);
		donerep2 = int32(donerep2 * (rate + favored_rep_mult));

		if (recruitAFriend) donerep2 = int32(
				donerep2
						* (1 + sWorld->getRate(
								RATE_REPUTATION_RECRUIT_A_FRIEND_BONUS)));
		...
		uint32 current_reputation_rank2 = GetReputationMgr().GetRank(
				factionEntry2);
		if (factionEntry2
				&& current_reputation_rank2 <= Rew->reputation_max_cap2) GetReputationMgr().ModifyReputation(
				factionEntry2, donerep2);
	}
...
void Player::RewardReputation(Quest const *pQuest)
{
	...
	{
		if (!pQuest->RewRepFaction [i]) continue;
		if (pQuest->RewRepValue [i])
		{
			...
			if (recruitAFriend) rep = int32(
					rep
							* (1 + sWorld->getRate(
									RATE_REPUTATION_RECRUIT_A_FRIEND_BONUS)));

			if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(pQuest->RewRepFaction[i])) GetReputationMgr().ModifyReputation(
					factionEntry, rep);
		}
		else
		{
			...
			if (const QuestFactionRewEntry *pRow = sQuestFactionRewardStore.LookupEntry(row))
			{
				...
				if (!repPoints) continue;

				repPoints = CalculateReputationGain(GetQuestLevel(pQuest),
						repPoints, pQuest->RewRepFaction [i], true);

				if (recruitAFriend) repPoints =
						int32(
								repPoints
										* (1 + sWorld->getRate(
												RATE_REPUTATION_RECRUIT_A_FRIEND_BONUS)));

				if (const FactionEntry* factionEntry = sFactionStore.LookupEntry(pQuest->RewRepFaction[i])) GetReputationMgr().ModifyReputation(
						factionEntry, repPoints);
			}
		}
	}
}
		...
		bool pvptoken)
{
	...
	if (InArena())
	{
		if (!uVictim || uVictim == this
				|| uVictim->GetTypeId() != TYPEID_PLAYER) return false;

		if (GetBGTeam() == uVictim->ToPlayer()->GetBGTeam()) return false;

		return true;
	}

	// 'Inactive' this aura prevents the player from gaining honor points and battleground tokens
	if (HasAura(SPELL_AURA_PLAYER_INACTIVE)) return false;
	...
	if (InBattleground() && GetBattleground() && GetBattleground()->isArena()) return true;
	...
	{
		if (!uVictim || uVictim == this
				|| uVictim->HasAuraType(SPELL_AURA_NO_PVP_CREDIT)) return false;
		...
		if (uVictim->GetTypeId() == TYPEID_PLAYER)
		{
			...
			//  title[other]  -> 0
			if (victim_title == 0) victim_guid = 0; // Don't show HK: <rank> message, only log.
			else if (victim_title < 15) victim_rank = victim_title + 4;
			else if (victim_title < 29) victim_rank = victim_title - 14 + 4;
			else victim_guid = 0; // Don't show HK: <rank> message, only log.
		...
		{
			if (!uVictim->ToCreature()->isRacialLeader()) return false;
	...
	{
		if (!uVictim || uVictim == this
				|| uVictim->HasAuraType(SPELL_AURA_NO_PVP_CREDIT)) return true;
		...
		if (uVictim->GetTypeId() == TYPEID_PLAYER)
		{
			...
			if ((MapType == 1 && !InBattleground()
					...
					|| (MapType == 2
							&& !HasByteFlag(UNIT_FIELD_BYTES_2, 1,
									UNIT_BYTE2_FLAG_FFA_PVP))
					|| (MapType == 3 && !InBattleground())) return true;

			uint32 itemId = sWorld->getIntConfig(CONFIG_PVP_TOKEN_ID);
			int32 count = sWorld->getIntConfig(CONFIG_PVP_TOKEN_COUNT);

			if (AddItem(itemId, count)) ChatHandler(this).PSendSysMessage(
...
uint32 Player::GetZoneIdFromDB(uint64 guid)
{
	...
	if (!zone)
	{
		...
		if (zone > 0) CharacterDatabase.PExecute(
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
	...
	{
		if (Guild * pGuild = sObjectMgr->GetGuildById(GetGuildId())) pGuild->UpdateMemberData(
				this, GUILD_MEMBER_DATA_ZONEID, newZone);
		...
		if (InstanceTemplate const* mInstance = sObjectMgr->GetInstanceTemplate(GetMapId())) allowMount =
				mInstance->allowMount;
		else if (MapEntry const* mEntry = sMapStore.LookupEntry(GetMapId())) allowMount =
				!mEntry->IsDungeon() || mEntry->IsBattlegroundOrArena();

		if (!allowMount)
		{
			RemoveAurasByType(SPELL_AURA_MOUNTED);

			if (IsInDisallowedMountForm()) RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT);
		}
	}
	...
	if (sWorld->getBoolConfig(CONFIG_WEATHER))
	{
		...
		if (wth) wth->SendWeatherUpdateToPlayer(this);
		else
		{
	...
	else RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_SANCTUARY);
	...
	if (isAlive()) DestroyZoneLimitedItem(true, newZone);
	...
	// group update
	if (GetGroup()) SetGroupUpdateFlag(GROUP_UPDATE_FLAG_ZONE);
...
void Player::CheckDuelDistance(time_t currTime)
{
	...
	else
	{
		...
		else if (currTime >= (duel->outOfBound + 10)) DuelComplete(DUEL_FLED);
	}
}
...
void Player::DuelComplete(DuelCompleteType type)
{
	...
	if (duel->opponent->GetSession()) duel->opponent->GetSession()->SendPacket(
			&data);
	...
	switch (type)
	{
		case DUEL_FLED:
			...
			{
				if (!duel->initiator->IsPvP()) duel->initiator->AttackStop();
				if (!duel->opponent->IsPvP()) duel->opponent->AttackStop();
			}
			break;
		case DUEL_WON:
			GetAchievementMgr().UpdateAchievementCriteria(
			...
			if (duel->opponent)
			{
				...
				if (getClass() == CLASS_DEATH_KNIGHT
						&& duel->opponent->GetQuestStatus(12733)
								== QUEST_STATUS_INCOMPLETE) duel->opponent->CastSpell(
						duel->opponent, 52994, true);
			}
			break;
		default:
			break;
	...
	if (obj) duel->initiator->RemoveGameObject(obj, true);
			...
			)
	{
		...
		if (!i->second->IsPositive() && aura->GetCasterGUID() == GetGUID()
				&& aura->GetApplyTime() >= duel->startTime) duel->opponent->RemoveAura(
				i);
		else ++i;
	}
	...
	for (AuraApplicationMap::iterator i = myAuras.begin(); i != myAuras.end();)
	{
		...
		if (!i->second->IsPositive()
				...
				&& aura->GetApplyTime() >= duel->startTime) RemoveAura(i);
		else ++i;
	}

	// cleanup combo points
	if (GetComboTarget() == duel->opponent->GetGUID()) ClearComboPoints();
	else if (GetComboTarget() == duel->opponent->GetPetGUID()) ClearComboPoints();

	if (duel->opponent->GetComboTarget() == GetGUID()) duel->opponent->ClearComboPoints();
	else if (duel->opponent->GetComboTarget() == GetPetGUID()) duel->opponent->ClearComboPoints();

	// Honor points after duel (the winner) - ImpConfig
	if (uint32 amount = sWorld->getIntConfig(CONFIG_HONOR_AFTER_DUEL)) duel->opponent->RewardHonor(
			NULL, 1, amount);
...
void Player::_ApplyItemMods(Item *item, uint8 slot, bool apply)
{
	...
	if (attacktype < MAX_ATTACK) _ApplyWeaponDependentAuraMods(item,
			WeaponAttackType(attacktype), apply);

	_ApplyItemBonuses(proto, slot, apply);

	if (slot == EQUIPMENT_SLOT_RANGED) _ApplyAmmoBonuses();
		...
		bool apply, bool only_level_scale /*= false*/)
{
	...
	for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
	{
		...
		{
			if (proto->ItemStat [i].ItemStatValue == 0) continue;
		...
		{
			case ITEM_MOD_MANA:
				HandleStatModifier(UNIT_MOD_MANA, BASE_VALUE, float(val),
						apply);
				break;
			case ITEM_MOD_HEALTH: // modify HP
				HandleStatModifier(UNIT_MOD_HEALTH, BASE_VALUE, float(val),
						apply);
				break;
			case ITEM_MOD_AGILITY: // modify agility
				HandleStatModifier(UNIT_MOD_STAT_AGILITY, BASE_VALUE,
						float(val), apply);
				ApplyStatBuffMod(STAT_AGILITY, float(val), apply);
				break;
			case ITEM_MOD_STRENGTH: //modify strength
				HandleStatModifier(UNIT_MOD_STAT_STRENGTH, BASE_VALUE,
						float(val), apply);
				ApplyStatBuffMod(STAT_STRENGTH, float(val), apply);
				break;
			case ITEM_MOD_INTELLECT: //modify intellect
				HandleStatModifier(UNIT_MOD_STAT_INTELLECT, BASE_VALUE,
						float(val), apply);
			...
			case ITEM_MOD_STAMINA: //modify stamina
				HandleStatModifier(UNIT_MOD_STAT_STAMINA, BASE_VALUE,
						float(val), apply);
			...
			case ITEM_MOD_RANGED_ATTACK_POWER:
				if (float(val) > 0.f) HandleStatModifier(
						UNIT_MOD_ATTACK_POWER_RANGED_POS, TOTAL_VALUE,
						float(val), apply);
				else HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED_NEG,
						TOTAL_VALUE, -float(val), apply);
				break;
...
//                break;
			...
			case ITEM_MOD_SPELL_PENETRATION:
				ApplyModInt32Value(PLAYER_FIELD_MOD_TARGET_RESISTANCE, -val,
						apply);
	...
	if (ssv)
	{
		if (int32 spellbonus = ssv->getSpellBonus(proto->ScalingStatValue)) ApplySpellPowerBonus(
				spellbonus, apply);
	}
	...
	if (ssv)
	{
		if (uint32 ssvarmor = ssv->getArmorMod(proto->ScalingStatValue)) armor =
				ssvarmor;
	}
	...
	if (proto->ArmorDamageModifier > 0) HandleStatModifier(UNIT_MOD_ARMOR,
			TOTAL_VALUE, float(proto->ArmorDamageModifier), apply);
	...
	if (CanUseAttackType(attType)) _ApplyWeaponDamage(slot, proto, ssv, apply);
	...
	if (ssv)
	{
		if (int32 feral_bonus = ssv->getFeralBonus(proto->ScalingStatValue)) ApplyFeralAPBonus(
				feral_bonus, apply);
	}
	...
	if (getClass() == CLASS_DRUID)
	{
		...
		if (feral_bonus > 0) ApplyFeralAPBonus(feral_bonus, apply);
	}
}
		...
		ScalingStatValuesEntry const *ssv, bool apply)
{
	...
	{
		if (slot == EQUIPMENT_SLOT_RANGED) SetAttackTime(RANGED_ATTACK,
				apply ? proto->Delay : BASE_ATTACK_TIME);
		else if (slot == EQUIPMENT_SLOT_MAINHAND) SetAttackTime(BASE_ATTACK,
				apply ? proto->Delay : BASE_ATTACK_TIME);
		else if (slot == EQUIPMENT_SLOT_OFFHAND) SetAttackTime(OFF_ATTACK,
				apply ? proto->Delay : BASE_ATTACK_TIME);
	}

	// No need to modify any physical damage for ferals as it is calculated from stats only
	if (IsInFeralForm()) return;

	if (CanModifyStats() && (damage || proto->Delay)) UpdateDamagePhysical(
			attType);
}
		...
		WeaponAttackType attackType, bool apply)
{
	...
	for (AuraEffectList::const_iterator itr = auraDamagePctList.begin();
		...
		if ((apply && item->IsFitToSpellRequirements((*itr)->GetSpellProto()))
				|| HasItemFitToSpellRequirements((*itr)->GetSpellProto(), item)) mod +=
				(*itr)->GetAmount();
		...
		WeaponAttackType attackType, AuraEffect const* aura, bool apply)
{
	...
	if (aura->GetSpellProto()->EquippedItemClass == -1) return;
	...
	if (!item->IsBroken()
			&& item->IsFitToSpellRequirements(aura->GetSpellProto())) HandleBaseModValue(
			mod, FLAT_MOD, float(aura->GetAmount()), apply);
}
		...
		WeaponAttackType attackType, AuraEffect const* aura, bool apply)
{
	...
	if (item->IsBroken() || !CanUseAttackType(attackType)) return;

	// ignore spell mods for not wands
	if ((aura->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) == 0
			&& (getClassMask() & CLASSMASK_WAND_USERS) == 0) return;
	...
	if (item->IsFitToSpellRequirements(aura->GetSpellProto()))
	{
		...
		if (unitModType == TOTAL_PCT) ApplyModSignedFloatValue(
				PLAYER_FIELD_MOD_DAMAGE_DONE_PCT, aura->GetAmount() / 100.0f,
				apply);
		else ApplyModUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS,
...
void Player::ApplyItemEquipSpell(Item *item, bool apply, bool form_change)
{
	...
	for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
	{
		...
		// no spell
		if (!spellData.SpellId) continue;

		// wrong triggering type
		if (apply && spellData.SpellTrigger != ITEM_SPELLTRIGGER_ON_EQUIP) continue;
		...
		bool apply, bool form_change)
{
	...
	else
	{
		if (form_change) // check aura compatibility
		{
			// Cannot be used in this stance/form
			if (GetErrorAtShapeshiftedCast(spellInfo, GetShapeshiftForm())
					== SPELL_CAST_OK) return; // and remove only not compatible at form change
		}

		if (item) RemoveAurasDueToItemSpell(item, spellInfo->Id); // un-apply all spells, not only at-equipped
		else RemoveAurasDueToSpell(spellInfo->Id); // un-apply spell (item set case)
	}
}
...
{
	if (!target || !target->isAlive() || target == this) return;

	for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
	{
		// If usable, try to cast item spell
		if (Item * item = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) if (!item->IsBroken()
				&& CanUseAttackType(attType)) if (ItemPrototype const *proto = item->GetProto())
		{
			...
			if (proto->Class == ITEM_CLASS_WEAPON)
			{
				...
				if (attType == BASE_ATTACK) if (!IsUseEquipedWeapon(true)
						&& !IsInFeralForm()) continue;
			}
			CastItemCombatSpell(target, attType, procVictim, procEx, item,
					proto);
		}
	}
}
		...
		ItemPrototype const * proto)
{
	...
	//if (damageInfo->procVictim & PROC_FLAG_TAKEN_ANY_DAMAGE)
	{
		for (uint8 i = 0; i < MAX_ITEM_SPELLS; ++i)
		{
			...
			// wrong triggering type
			if (spellData.SpellTrigger != ITEM_SPELLTRIGGER_CHANCE_ON_HIT) continue;
			...
			if (m_extraAttacks
					&& IsSpellHaveEffect(spellInfo,
							SPELL_EFFECT_ADD_EXTRA_ATTACKS)) return;
			...
			if (roll_chance_f(chance)) CastSpell(target, spellInfo->Id, true,
					item);
		}
	}
	...
	for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
	{
		...
		{
			if (pEnchant->type [s] != ITEM_ENCHANTMENT_TYPE_COMBAT_SPELL) continue;
			...
			if (entry && entry->procEx)
			{
				...
				if ((entry->procEx & procEx) == 0) continue;
			}
			else
			{
				// Can do effect if any damage done to target
			...
			if (entry)
			{
				if (entry->PPMChance) chance = GetPPMProcChance(proto->Delay,
						entry->PPMChance, spellInfo);
				else if (entry->customChance) chance =
						(float) entry->customChance;
			}
			...
			if (FindCurrentSpellBySpellId(5938)
					&& e_slot == TEMP_ENCHANTMENT_SLOT) chance = 100.0f;

			if (roll_chance_f(chance))
			{
				if (IsPositiveSpell(pEnchant->spellid [s])) CastSpell(this,
						pEnchant->spellid [s], true, item);
				else CastSpell(target, pEnchant->spellid [s], true, item);
			}
		}
	}
}
		...
		uint8 cast_count)
{
	...
	for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
	{
		...
		// wrong triggering type
		if (spellData.SpellTrigger != ITEM_SPELLTRIGGER_ON_USE) continue;
	...
	for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
	{
		...
		for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
		{
			if (pEnchant->type [s] != ITEM_ENCHANTMENT_TYPE_USE_SPELL) continue;
...
void Player::_RemoveAllItemMods()
{
	...
	for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
	{
		if (m_items [i])
		{
			...
			// item set bonuses not dependent from item broken state
			if (proto->ItemSet) RemoveItemsSetItem(this, proto);

			if (m_items [i]->IsBroken()
					|| !CanUseAttackType(GetAttackBySlot(i))) continue;
	...
	for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
	{
		if (m_items [i])
		{
			...
			if (attacktype < MAX_ATTACK) _ApplyWeaponDependentAuraMods(
					m_items [i], WeaponAttackType(attacktype), false);

			_ApplyItemBonuses(proto, i, false);

			if (i == EQUIPMENT_SLOT_RANGED) _ApplyAmmoBonuses();
		}
	}
...
void Player::_ApplyAllItemMods()
{
	...
	for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
	{
		if (m_items [i])
		{
			...
			// item set bonuses not dependent from item broken state
			if (proto->ItemSet) AddItemsSetItem(this, m_items [i]);
...
{
	if (uint64 lguid = GetLootGUID()) m_session->DoLootRelease(lguid);
	...
	if (IS_GAMEOBJECT_GUID(guid))
	{
		...
		if (go->getLootState() == GO_READY)
		{
			...
			if ((go->GetEntry() == BG_AV_OBJECTID_MINE_N
					|| go->GetEntry() == BG_AV_OBJECTID_MINE_S)) if (Battleground *bg = GetBattleground()) if (bg->GetTypeID(
					true) == BATTLEGROUND_AV) if (!(((BattlegroundAV*) bg)->PlayerCanDoMineQuest(
					go->GetEntry(), GetTeam())))
			{
				SendLootRelease(guid);
				return;
			...
			if (lootid)
			{
				...
				if (groupRules) group->UpdateLooterGuid(go, true);

				loot->FillLoot(lootid, LootTemplates_Gameobject, this,
						!groupRules, false, go->GetLootMode());

				// get next RR player (for next loot)
				if (groupRules) group->UpdateLooterGuid(go);
			}

			if (loot_type == LOOT_FISHING) go->getFishLoot(loot, this);
		...
		if (go->getLootState() == GO_ACTIVATED)
		{
			...
			else permission = ALL_PERMISSION;
		}
	}
		...
		if (!item->m_lootGenerated)
		{
			...
			switch (loot_type)
			{
				...
				case LOOT_PROSPECTING:
					...
					break;
				case LOOT_MILLING:
					loot->FillLoot(item->GetEntry(), LootTemplates_Milling,
							this, true);
					break;
				default:
	...
	else if (IS_CORPSE_GUID(guid)) // remove insignia
	{
		...
		if (!bones->lootForBody)
		{
			...
			bones->loot.clear();
			if (Battleground *bg = GetBattleground()) if (bg->GetTypeID(true)
					== BATTLEGROUND_AV) loot->FillLoot(1,
					LootTemplates_Creature, this, true);
		...
		if (bones->lootRecipient != this) permission = NONE_PERMISSION;
		else permission = OWNER_PERMISSION;
	}
	else
	{
		...
		if (loot_type == LOOT_PICKPOCKETING)
		{
			if (!creature->lootForPickPocketed)
			{
				...
				if (uint32 lootid = creature->GetCreatureInfo()->pickpocketLootId) loot->FillLoot(
		...
		else
		{
			...
			else
			{
				if (Group* group = GetGroup())
				{
					...
					else permission = NONE_PERMISSION;
				}
				else if (recipient == this) permission = OWNER_PERMISSION;
				else permission = NONE_PERMISSION;
			}
		}
	}
	...
	if (permission != NONE_PERMISSION) loot->AddLooter(GetGUID());

	if (loot_type == LOOT_CORPSE && !IS_ITEM_GUID(guid)) SetFlag(
			UNIT_FIELD_FLAGS, UNIT_FLAG_LOOTING);
}
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
	...
	if (sWorld->GetWintergrapsState() == 4354) data << uint32(0x1102) << sWorld->GetWintergrapsTimer();
	else data << uint32(0xEC5) << sWorld->GetWintergrapsTimer();
	// ---
	...
	switch (zoneid)
	{
		...
		case 139: // Eastern Plaguelands
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_EP) pvp->FillInitialWorldStates(
					data);
			else
			{
			...
			break;
		case 1377: // Silithus
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_SI) pvp->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3277: // Warsong Gulch
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_WS) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3358: // Arathi Basin
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_AB) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3820: // Eye of the Storm
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_EY) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3483: // Hellfire Peninsula
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_HP) pvp->FillInitialWorldStates(
					data);
			else
			{
			...
			break;
		case 3518: // Nagrand
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_NA) pvp->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3519: // Terokkar Forest
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_TF) pvp->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3521: // Zangarmarsh
			if (pvp && pvp->GetTypeId() == OUTDOOR_PVP_ZM) pvp->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3698: // Nagrand Arena
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_NA) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 3702: // Blade's Edge Arena
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_BE) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 4378: // Dalaran Sewers
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_DS) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 4384: // Strand of the Ancients
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_SA) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 4406: // Ring of Valor
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_RV) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 4710: // Isle of Conquest
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_IC) bg->FillInitialWorldStates(
					data);
			else
			{
		...
		case 5031: // Twin Peaks
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_TP) bg->FillInitialWorldStates(
					data);
			else
			{
				data << uint32(0x62d) << uint32(0x0); // 7 1581 alliance flag captures
			...
			break;
		case 5449:
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_BG) bg->FillInitialWorldStates(
					data);
			break;
...
void Player::SendBGWeekendWorldStates()
{
	for (uint32 i = 1; i < sBattlemasterListStore.GetNumRows(); ++i)
	{
		...
		{
			if (BattlegroundMgr::IsBGWeekend((BattlegroundTypeId) bl->id)) SendUpdateWorldState(
					bl->HolidayWorldStateId, 1);
			else SendUpdateWorldState(bl->HolidayWorldStateId, 0);
		}
	}
}
...
void Player::ResetPetTalents()
{
	...
	if (!pet || pet->getPetType() != HUNTER_PET || pet->m_usedTalentCount == 0) return;
...
void Player::SetVirtualItemSlot(uint8 i, Item* item)
{
	...
	if (i < 2 && item)
	{
		if (!item->GetEnchantmentId(TEMP_ENCHANTMENT_SLOT)) return;
		uint32 charges = item->GetEnchantmentCharges(TEMP_ENCHANTMENT_SLOT);
		if (charges == 0) return;
		if (charges > 1) item->SetEnchantmentCharges(TEMP_ENCHANTMENT_SLOT,
				charges - 1);
		else if (charges <= 1)
		{
		...
		bool swap) const
{
	...
	switch (proto->InventoryType)
	{
		...
		case INVTYPE_2HWEAPON:
			slots [0] = EQUIPMENT_SLOT_MAINHAND;
			if (Item *mhWeapon = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND)) if (ItemPrototype const *mhWeaponProto = mhWeapon->GetProto()) if (mhWeaponProto->SubClass
			...
			if (CanDualWield() && CanTitanGrip()
					...
					&& proto->SubClass != ITEM_SUBCLASS_WEAPON_STAFF) slots [1] =
	...
	if (slot != NULL_SLOT)
	{
		if (swap || !GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) for (uint8 i = 0;
				i < 4; ++i)
			if (slots [i] == slot) return slot;
	}
	else
	{
		...
		for (uint8 i = 0; i < 4; ++i)
			...
			if (slots [i] != EQUIPMENT_SLOT_OFFHAND || !IsTwoHandUsed()) return slots [i];

		// if not found free and can swap return first appropriate from used
		for (uint8 i = 0; i < 4; ++i)
			if (slots [i] != NULL_SLOT && swap) return slots [i];
	}
...
uint8 Player::CanUnequipItems(uint32 item, uint32 count) const
{
	...
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
		if (Bag *pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
				0; j < pBag->GetBagSize(); ++j)
			if (Item *pItem = GetItemByPos(i, j)) if (pItem->GetEntry() == item)
			{
				tempcount += pItem->GetCount();
				if (tempcount >= count) return EQUIP_ERR_OK;
			}
...
uint32 Player::GetItemCount(uint32 item, bool inBankAlso, Item* skipItem) const
{
	...
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
		if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) count +=
				pBag->GetItemCount(item, skipItem);

	if (skipItem && skipItem->GetProto()->GemProperties) for (uint8 i =
			EQUIPMENT_SLOT_START; i < INVENTORY_SLOT_ITEM_END; ++i)
		if (Item *pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) if (pItem
				!= skipItem && pItem->GetProto()->Socket [0].Color) count +=
				pItem->GetGemCountWithID(item);
	...
	if (inBankAlso)
	{
		...
		if (skipItem && skipItem->GetProto()->GemProperties) for (uint8 i =
				BANK_SLOT_ITEM_START; i < BANK_SLOT_ITEM_END; ++i)
			if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) if (pItem
					!= skipItem && pItem->GetProto()->Socket [0].Color) count +=
					pItem->GetGemCountWithID(item);
	}
...
Item* Player::GetItemByGuid(uint64 guid) const
{
	...
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
		if (Bag *pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
				0; j < pBag->GetBagSize(); ++j)
			if (Item* pItem = pBag->GetItemByPos(j)) if (pItem->GetGUID()
					== guid) return pItem;
...
Item* Player::GetItemByPos(uint8 bag, uint8 slot) const
{
	...
	{
		if (Bag *pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, bag)) return pBag->GetItemByPos(
				slot);
	}
	return NULL;
}
		...
		bool useable /*= false*/) const
{
	...
	Item* item = NULL;
	if (useable) item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, slot);
	else item = GetItemByPos(INVENTORY_SLOT_BAG_0, slot);
	if (!item || item->GetProto()->Class != ITEM_CLASS_WEAPON) return NULL;

	if (!useable) return item;

	if (item->IsBroken() || IsInFeralForm()) return NULL;
...
{
	Item* item = NULL;
	if (useable) item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0,
			EQUIPMENT_SLOT_OFFHAND);
	else item = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
	if (!item || item->GetProto()->Class != ITEM_CLASS_ARMOR) return NULL;
...
{
	if (bag == INVENTORY_SLOT_BAG_0 && slot == NULL_SLOT) return true;
	if (bag == INVENTORY_SLOT_BAG_0
...
{
	if (bag == INVENTORY_SLOT_BAG_0 && (slot < EQUIPMENT_SLOT_END)) return true;
	if (bag == INVENTORY_SLOT_BAG_0
			&& (slot >= INVENTORY_SLOT_BAG_START
					&& slot < INVENTORY_SLOT_BAG_END)) return true;
	return false;
}
...
{
	if (bag == INVENTORY_SLOT_BAG_0
			&& (slot >= BANK_SLOT_ITEM_START && slot < BANK_SLOT_ITEM_END)) return true;
	if (bag == INVENTORY_SLOT_BAG_0
			&& (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END)) return true;
	if (bag >= BANK_SLOT_BAG_START && bag < BANK_SLOT_BAG_END) return true;
	return false;
}
...
{
	// post selected
	if (bag == NULL_BAG && !explicit_pos) return true;

	if (bag == INVENTORY_SLOT_BAG_0)
	{
		// any post selected
		if (slot == NULL_SLOT && !explicit_pos) return true;

		// equipment
		if (slot < EQUIPMENT_SLOT_END) return true;

		// bag equip slots
		if (slot >= INVENTORY_SLOT_BAG_START && slot < INVENTORY_SLOT_BAG_END) return true;

		// backpack slots
		if (slot >= INVENTORY_SLOT_ITEM_START && slot < INVENTORY_SLOT_ITEM_END) return true;

		// keyring slots
		if (slot >= KEYRING_SLOT_START && slot < KEYRING_SLOT_END) return true;

		// bank main slots
		if (slot >= BANK_SLOT_ITEM_START && slot < BANK_SLOT_ITEM_END) return true;

		// bank bag slots
		if (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END) return true;
		...
		uint32* no_space_count) const
{
	...
	if (pItem && pItem->m_lootGenerated) return EQUIP_ERR_ALREADY_LOOTED;

	// no maximum
	if ((pProto->MaxCount <= 0 && pProto->ItemLimitCategory == 0)
			|| pProto->MaxCount == 2147483647) return EQUIP_ERR_OK;
	...
	if (pProto->MaxCount > 0)
	{
		...
		{
			if (no_space_count) *no_space_count = count + curcount
					- pProto->MaxCount;
			return EQUIP_ERR_CANT_CARRY_MORE_OF_THIS;
		}
	}
		...
		bool swap, Item* pSrcItem) const
{
	...
	if (pItem2 == pSrcItem) pItem2 = NULL;

	uint32 need_space;

	if (pSrcItem && pSrcItem->IsBag() && !((Bag*) pSrcItem)->IsEmpty()
			&& !IsBagPos(uint16(bag) << 8 | slot)) return EQUIP_ERR_CAN_ONLY_DO_WITH_EMPTY_BAGS;
	...
	if (!pItem2 || swap)
	{
		if (bag == INVENTORY_SLOT_BAG_0)
		{
			...
			if ((slot >= BUYBACK_SLOT_START && slot < BUYBACK_SLOT_END)
					|| slot >= PLAYER_SLOT_END) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;
		}
		else
		{
			Bag* pBag = (Bag*) GetItemByPos(INVENTORY_SLOT_BAG_0, bag);
			if (!pBag) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;

			ItemPrototype const* pBagProto = pBag->GetProto();
			if (!pBagProto) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;

			if (slot >= pBagProto->ContainerSlots) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;

			if (!ItemCanGoIntoBag(pProto, pBagProto)) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;
		}
	...
	if (need_space > count) need_space = count;
		...
		uint8 skip_slot) const
{
	...
	if (bag == skip_bag) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;

	// skip not existed bag or self targeted bag
	Bag* pBag = (Bag*) GetItemByPos(INVENTORY_SLOT_BAG_0, bag);
	if (!pBag || pBag == pSrcItem) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;

	if (pSrcItem && pSrcItem->IsBag() && !((Bag*) pSrcItem)->IsEmpty()) return EQUIP_ERR_CAN_ONLY_DO_WITH_EMPTY_BAGS;
	...
	if (non_specialized
			!= (pBagProto->Class == ITEM_CLASS_CONTAINER
					&& pBagProto->SubClass == ITEM_SUBCLASS_CONTAINER)) return EQUIP_ERR_ITEM_DOESNT_GO_INTO_BAG;
		...
		bool merge, Item* pSrcItem, uint8 skip_bag, uint8 skip_slot) const
{
	...
	{
		// skip specific slot already processed in first called _CanStoreItem_InSpecificSlot
		if (INVENTORY_SLOT_BAG_0 == skip_bag && j == skip_slot) continue;
		...
		uint32* no_space_count) const
{
	...
	if (res != EQUIP_ERR_OK)
	{
		...
		{
			if (no_space_count) *no_space_count = no_similar_count;
			return res;
		}
	...
	if (bag != NULL_BAG)
	{
		...
		if (pProto->Stackable != 1)
		{
			...
			else // equipped bag
			{
				...
				res = _CanStoreItem_InBag(bag, dest, pProto, count, true, false,
						pItem, NULL_BAG, slot);
				if (res != EQUIP_ERR_OK) res = _CanStoreItem_InBag(bag, dest,
						pProto, count, true, true, pItem, NULL_BAG, slot);
		...
		else // equipped bag
		{
			res = _CanStoreItem_InBag(bag, dest, pProto, count, false, false,
					pItem, NULL_BAG, slot);
			if (res != EQUIP_ERR_OK) res = _CanStoreItem_InBag(bag, dest,
					pProto, count, false, true, pItem, NULL_BAG, slot);
	...
	if (pItem && pItem->IsBag() && !((Bag*) pItem)->IsEmpty()) return EQUIP_ERR_NONEMPTY_BAG_OVER_OTHER_BAG;
...
void Player::ModifyCurrency(uint32 id, int32 count)
{
	...
	if (newWeekCount < 0) newWeekCount = 0;
	...
	{
		if (itr->second.state != PLAYERCURRENCY_NEW) itr->second.state =
...
uint32 Player::_GetCurrencyWeekCap(const CurrencyTypesEntry* currency) const
{
	...
	switch (currency->ID)
	{
		...
		case CURRENCY_TYPE_JUSTICE_POINTS:
		{
			...
			if (justicecap > 0) cap = justicecap;
			break;
		}
	}
...
uint32 Player::_GetCurrencyTotalCap(const CurrencyTypesEntry* currency) const
{
	...
	{
		case CURRENCY_TYPE_CONQUEST_POINTS:
			cap = sWorld->getIntConfig(CONFIG_MAX_CONQUEST_POINTS)
					* PLAYER_CURRENCY_PRECISION;
			break;
		case CURRENCY_TYPE_HONOR_POINTS:
			cap = sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS)
					* PLAYER_CURRENCY_PRECISION;
			break;
		case CURRENCY_TYPE_JUSTICE_POINTS:
			cap = sWorld->getIntConfig(CONFIG_MAX_JUSTICE_POINTS)
					* PLAYER_CURRENCY_PRECISION;
			break;
	}
...
uint8 Player::CanStoreItems(Item **pItems, int count) const
{
	...
	for (int k = 0; k < count; ++k)
	{
		...
		// strange item
		if (!pProto) return EQUIP_ERR_ITEM_NOT_FOUND;
				...
				!b_found && t < INVENTORY_SLOT_BAG_END; ++t)
		{
			...
			if (pBag)
			{
				...
				if (pBagProto
						&& (pBagProto->Class != ITEM_CLASS_CONTAINER
								|| pBagProto->SubClass
										!= ITEM_SUBCLASS_CONTAINER)) continue;
		...
		// no free slot found?
		if (!b_found) return EQUIP_ERR_INVENTORY_FULL;
	}
	...
	if (pItem)
	{
		...
		if (pProto)
		{
			...
			if (not_loading)
			{
				...
				if (HasUnitState(UNIT_STAT_STUNNED)) return EQUIP_ERR_YOU_ARE_STUNNED;
				...
				{
					if (isInCombat()) return EQUIP_ERR_NOT_IN_COMBAT;

					if (Battleground* bg = GetBattleground()) if (bg->isArena()
							&& bg->GetStatus() == STATUS_IN_PROGRESS) return EQUIP_ERR_NOT_DURING_ARENA_MATCH;
				}

				if (isInCombat()
						&& (pProto->Class == ITEM_CLASS_WEAPON
								|| pProto->InventoryType == INVTYPE_RELIC)
						&& m_weaponChangeTimer != 0) return EQUIP_ERR_CANT_DO_RIGHT_NOW; // maybe exist better err

				if (IsNonMeleeSpellCasted(false)) return EQUIP_ERR_CANT_DO_RIGHT_NOW;
			}
			...
			if (ssd && ssd->MaxLevel < DEFAULT_MAX_LEVEL
					&& ssd->MaxLevel < getLevel()) return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;

			uint8 eslot = FindEquipSlot(pProto, slot, swap);
			if (eslot == NULL_SLOT) return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
			...
			uint8 res2 = 0;
			if (swap) res2 = CanEquipUniqueItem(pItem, eslot);
			else res2 = CanEquipUniqueItem(pItem, NULL_SLOT);
			if (res2) return res2;
						...
						if (pBag != pItem)
						{
							if (ItemPrototype const* pBagProto = pBag->GetProto())
							{
								if (pBagProto->Class == pProto->Class
										&& (!swap || pBag->GetSlot() != eslot)) return
										(pBagProto->SubClass
			...
			if (eslot == EQUIPMENT_SLOT_OFFHAND)
			{
				...
				{
					if (!CanDualWield()) return EQUIP_ERR_CANT_DUAL_WIELD;
				}
				else if (type == INVTYPE_2HWEAPON)
				{
					if (!CanDualWield() || !CanTitanGrip()) return EQUIP_ERR_CANT_DUAL_WIELD;
				}

				if (IsTwoHandUsed()) return EQUIP_ERR_CANT_EQUIP_WITH_TWOHANDED;
			}
			...
			if (type == INVTYPE_2HWEAPON)
			{
				if (eslot == EQUIPMENT_SLOT_OFFHAND)
				{
					if (!CanTitanGrip()) return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
				}
				else if (eslot != EQUIPMENT_SLOT_MAINHAND) return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
					...
					if (offItem
							&& (!not_loading
									...
									|| CanStoreItem(NULL_BAG, NULL_SLOT,
											...
											!= EQUIP_ERR_OK)) return
							swap ? EQUIP_ERR_ITEMS_CANT_BE_SWAPPED : EQUIP_ERR_INVENTORY_FULL;
				}
			}
...
uint8 Player::CanUnequipItem(uint16 pos, bool swap) const
{
	...
	if (!IsEquipmentPos(pos) && !IsBagPos(pos)) return EQUIP_ERR_OK;

	Item* pItem = GetItemByPos(pos);

	// Applied only to existed equipped item
	if (!pItem) return EQUIP_ERR_OK;
	...
	if (!swap && pItem->IsBag() && !((Bag*) pItem)->IsEmpty()) return EQUIP_ERR_CAN_ONLY_DO_WITH_EMPTY_BAGS;
...
{
	if (!pItem) return
			swap ? EQUIP_ERR_ITEMS_CANT_BE_SWAPPED : EQUIP_ERR_ITEM_NOT_FOUND;
	...
	if (bag != NULL_BAG && slot != NULL_SLOT)
	{
		...
		{
			if (!pItem->IsBag()) return EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT;

			if (slot - BANK_SLOT_BAG_START >= GetBankBagSlotCount()) return EQUIP_ERR_MUST_PURCHASE_THAT_BAG_SLOT;
	...
	if (bag != NULL_BAG)
	{
		...
		{
			Bag *pBag = (Bag*) pItem;
			if (pBag && !pBag->IsEmpty()) return EQUIP_ERR_NONEMPTY_BAG_OVER_OTHER_BAG;
		}
...
uint8 Player::CanUseItem(Item *pItem, bool not_loading) const
{
	if (pItem)
	{
		...
		if (!isAlive() && not_loading) return EQUIP_ERR_YOU_ARE_DEAD;
		...
		if (pProto)
		{
			...
			if (pItem->GetSkill() != 0)
			{
				...
				if (!allowEquip && GetSkillValue(itemSkill) == 0) return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;
			}
			...
			if (pProto->RequiredReputationFaction
					&& uint32(
							...
							< pProto->RequiredReputationRank) return EQUIP_ERR_CANT_EQUIP_REPUTATION;
...
uint8 Player::CanUseItem(ItemPrototype const *proto) const
{
	...
	if (proto)
	{
		if ((proto->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY) && GetTeam() != HORDE) return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if ((proto->Flags2 & ITEM_FLAGS_EXTRA_ALLIANCE_ONLY)
				&& GetTeam() != ALLIANCE) return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if ((proto->AllowableClass & getClassMask()) == 0
				|| (proto->AllowableRace & getRaceMask()) == 0) return EQUIP_ERR_YOU_CAN_NEVER_USE_THAT_ITEM;

		if (proto->RequiredSkill != 0)
		{
			if (GetSkillValue(proto->RequiredSkill) == 0) return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;
			else if (GetSkillValue(proto->RequiredSkill)
					< proto->RequiredSkillRank) return EQUIP_ERR_CANT_EQUIP_SKILL;
		}

		if (proto->RequiredSpell != 0 && !HasSpell(proto->RequiredSpell)) return EQUIP_ERR_NO_REQUIRED_PROFICIENCY;

		if (getLevel() < proto->RequiredLevel) return EQUIP_ERR_CANT_EQUIP_LEVEL_I;

		// If World Event is not active, prevent using event dependent items
		if (proto->HolidayId && !IsHolidayActive((HolidayIds) proto->HolidayId)) return EQUIP_ERR_CANT_DO_RIGHT_NOW;
...
uint8 Player::CanUseAmmo(uint32 item) const
{
	sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "STORAGE: CanUseAmmo item = %u",
			item);
	if (!isAlive()) return EQUIP_ERR_YOU_ARE_DEAD;
	//if (isStunned())
	//    return EQUIP_ERR_YOU_ARE_STUNNED;
	ItemPrototype const *pProto = ObjectMgr::GetItemPrototype(item);
	if (pProto)
	{
		if (pProto->InventoryType != INVTYPE_AMMO) return EQUIP_ERR_ONLY_AMMO_CAN_GO_HERE;
		...
		// Requires No Ammo
		if (HasAura(46699)) return EQUIP_ERR_BAG_FULL6;
		...
		bool update)
{
	...
	if (!pItem2)
	{
		if (clone) pItem = pItem->CloneItem(count, this);
		else pItem->SetCount(count);
	...
	else
	{
		...
		if (IsInWorld() && update) pItem2->SendUpdateToPlayer(this);
...
Item* Player::EquipItem(uint16 pos, Item *pItem, bool update)
{
	...
	if (!pItem2)
	{
		...
		if (isAlive())
		{
			...
			if (pProto && pProto->ItemSet) AddItemsSetItem(this, pItem);
					...
					&& m_weaponChangeTimer == 0)
			{
				...
				if (getClass() == CLASS_ROGUE) cooldownSpell = 6123;
		...
		if (slot == EQUIPMENT_SLOT_MAINHAND) UpdateExpertise(BASE_ATTACK);

		else if (slot == EQUIPMENT_SLOT_OFFHAND) UpdateExpertise(OFF_ATTACK);
...
void Player::VisualizeItem(uint8 slot, Item *pItem)
{
	...
	if (pItem->GetProto()->Bonding == BIND_WHEN_EQUIPED
			...
			|| pItem->GetProto()->Bonding == BIND_QUEST_ITEM) pItem->SetBinding(
			true);
	...
	if (slot < EQUIPMENT_SLOT_END) SetVisibleItemSlot(slot, pItem);
	...
	if (pItem)
	{
		...
		if (bag == INVENTORY_SLOT_BAG_0)
		{
			if (slot < INVENTORY_SLOT_BAG_END)
			{
				...
				if (pProto && pProto->ItemSet) RemoveItemsSetItem(this, pProto);
			...
			if (slot < EQUIPMENT_SLOT_END) SetVisibleItemSlot(slot, NULL);
		}
		else
		{
			if (Bag *pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, bag)) pBag->RemoveItem(
					slot, update);
		}
		...
		bool update, bool in_characterInventoryDB)
{
	...
	if (pLastItem == pItem)
	{
		...
		if (pLastItem->GetOwnerGUID() != GetGUID()) pLastItem->SetOwnerGUID(
				GetGUID());
	...
	if (pLastItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_BOP_TRADEABLE)) m_itemSoulboundTradeable.push_back(
			pLastItem);
}
...
void Player::DestroyItem(uint8 bag, uint8 slot, bool update)
{
	...
	if (pItem)
	{
		...
		if (pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED)) CharacterDatabase.PExecute(
		...
		else if (Bag *pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, bag)) pBag->RemoveItem(
				slot, update);
				...
				if (Item* pItem = pBag->GetItemByPos(j))
				{
					if (pItem->GetEntry() == item && !pItem->IsInTrade())
					{
						...
						else
						{
							...
							if (IsInWorld() && update) pItem->SendUpdateToPlayer(
									this);
							pItem->SetState(ITEM_CHANGED, this);
							return;
						}
...
void Player::DestroyZoneLimitedItem(bool update, uint32 new_zone)
{
	...
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
		if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
				0; j < pBag->GetBagSize(); j++)
			if (Item* pItem = pBag->GetItemByPos(j)) if (pItem->IsLimitedToAnotherMapOrZone(
					GetMapId(), new_zone)) DestroyItem(i, j, update);
...
void Player::DestroyConjuredItems(bool update)
{
	...
	// in inventory
	for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; i++)
		if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) if (pItem->IsConjuredConsumable()) DestroyItem(
				INVENTORY_SLOT_BAG_0, i, update);

	// in inventory bags
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; i++)
		if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
				0; j < pBag->GetBagSize(); j++)
			if (Item* pItem = pBag->GetItemByPos(j)) if (pItem->IsConjuredConsumable()) DestroyItem(
					i, j, update);
...
Item* Player::GetItemByEntry(uint32 entry) const
{
	...
	for (int i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
	{
		if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i))
		{
			for (uint32 j = 0; j < pBag->GetBagSize(); ++j)
			{
				if (Item* pItem = pBag->GetItemByPos(j)) if (pItem->GetEntry()
						== entry) return pItem;
			}
		}
	}
...
void Player::SplitItem(uint16 src, uint16 dst, uint32 count)
{
	...
	if (IsInventoryPos(dst))
	{
		...
		if (IsInWorld()) pSrcItem->SendUpdateToPlayer(this);
...
void Player::SwapItem(uint16 src, uint16 dst)
{
	...
	if (!pSrcItem->IsBag() && !pDstItem->IsBag())
	{
		...
		ItemPosCountVec sDest;
		uint16 eDest = 0;
		if (IsInventoryPos(dst)) msg = CanStoreItem(dstbag, dstslot, sDest,
				pSrcItem, false);
		else if (IsBankPos(dst)) msg = CanBankItem(dstbag, dstslot, sDest,
				pSrcItem, false);
		else if (IsEquipmentPos(dst)) msg = CanEquipItem(dstslot, eDest,
				pSrcItem, false);
		else return;
		...
		if (msg == EQUIP_ERR_OK)
		{
					...
					<= pSrcItem->GetProto()->GetMaxStackSize())
			{
				...
				if (IsInventoryPos(dst)) StoreItem(sDest, pSrcItem, true);
				else if (IsBankPos(dst)) BankItem(sDest, pSrcItem, true);
				else if (IsEquipmentPos(dst))
				{
	...
	if (IsInventoryPos(dst)) msg = CanStoreItem(dstbag, dstslot, sDest,
			pSrcItem, true);
	else if (IsBankPos(dst)) msg = CanBankItem(dstbag, dstslot, sDest, pSrcItem,
			true);
	else if (IsEquipmentPos(dst))
	{
		msg = CanEquipItem(dstslot, eDest, pSrcItem, true);
		if (msg == EQUIP_ERR_OK) msg = CanUnequipItem(eDest, true);
	}
	...
	uint16 eDest2 = 0;
	if (IsInventoryPos(src)) msg = CanStoreItem(srcbag, srcslot, sDest2,
			pDstItem, true);
	else if (IsBankPos(src)) msg = CanBankItem(srcbag, srcslot, sDest2,
			pDstItem, true);
	else if (IsEquipmentPos(src))
	{
		msg = CanEquipItem(srcslot, eDest2, pDstItem, true);
		if (msg == EQUIP_ERR_OK) msg = CanUnequipItem(eDest2, true);
	}
	...
	else if (IsEquipmentPos(dst)) EquipItem(eDest, pSrcItem, true);

	// add to src
	if (IsInventoryPos(src)) StoreItem(sDest2, pDstItem, true);
	else if (IsBankPos(src)) BankItem(sDest2, pDstItem, true);
	else if (IsEquipmentPos(src)) EquipItem(eDest2, pDstItem, true);
...
void Player::AddItemToBuyBackSlot(Item *pItem)
{
	if (pItem)
	{
		...
		SetUInt64Value(PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + (eslot * 2),
				pItem->GetGUID());
		if (ItemPrototype const *pProto = pItem->GetProto()) SetUInt32Value(
				PLAYER_FIELD_BUYBACK_PRICE_1 + eslot,
				pProto->SellPrice * pItem->GetCount());
		else SetUInt32Value(PLAYER_FIELD_BUYBACK_PRICE_1 + eslot, 0);
		SetUInt32Value(PLAYER_FIELD_BUYBACK_TIMESTAMP_1 + eslot,
				(uint32) etime);

		// move to next (for non filled list is move most optimized choice)
		if (m_currentBuybackSlot < BUYBACK_SLOT_END - 1) ++m_currentBuybackSlot;
	}
}
...
Item* Player::GetItemFromBuyBackSlot(uint32 slot)
{
	...
	if (slot >= BUYBACK_SLOT_START && slot < BUYBACK_SLOT_END) return m_items [slot];
	return NULL;
}
...
void Player::RemoveItemFromBuyBackSlot(uint32 slot, bool del)
{
	...
	if (slot >= BUYBACK_SLOT_START && slot < BUYBACK_SLOT_END)
	{
		...
		if (m_items [m_currentBuybackSlot]) m_currentBuybackSlot = slot;
	}
}
...
void Player::TradeCancel(bool sendback)
{
	if (m_trade)
	{
		...
		if (sendback) GetSession()->SendCancelTrade();
...
{
	if (m_itemSoulboundTradeable.empty()) return;
...
{
	if (m_itemDuration.empty()) return;
			...
			itr != m_itemDuration.end();)
	{
		...
		if ((realtimeonly && item->GetProto()->Duration < 0) || !realtimeonly) item->UpdateDuration(
				this, time);
	}
}
...
void Player::AddEnchantmentDurations(Item *item)
{
	...
	{
		if (!item->GetEnchantmentId(EnchantmentSlot(x))) continue;

		uint32 duration = item->GetEnchantmentDuration(EnchantmentSlot(x));
		if (duration > 0) AddEnchantmentDuration(item, EnchantmentSlot(x),
				duration);
	}
}
...
void Player::RemoveArenaEnchantments(EnchantmentSlot slot)
{
	...
	// in inventory
	for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
		if (Item* pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, i)) if (pItem->GetEnchantmentId(
				slot)) pItem->ClearEnchantment(slot);

	// in inventory bags
	for (uint8 i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
		if (Bag* pBag = (Bag*)GetItemByPos(INVENTORY_SLOT_BAG_0, i)) for (uint32 j =
				0; j < pBag->GetBagSize(); j++)
			if (Item* pItem = pBag->GetItemByPos(j)) if (pItem->GetEnchantmentId(
					slot)) pItem->ClearEnchantment(slot);
}
		...
		uint32 duration)
{
	...
	if (slot >= MAX_ENCHANTMENT_SLOT) return;
		...
		bool apply_dur, bool ignore_condition)
{
	...
	if (!ignore_condition && pEnchant->EnchantmentCondition
			&& !EnchantmentFitsRequirements(pEnchant->EnchantmentCondition, -1)) return;

	if (pEnchant->requiredLevel > getLevel()) return;

	if (pEnchant->requiredSkill > 0
			&& pEnchant->requiredSkillValue
					> GetSkillValue(pEnchant->requiredSkill)) return;
		...
		if (!pPrismaticEnchant
				|| (pPrismaticEnchant->requiredSkill > 0
						&& pPrismaticEnchant->requiredSkillValue
								> GetSkillValue(
										pPrismaticEnchant->requiredSkill))) return;
	}
		...
		for (int s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
		{
			...
			switch (enchant_display_type)
			{
				...
				case ITEM_ENCHANTMENT_TYPE_DAMAGE:
					if (item->GetSlot() == EQUIPMENT_SLOT_MAINHAND) HandleStatModifier(
							UNIT_MOD_DAMAGE_MAINHAND, TOTAL_VALUE,
							float(enchant_amount), apply);
					else if (item->GetSlot() == EQUIPMENT_SLOT_OFFHAND) HandleStatModifier(
							UNIT_MOD_DAMAGE_OFFHAND, TOTAL_VALUE,
							float(enchant_amount), apply);
					else if (item->GetSlot() == EQUIPMENT_SLOT_RANGED) HandleStatModifier(
							UNIT_MOD_DAMAGE_RANGED, TOTAL_VALUE,
							float(enchant_amount), apply);
					break;
				case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
					if (enchant_spell_id)
					{
						if (apply)
						{
							...
							if (basepoints) CastCustomSpell(this,
									enchant_spell_id, &basepoints, &basepoints,
									&basepoints, true, item);
							else CastSpell(this, enchant_spell_id, true, item);
						}
				...
				case ITEM_ENCHANTMENT_TYPE_STAT:
				{
					if (!enchant_amount)
					{
						...
						if (item_rand_suffix)
						{
							for (int k = 0; k < MAX_ITEM_ENCHANTMENT_EFFECTS;
									++k)
							{
								if (item_rand_suffix->enchant_id [k]
										== enchant_id)
								{
									enchant_amount =
											uint32(
					...
					switch (enchant_spell_id)
					{
						...
						case ITEM_MOD_HEALTH:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u HEALTH", enchant_amount);
							HandleStatModifier(UNIT_MOD_HEALTH, BASE_VALUE,
									float(enchant_amount), apply);
							break;
						case ITEM_MOD_AGILITY:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u AGILITY", enchant_amount);
							HandleStatModifier(UNIT_MOD_STAT_AGILITY,
									TOTAL_VALUE, float(enchant_amount), apply);
							ApplyStatBuffMod(STAT_AGILITY,
									(float) enchant_amount, apply);
							break;
						case ITEM_MOD_STRENGTH:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u STRENGTH", enchant_amount);
							HandleStatModifier(UNIT_MOD_STAT_STRENGTH,
									TOTAL_VALUE, float(enchant_amount), apply);
							ApplyStatBuffMod(STAT_STRENGTH,
									(float) enchant_amount, apply);
							break;
						case ITEM_MOD_INTELLECT:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u INTELLECT", enchant_amount);
							HandleStatModifier(UNIT_MOD_STAT_INTELLECT,
									TOTAL_VALUE, float(enchant_amount), apply);
							ApplyStatBuffMod(STAT_INTELLECT,
									(float) enchant_amount, apply);
							break;
						case ITEM_MOD_SPIRIT:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u SPIRIT", enchant_amount);
							HandleStatModifier(UNIT_MOD_STAT_SPIRIT,
									TOTAL_VALUE, float(enchant_amount), apply);
							ApplyStatBuffMod(STAT_SPIRIT,
									(float) enchant_amount, apply);
							break;
						case ITEM_MOD_STAMINA:
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u STAMINA", enchant_amount);
							HandleStatModifier(UNIT_MOD_STAT_STAMINA,
									TOTAL_VALUE, float(enchant_amount), apply);
							ApplyStatBuffMod(STAT_STAMINA,
									(float) enchant_amount, apply);
							break;
						case ITEM_MOD_DEFENSE_SKILL_RATING:
							ApplyRatingMod(CR_DEFENSE_SKILL, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u DEFENCE", enchant_amount);
							break;
						case ITEM_MOD_DODGE_RATING:
							ApplyRatingMod(CR_DODGE, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u DODGE", enchant_amount);
							break;
						case ITEM_MOD_PARRY_RATING:
							ApplyRatingMod(CR_PARRY, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u PARRY", enchant_amount);
							break;
						case ITEM_MOD_BLOCK_RATING:
							ApplyRatingMod(CR_BLOCK, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u SHIELD_BLOCK", enchant_amount);
							break;
						case ITEM_MOD_HIT_MELEE_RATING:
							ApplyRatingMod(CR_HIT_MELEE, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u MELEE_HIT", enchant_amount);
							break;
						case ITEM_MOD_HIT_RANGED_RATING:
							ApplyRatingMod(CR_HIT_RANGED, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u RANGED_HIT", enchant_amount);
							break;
						case ITEM_MOD_HIT_SPELL_RATING:
							ApplyRatingMod(CR_HIT_SPELL, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u SPELL_HIT", enchant_amount);
							break;
						case ITEM_MOD_CRIT_MELEE_RATING:
							ApplyRatingMod(CR_CRIT_MELEE, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u MELEE_CRIT", enchant_amount);
							break;
						case ITEM_MOD_CRIT_RANGED_RATING:
							ApplyRatingMod(CR_CRIT_RANGED, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u RANGED_CRIT", enchant_amount);
							break;
						case ITEM_MOD_CRIT_SPELL_RATING:
							ApplyRatingMod(CR_CRIT_SPELL, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u SPELL_CRIT", enchant_amount);
							break;
...
//                            break;
						case ITEM_MOD_HASTE_SPELL_RATING:
							ApplyRatingMod(CR_HASTE_SPELL, enchant_amount,
									apply);
							break;
						case ITEM_MOD_HIT_RATING:
							ApplyRatingMod(CR_HIT_MELEE, enchant_amount, apply);
...
//                            break;
						case ITEM_MOD_RESILIENCE_RATING:
							...
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u RESILIENCE", enchant_amount);
							break;
						case ITEM_MOD_HASTE_RATING:
							ApplyRatingMod(CR_HASTE_MELEE, enchant_amount,
									apply);
							ApplyRatingMod(CR_HASTE_RANGED, enchant_amount,
									apply);
							ApplyRatingMod(CR_HASTE_SPELL, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u HASTE", enchant_amount);
							break;
						case ITEM_MOD_EXPERTISE_RATING:
							ApplyRatingMod(CR_EXPERTISE, enchant_amount, apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u EXPERTISE", enchant_amount);
							break;
						case ITEM_MOD_ATTACK_POWER:
							if (float(enchant_amount) > 0.f)
							{
								HandleStatModifier(UNIT_MOD_ATTACK_POWER_POS,
										TOTAL_VALUE, float(enchant_amount),
										apply);
								HandleStatModifier(
							...
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u ATTACK_POWER", enchant_amount);
							break;
						case ITEM_MOD_RANGED_ATTACK_POWER:
							if (float(enchant_amount) > 0.f) HandleStatModifier(
									UNIT_MOD_ATTACK_POWER_RANGED_POS,
									TOTAL_VALUE, float(enchant_amount), apply);
							else HandleStatModifier(
									UNIT_MOD_ATTACK_POWER_RANGED_NEG,
									TOTAL_VALUE, -float(enchant_amount), apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
...
//                            break;
						case ITEM_MOD_MANA_REGENERATION:
							...
							break;
						case ITEM_MOD_ARMOR_PENETRATION_RATING:
							ApplyRatingMod(CR_ARMOR_PENETRATION, enchant_amount,
									apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
						...
						case ITEM_MOD_BLOCK_VALUE:
							HandleBaseModValue(SHIELD_BLOCK_VALUE, FLAT_MOD,
									float(enchant_amount), apply);
							sLog->outDebug(LOG_FILTER_PLAYER_ITEMS,
									"+ %u BLOCK_VALUE", enchant_amount);
							break;
						case ITEM_MOD_MASTERY_RATING:
							ApplyRatingMod(CR_MASTERY, int32(enchant_amount),
									apply);
							break;
	...
	if (slot == PERM_ENCHANTMENT_SLOT) SetUInt16Value(
			PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (item->GetSlot() * 2), 0,
			apply ? item->GetEnchantmentId(slot) : 0);

	if (slot == TEMP_ENCHANTMENT_SLOT) SetUInt16Value(
	...
	if (apply_dur)
	{
		if (apply)
		{
			...
			if (duration > 0) AddEnchantmentDuration(item, slot, duration);
		}
		else
		{
...
{
	if (!item || !item->IsEquipped() || item->IsBroken()) return;

	uint32 reforge_id = item->GetEnchantmentId(REFORGE_ENCHANTMENT_SLOT);
	if (!reforge_id) return;
	...
	for (int32 i = 0; i < MAX_ITEM_PROTO_STATS; i++)
	{
		...
		if (item->GetProto()->ItemStat [i].ItemStatType == reforge->oldstat) statBaseValue =
				item->GetProto()->ItemStat [i].ItemStatValue;
	}
		...
		switch (statType [i])
		{
			case ITEM_MOD_SPIRIT:
				...
				ApplyStatBuffMod(STAT_SPIRIT, (float) statValue [i],
						(i == 0) ? (!apply) : apply);
			case ITEM_MOD_DODGE_RATING:
				ApplyRatingMod(CR_DODGE, statValue [i],
						(i == 0) ? (!apply) : apply);
				sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "+ %u DODGE",
						statValue [i]);
				break;
			case ITEM_MOD_PARRY_RATING:
				ApplyRatingMod(CR_PARRY, statValue [i],
						(i == 0) ? (!apply) : apply);
			...
			for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
			{
				...
				if (Enchant->requiredSkill == skill_id)
				{
					...
					if (curr_value < Enchant->requiredSkillValue
							&& new_value >= Enchant->requiredSkillValue) ApplyEnchantment(
							m_items [i], EnchantmentSlot(slot), true);
					else if (new_value < Enchant->requiredSkillValue
							&& curr_value >= Enchant->requiredSkillValue) ApplyEnchantment(
								...
								- SOCK_ENCHANTMENT_SLOT].Color)
				{
							...
							&& pPrismaticEnchant->requiredSkill == skill_id)
					{
						if (curr_value < pPrismaticEnchant->requiredSkillValue
								&& new_value
										>= pPrismaticEnchant->requiredSkillValue) ApplyEnchantment(
								m_items [i], EnchantmentSlot(slot), true);
						else if (new_value
								< pPrismaticEnchant->requiredSkillValue
								&& curr_value
		...
		bool broadcast)
{
	...
	if (broadcast && GetGroup()) GetGroup()->BroadcastPacket(&data, true);
	else GetSession()->SendPacket(&data);
}
		...
		bool showQuests)
{
	...
	if (pMenuItemBounds.first == pMenuItemBounds.second
			&& menuId == GetDefaultGossipMenuForSource(pSource)) pMenuItemBounds =
			sObjectMgr->GetGossipMenuItemsMapBounds(0);
	...
	if (pSource->GetTypeId() == TYPEID_UNIT)
	{
		...
		if (npcflags & UNIT_NPC_FLAG_QUESTGIVER && showQuests) PrepareQuestMenu(
				pSource->GetGUID());
	}
			...
			itr != pMenuItemBounds.second; ++itr)
	{
		...
		if (!sConditionMgr->IsPlayerMeetToConditions(this,
				itr->second.conditions)) continue;
		...
		if (pSource->GetTypeId() == TYPEID_UNIT)
		{
			...
			switch (itr->second.option_id)
			{
				...
				case GOSSIP_OPTION_TRAINER:
					if (!pCreature->isCanTrainingOf(this, false)) bCanTalk =
							false;
					break;
				case GOSSIP_OPTION_LEARNDUALSPEC:
					if (!(GetSpecsCount() == 1
							&& pCreature->isCanTrainingAndResetTalentsOf(this)
							&& !(getLevel() < sWorld->getIntConfig(
									CONFIG_MIN_DUALSPEC_LEVEL)))) bCanTalk =
							false;
					break;
				case GOSSIP_OPTION_UNLEARNTALENTS:
					if (!pCreature->isCanTrainingAndResetTalentsOf(this)) bCanTalk =
							false;
					break;
				case GOSSIP_OPTION_UNLEARNPETTALENTS:
					if (!GetPet() || GetPet()->getPetType() != HUNTER_PET
							...
							|| pCreature->GetCreatureInfo()->trainer_class
									!= CLASS_HUNTER) bCanTalk = false;
					break;
				case GOSSIP_OPTION_TAXIVENDOR:
					if (GetSession()->SendLearnNewTaxiNode(pCreature)) return;
					break;
				case GOSSIP_OPTION_BATTLEFIELD:
					if (!pCreature->isCanInteractWithBattleMaster(this, false)) bCanTalk =
							false;
					break;
				case GOSSIP_OPTION_STABLEPET:
					if (getClass() != CLASS_HUNTER) bCanTalk = false;
					break;
				...
				case GOSSIP_OPTION_OUTDOORPVP:
					if (!sOutdoorPvPMgr->CanTalkTo(this, pCreature,
							itr->second)) bCanTalk = false;
					break;
				default:
					sLog->outErrorDb(
		...
		else if (pSource->GetTypeId() == TYPEID_GAMEOBJECT)
		{
			...
			{
				case GOSSIP_OPTION_QUESTGIVER:
					if (pGo->GetGoType() == GAMEOBJECT_TYPE_QUESTGIVER) PrepareQuestMenu(
							pSource->GetGUID());
					bCanTalk = false;
					break;
				case GOSSIP_OPTION_GOSSIP:
					if (pGo->GetGoType() != GAMEOBJECT_TYPE_QUESTGIVER
							&& pGo->GetGoType() != GAMEOBJECT_TYPE_GOOBER) bCanTalk =
							false;
					break;
				default:
					bCanTalk = false;
...
void Player::SendPreparedGossip(WorldObject *pSource)
{
	...
	if (uint32 menuId = PlayerTalkClass->GetGossipMenu().GetMenuId()) textId =
		...
		uint32 menuId)
{
	...
	if (gossipListId >= gossipmenu.MenuItemCount()) return;

	// if not same, then something funky is going on
	if (menuId != gossipmenu.GetMenuId()) return;
	...
	switch (gossipOptionId)
	{
		case GOSSIP_OPTION_GOSSIP:
		{
			...
			if (pMenuData.m_gAction_poi) PlayerTalkClass->SendPointOfInterest(
					pMenuData.m_gAction_poi);

			if (pMenuData.m_gAction_script)
			{
				if (pSource->GetTypeId() == TYPEID_UNIT) GetMap()->ScriptsStart(
						sGossipScripts, pMenuData.m_gAction_script, this,
						pSource);
				else if (pSource->GetTypeId() == TYPEID_GAMEOBJECT) GetMap()->ScriptsStart(
						sGossipScripts, pMenuData.m_gAction_script, pSource,
						this);
			}
			break;
		}
		case GOSSIP_OPTION_OUTDOORPVP:
			...
			break;
		case GOSSIP_OPTION_SPIRITHEALER:
			if (isDead()) pSource->ToCreature()->CastSpell(
					(pSource->ToCreature()), 17251, true, NULL, NULL,
					GetGUID());
			break;
		case GOSSIP_OPTION_QUESTGIVER:
		...
		case GOSSIP_OPTION_TRAINER:
			...
			break;
		case GOSSIP_OPTION_LEARNDUALSPEC:
			if (GetSpecsCount() == 1 && getLevel() >= sWorld->getIntConfig(
...
uint32 Player::GetGossipTextId(WorldObject *pSource)
{
	if (!pSource || pSource->GetTypeId() != TYPEID_UNIT
			|| !pSource->ToCreature()->GetDBTableGUIDLow()) return DEFAULT_GOSSIP_MESSAGE;

	if (uint32 pos = sObjectMgr->GetNpcGossip(pSource->ToCreature()->GetDBTableGUIDLow())) return pos;
...
uint32 Player::GetGossipTextId(uint32 menuId)
{
	...
	{
		if (sConditionMgr->IsPlayerMeetToConditions(this,
				itr->second.conditions)) textId = itr->second.text_id;
	}
...
{
	if (pSource->GetTypeId() == TYPEID_UNIT) return pSource->ToCreature()->GetCreatureInfo()->GossipMenuId;
	else if (pSource->GetTypeId() == TYPEID_GAMEOBJECT) return ((GameObject*) pSource)->GetGOInfo()->GetGossipMenuId();
...
void Player::PrepareQuestMenu(uint64 guid)
{
			...
			i != pObjectQIR.second; ++i)
	{
		...
		if (status == QUEST_STATUS_COMPLETE) qm.AddMenuItem(quest_id, 4);
		else if (status == QUEST_STATUS_INCOMPLETE) qm.AddMenuItem(quest_id, 4);
			...
			i != pObjectQR.second; ++i)
	{
		...
		if (!CanTakeQuest(pQuest, false)) continue;

		if (pQuest->IsAutoComplete()) qm.AddMenuItem(quest_id, 4);
		else if (GetQuestStatus(quest_id) == QUEST_STATUS_NONE) qm.AddMenuItem(
				quest_id, 2);
	}
}
...
void Player::SendPreparedQuest(uint64 guid)
{
	...
	if (questMenu.MenuItemCount() == 1)
	{
		...
		if (pQuest)
		{
			if (icon == 4 && !GetQuestRewardStatus(quest_id)) PlayerTalkClass->SendQuestGiverRequestItems(
					pQuest, guid, CanRewardQuest(pQuest, false), true);
			else if (icon == 4) PlayerTalkClass->SendQuestGiverRequestItems(
					pQuest, guid, CanRewardQuest(pQuest, false), true);
			...
			else
			{
				...
				if (pQuest->IsAutoAccept() && CanAddQuest(pQuest, true))
				{
					...
					if (CanCompleteQuest(quest_id)) CompleteQuest(quest_id);
				}

				if ((pQuest->IsAutoComplete() && pQuest->IsRepeatable()
						&& !pQuest->IsDailyOrWeekly())
						|| pQuest->HasFlag(QUEST_FLAGS_AUTOCOMPLETE)) PlayerTalkClass->SendQuestGiverRequestItems(
						pQuest, guid, CanCompleteRepeatableQuest(pQuest), true);
				else PlayerTalkClass->SendQuestGiverQuestDetails(pQuest, guid,
						true);
			}
		}
	}
		...
		if (pCreature)
		{
			...
			else
			{
				...
				if (!gossiptext->Options [0].Text_0.empty())
				{
					...
					int loc_idx = GetSession()->GetSessionDbLocaleIndex();
					if (loc_idx >= 0) if (NpcTextLocale const *nl = sObjectMgr->GetNpcTextLocale(textid)) sObjectMgr->GetLocaleString(
							nl->Text_0 [0], loc_idx, title);
				}
				else
				{
...
Quest const * Player::GetNextQuest(uint64 guid, Quest const *pQuest)
{
	...
	Creature *pCreature = ObjectAccessor::GetCreatureOrPetOrVehicle(*this,
			guid);
	if (pCreature) pObjectQR = sObjectMgr->GetCreatureQuestRelationBounds(
			pCreature->GetEntry());
	else
	{
		//we should obtain map pointer from GetMap() in 99% of cases. Special case
		...
		ASSERT(_map);
		GameObject *pGameObject = _map->GetGameObject(guid);
		if (pGameObject) pObjectQR = sObjectMgr->GetGOQuestRelationBounds(
				pGameObject->GetEntry());
		else return NULL;
	}
	...
	{
		if (itr->second == nextQuestID) return sObjectMgr->GetQuestTemplate(
				nextQuestID);
	}
...
bool Player::CanSeeStartQuest(Quest const *pQuest)
{
	...
	{
		return getLevel() + sWorld->getIntConfig(
...
{
	if (!SatisfyQuestLog(msg)) return false;
	...
	if (srcitem > 0)
	{
		...
		if (msg2 == EQUIP_ERR_CANT_CARRY_MORE_OF_THIS) return true;
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
	if (quest_id)
	{
		...
		if (!qInfo->IsRepeatable() && rewItr != m_RewardedQuests.end()) return false; // not allow re-complete quest

		// auto complete quest
		if ((qInfo->IsAutoComplete()
				|| qInfo->GetQuestFlags() & QUEST_FLAGS_AUTOCOMPLETE)
				&& CanTakeQuest(qInfo, false)) return true;

		QuestStatusMap::iterator itr = m_QuestStatus.find(quest_id);
		if (itr == m_QuestStatus.end()) return false;
		...
		if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
		{
			if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_DELIVER))
			{
				for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
				{
					if (qInfo->ReqItemCount [i] != 0
							&& q_status.m_itemcount [i]
									< qInfo->ReqItemCount [i]) return false;
				}
			}
							...
							| QUEST_SPECIAL_FLAG_SPEAKTO))
			{
				...
				{
					if (qInfo->ReqCreatureOrGOId [i] == 0) continue;

					if (qInfo->ReqCreatureOrGOCount [i] != 0
							&& q_status.m_creatureOrGOcount [i]
									< qInfo->ReqCreatureOrGOCount [i]) return false;
				}
			}

			if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT)
					&& !q_status.m_explored) return false;

			if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_TIMED)
					&& q_status.m_timer == 0) return false;

			if (qInfo->GetRewOrReqMoney() < 0)
			{
				if (!HasEnoughMoney(-qInfo->GetRewOrReqMoney())) return false;
			}

			uint32 repFacId = qInfo->GetRepObjectiveFaction();
			if (repFacId
					&& GetReputationMgr().GetReputation(repFacId)
							< qInfo->GetRepObjectiveValue()) return false;

			uint32 repFacId2 = qInfo->GetRepObjectiveFaction2();
			if (repFacId2
					&& GetReputationMgr().GetReputation(repFacId)
							< qInfo->GetRepObjectiveValue2()) return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const *pQuest)
{
	...
	if (!CanTakeQuest(pQuest, false)) return false;

	if (pQuest->HasSpecialFlag(QUEST_SPECIAL_FLAG_DELIVER)) for (uint8 i = 0;
			i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
		if (pQuest->ReqItemId [i] && pQuest->ReqItemCount [i]
				&& !HasItemCount(pQuest->ReqItemId [i],
						pQuest->ReqItemCount [i])) return false;

	if (!CanRewardQuest(pQuest, false)) return false;
...
bool Player::CanRewardQuest(Quest const *pQuest, bool msg)
{
	...
	if (!pQuest->IsDFQuest() && !pQuest->IsAutoComplete()
			...
			&& GetQuestStatus(pQuest->GetQuestId()) != QUEST_STATUS_COMPLETE) return false;

	// daily quest can't be rewarded (25 daily quest already completed)
	if (!SatisfyQuestDay(pQuest, true) || !SatisfyQuestWeek(pQuest, true)) return false;

	// rewarded and not repeatable quest (only cheating case, then ignore without message)
	if (GetQuestRewardStatus(pQuest->GetQuestId())) return false;
	...
	if (pQuest->HasFlag(QUEST_SPECIAL_FLAG_DELIVER))
	{
		for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
		{
			...
			{
				if (msg) SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, NULL, NULL,
						pQuest->ReqItemId [i]);
				return false;
			}
		}
	...
	if (pQuest->GetRewOrReqMoney() < 0
			&& !HasEnoughMoney(-pQuest->GetRewOrReqMoney())) return false;
...
bool Player::CanRewardQuest(Quest const *pQuest, uint32 reward, bool msg)
{
	...
	if (!CanRewardQuest(pQuest, msg)) return false;
...
void Player::AddQuest(Quest const *pQuest, Object *questGiver)
{
	...
	if (pQuest->GetRepObjectiveFaction()) if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(pQuest->GetRepObjectiveFaction())) GetReputationMgr().SetVisible(
			factionEntry);

	if (pQuest->GetRepObjectiveFaction2()) if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(pQuest->GetRepObjectiveFaction2())) GetReputationMgr().SetVisible(
			factionEntry);
	...
	if (pQuest->HasFlag(QUEST_SPECIAL_FLAG_TIMED))
	{
		...
		// shared timed quest
		if (questGiver && questGiver->GetTypeId() == TYPEID_PLAYER) limittime =
	...
	else questStatusData.m_timer = 0;
	...
	if (questGiver && pQuest->GetQuestStartScript() != 0) GetMap()->ScriptsStart(
			sQuestStartScripts, pQuest->GetQuestStartScript(), questGiver,
			this);
	...
	if (saBounds.first != saBounds.second)
	{
		...
		for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
			...
			if (itr->second->autocast
					&& itr->second->IsFitToRequirements(this, zone, area)) if (!HasAura(
					itr->second->spellId)) CastSpell(this, itr->second->spellId,
					true);
	}
	...
	switch (pQuest->GetQuestId())
	{
		...
		case 10069:
		{
			if (this->HasSpell(20271)) this->KilledMonsterCredit(44420, NULL);
		}
		default:
			break;
	}
...
void Player::CompleteQuest(uint32 quest_id)
{
	if (quest_id)
	{
		...
		if (log_slot < MAX_QUEST_LOG_SIZE) SetQuestSlotState(log_slot,
				QUEST_STATE_COMPLETE);

		if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(quest_id))
		{
			if (qInfo->HasFlag(QUEST_FLAGS_AUTO_REWARDED)) RewardQuest(qInfo, 0,
					this, false);
			else SendQuestComplete(quest_id);
		}
	}
}
...
void Player::IncompleteQuest(uint32 quest_id)
{
	if (quest_id)
	{
		...
		uint16 log_slot = FindQuestSlot(quest_id);
		if (log_slot < MAX_QUEST_LOG_SIZE) RemoveQuestSlotState(log_slot,
				QUEST_STATE_COMPLETE);
	}
}
		...
		bool announce)
{
	...
	for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; ++i)
		if (pQuest->ReqItemId [i]) DestroyItemCount(pQuest->ReqItemId [i],
				pQuest->ReqItemCount [i], true);
	...
	uint16 log_slot = FindQuestSlot(quest_id);
	if (log_slot < MAX_QUEST_LOG_SIZE) SetQuestSlot(log_slot, 0);
	...
	int32 moneyRew = 0;
	if (getLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) GiveXP(XP,
			NULL);
	else moneyRew = int32(
			pQuest->GetRewMoneyMaxLevel() * sWorld->getRate(RATE_DROP_MONEY));

	// If the player has a guild, it should gain 1/4 of his experience.
	// Despite of him being at max level or not.
	if (Guild * pGuild = sObjectMgr->GetGuildById(GetGuildId())) pGuild->GainXP(
			XP / 4);

	// Give player extra money if GetRewOrReqMoney > 0 and get ReqMoney if negative
	if (pQuest->GetRewOrReqMoney()) moneyRew += pQuest->GetRewOrReqMoney();
	...
	// honor reward
	if (pQuest->GetRewHonorAddition()) RewardHonor(NULL, 0,
			pQuest->GetRewHonorAddition());
	if (pQuest->GetRewHonorMultiplier()) RewardHonor(
			NULL,
			0,
	...
	{
		if (CharTitlesEntry const* titleEntry = sCharTitlesStore.LookupEntry(pQuest->GetCharTitleId())) SetTitle(
				titleEntry);
	}
	...
	for (uint32 i = 0; i < QUEST_CURRENCY_COUNT; i++)
	{
		...
		if (currId && currCount) ModifyCurrency(currId,
	...
	else if (pQuest->IsWeekly()) SetWeeklyQuestStatus(quest_id);

	RemoveActiveQuest(quest_id);
	m_RewardedQuests.insert(quest_id);
	m_RewardedQuestsSave [quest_id] = true;

	if (announce) SendQuestReward(pQuest, XP, questGiver);

	// cast spells after mark quest complete (some spells have quest completed state reqyurements in spell_area data)
	if (pQuest->GetRewSpellCast() > 0) CastSpell(this,
			pQuest->GetRewSpellCast(), true);
	else if (pQuest->GetRewSpell() > 0) CastSpell(this, pQuest->GetRewSpell(),
			true);

	// Hidden Spell Casts. This can still be present with RewSpellCast
	if (pQuest->GetRewSpellHiddenCast() > 0) CastSpell(this,
			pQuest->GetRewSpellHiddenCast(), true);

	if (pQuest->GetZoneOrSort() > 0) GetAchievementMgr().UpdateAchievementCriteria(
	...
	if (saEndBounds.first != saEndBounds.second)
	{
		...
		for (SpellAreaForAreaMap::const_iterator itr = saEndBounds.first;
			...
			if (!itr->second->IsFitToRequirements(this, zone, area)) RemoveAurasDueToSpell(
					itr->second->spellId);
	}
	...
	if (saBounds.first != saBounds.second)
	{
		if (!zone || !area) GetZoneAndAreaId(zone, area);
...
bool Player::SatisfyQuestSkillOrClass(Quest const* qInfo, bool msg)
{
	...
	if (zoneOrSort >= 0 && skillOrClassMask == 0) return true;
	...
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
		return false;
	}
...
bool Player::SatisfyQuestLevel(Quest const* qInfo, bool msg)
{
	...
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_QUEST_FAILED_LOW_LEVEL);
		return false;
	}
	else if (qInfo->GetMaxLevel() > 0 && getLevel() > qInfo->GetMaxLevel())
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ); // There doesn't seem to be a specific response for too high player level
		return false;
	}
	return true;
}
...
{
	// exist free slot
	if (FindQuestSlot(0) < MAX_QUEST_LOG_SIZE) return true;
...
bool Player::SatisfyQuestPreviousQuest(Quest const* qInfo, bool msg)
{
	...
	if (qInfo->prevQuests.empty()) return true;
			...
			iter != qInfo->prevQuests.end(); ++iter)
	{
		...
		if (qPrevInfo)
		{
					...
					&& m_RewardedQuests.find(prevId) != m_RewardedQuests.end())
			{
				...
				if (qPrevInfo->GetExclusiveGroup() >= 0) return true;
...
bool Player::SatisfyQuestRace(Quest const* qInfo, bool msg)
{
	...
	if (reqraces == 0) return true;
	if ((reqraces & getRaceMask()) == 0)
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_QUEST_FAILED_WRONG_RACE);
		return false;
	}
	return true;
}
...
bool Player::SatisfyQuestStatus(Quest const* qInfo, bool msg)
{
	...
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_QUEST_ALREADY_ON);
		return false;
	}
	return true;
}
...
bool Player::SatisfyQuestTimed(Quest const* qInfo, bool msg)
{
	...
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_QUEST_ONLY_ONE_TIMED);
		return false;
	}
	return true;
}
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg)
{
	...
	if (qInfo->GetExclusiveGroup() <= 0) return true;
	...
	for (; iter != end; ++iter)
	{
		...
		// skip checked quest id, only state of other quests in group is interesting
		if (exclude_Id == qInfo->GetQuestId()) continue;
...
bool Player::SatisfyQuestPrevChain(Quest const* qInfo, bool msg)
{
	...
	if (qInfo->prevChainQuests.empty()) return true;
...
{
	if (!qInfo->IsDaily() && !qInfo->IsDFQuest()) return true;

	if (qInfo->IsDFQuest())
	{
		if (!m_DFQuests.empty()) return false;
			...
			++quest_daily_idx)
	{
		...
		if (qInfo->GetQuestId() == id) return false;

		if (!id) have_slot = true;
	}

	if (!have_slot)
	{
		if (msg) SendCanTakeQuestResponse(INVALIDREASON_DAILY_QUESTS_REMAINING);
		return false;
	}
...
{
	if (!qInfo->IsWeekly() || m_weeklyquests.empty()) return true;
...
bool Player::GiveQuestSourceItem(Quest const *pQuest)
{
	...
	if (srcitem > 0)
	{
		...
		else if (msg == EQUIP_ERR_CANT_CARRY_MORE_OF_THIS) return true;
		else SendEquipError(msg, NULL, NULL, srcitem);
		return false;
	}
...
bool Player::TakeQuestSourceItem(uint32 quest_id, bool msg)
{
	...
	if (qInfo)
	{
		...
		if (srcitem > 0)
		{
			...
			if (res != EQUIP_ERR_OK)
			{
				if (msg) SendEquipError(res, NULL, NULL, srcitem);
				return false;
			}
...
bool Player::GetQuestRewardStatus(uint32 quest_id) const
{
	...
	if (qInfo)
	{
		...
		if (!qInfo->IsRepeatable()) return m_RewardedQuests.find(quest_id)
				!= m_RewardedQuests.end();
...
QuestStatus Player::GetQuestStatus(uint32 quest_id) const
{
	if (quest_id)
	{
		...
		if (itr != m_QuestStatus.end()) return itr->second.m_status;

		if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(quest_id)) if (!qInfo->IsRepeatable()
				&& m_RewardedQuests.find(quest_id) != m_RewardedQuests.end()) return QUEST_STATUS_REWARDED;
	}
...
uint16 Player::GetReqKillOrCastCurrentCount(uint32 quest_id, int32 entry)
{
	...
	for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
		if (qInfo->ReqCreatureOrGOId [j] == entry) return m_QuestStatus [quest_id].m_creatureOrGOcount [j];
...
uint16 Player::FindQuestSlot(uint32 quest_id) const
{
	for (uint16 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
		if (GetQuestSlotQuestId(i) == quest_id) return i;
...
void Player::AreaExploredOrEventHappens(uint32 questId)
{
	if (questId)
	{
		...
		if (CanCompleteQuest(questId)) CompleteQuest(questId);
	}
}
	...
	if (Group *pGroup = GetGroup())
	{
				...
				itr->next())
		{
			...
			if (pGroupGuy && pGroupGuy->IsAtGroupRewardDistance(pEventObject)
					&& !pGroupGuy->GetCorpse()) pGroupGuy->AreaExploredOrEventHappens(
					questId);
		}
	}
	else AreaExploredOrEventHappens(questId);
}
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
	for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
	{
		...
		if (q_status.m_status != QUEST_STATUS_INCOMPLETE) continue;

		Quest const* qInfo = sObjectMgr->GetQuestTemplate(questid);
		if (!qInfo || !qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_DELIVER)) continue;
...
void Player::ItemRemovedQuestCheck(uint32 entry, uint32 count)
{
	for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
	{
		...
		Quest const* qInfo = sObjectMgr->GetQuestTemplate(questid);
		if (!qInfo) continue;
		if (!qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_DELIVER)) continue;
		...
		for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j)
		{
			...
			if (reqitem == entry)
			{
				...
				uint32 reqitemcount = qInfo->ReqItemCount [j];
				uint16 curitemcount;
				if (q_status.m_status != QUEST_STATUS_COMPLETE) curitemcount =
						q_status.m_itemcount [j];
				else curitemcount = GetItemCount(entry, true);
...
{
	if (cInfo->Entry) KilledMonsterCredit(cInfo->Entry, guid);

	for (uint8 i = 0; i < MAX_KILL_CREDIT; ++i)
		if (cInfo->KillCredit [i]) KilledMonsterCredit(cInfo->KillCredit [i],
				0);
}
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
	...
	if (guid)
	{
		...
		if (killed && killed->GetEntry()) real_entry = killed->GetEntry();
	}
						...
						|| qInfo->IsAllowedInRaid()))
		{
			if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_KILL_OR_CAST))
			{
				for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
				{
					...
					if (qInfo->ReqCreatureOrGOId [j] <= 0) continue;

					// skip Cast at creature objective
					if (qInfo->ReqSpell [j] != 0) continue;
		...
		if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
		{
			if (qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAG_KILL_OR_CAST))
			{
				for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
				{
					...
					if (qInfo->ReqSpell [j] != spell_id) continue;
					...
					if (isCreature)
					{
						...
						if (qInfo->ReqCreatureOrGOId [j] > 0)
						{
							...
							if (reqTarget != entry) // if entry doesn't match, check for killcredits referenced in template
							{
								...
								for (uint8 i = 0; i < MAX_KILL_CREDIT; ++i)
									if (cinfo->KillCredit [i] == reqTarget) entry =
											cinfo->KillCredit [i];
							}
						}
					}
							...
							| QUEST_SPECIAL_FLAG_SPEAKTO))
			{
				for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
				{
					...
					if (qInfo->ReqSpell [j] > 0
							|| qInfo->ReqCreatureOrGOId [j] < 0) continue;
...
void Player::MoneyChanged(uint32 count)
{
	for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
	{
		...
		if (qInfo && qInfo->GetRewOrReqMoney() < 0)
		{
			...
			{
				if (int32(count) < -qInfo->GetRewOrReqMoney()) IncompleteQuest(
						questid);
			}
		}
	}
				...
				if (qInfo->GetRepObjectiveFaction() == factionEntry->ID)
				{
					...
					if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
					{
						if (GetReputationMgr().GetReputation(factionEntry)
								>= qInfo->GetRepObjectiveValue()) if (CanCompleteQuest(
								questid)) CompleteQuest(questid);
					}
					else if (q_status.m_status == QUEST_STATUS_COMPLETE)
					{
						if (GetReputationMgr().GetReputation(factionEntry)
								< qInfo->GetRepObjectiveValue()) IncompleteQuest(
								questid);
					}
				}
			}
		...
		if (uint32 questid = GetQuestSlotQuestId(i))
		{
			if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(questid))
			{
				if (qInfo->GetRepObjectiveFaction2() == factionEntry->ID)
				{
					...
					{
						if (GetReputationMgr().GetReputation(factionEntry)
								>= qInfo->GetRepObjectiveValue2()) if (CanCompleteQuest(
								questid)) CompleteQuest(questid);
					}
					else if (q_status.m_status == QUEST_STATUS_COMPLETE)
					{
						if (GetReputationMgr().GetReputation(factionEntry)
								< qInfo->GetRepObjectiveValue2()) IncompleteQuest(
								questid);
					}
				}
			}
...
bool Player::HasQuestForItem(uint32 itemid) const
{
	for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
	{
		...
		if (qs_itr == m_QuestStatus.end()) continue;
		...
		if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
		{
			...
			if (GetGroup() && GetGroup()->isRaidGroup()
					&& !qinfo->IsAllowedInRaid()) if (!InBattleground()) //there are two ways.. we can make every bg-quest a raidquest, or add this code here.. i don't know if this can be exploited by other quests, but i think all other quests depend on a specific area.. but keep this in mind, if something strange happens later
			continue;
			...
			for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j)
			{
				if (itemid == qinfo->ReqItemId [j]
						&& q_status.m_itemcount [j] < qinfo->ReqItemCount [j]) return true;
			}
				...
				if (qinfo->ReqSourceId [j] == itemid)
				{
					...
					if (pProto->MaxCount
							&& int32(GetItemCount(itemid, true))
									< pProto->MaxCount) return true;

					// allows custom amount drop when not 0
					if (qinfo->ReqSourceCount [j])
					{
						if (GetItemCount(itemid, true)
								< qinfo->ReqSourceCount [j]) return true;
					}
					else if (GetItemCount(itemid, true)
							< pProto->GetMaxStackSize()) return true;
				}
			}
		}
	}
		...
		Object * questGiver)
{
	...
	if (pQuest->GetQuestCompleteScript() != 0) GetMap()->ScriptsStart(
			sQuestEndScripts, pQuest->GetQuestCompleteScript(), questGiver,
			this);
}
...
void Player::SendQuestWindowClose(uint32 questId)
{
	if (questId)
	{
		...
		GetSession()->SendPacket(&data);
		sLog->outDebug(LOG_FILTER_NETWORKIO,
				"WORLD: Sent SMSG_QUESTGIVER_QUEST_FAILED for WindowClose");
	}
}
...
void Player::SendQuestConfirmAccept(const Quest* pQuest, Player* pReceiver)
{
	if (pReceiver)
	{
		...
		if (loc_idx >= 0) if (const QuestLocale* pLocale = sObjectMgr->GetQuestLocale(pQuest->GetQuestId())) sObjectMgr->GetLocaleString(
				pLocale->Title, loc_idx, strTitle);
		...
		uint32 creatureOrGO_idx, uint16 old_count, uint16 add_count)
{
	...
	if (log_slot < MAX_QUEST_LOG_SIZE) SetQuestSlotCounter(log_slot,
			creatureOrGO_idx,
			GetQuestSlotCounter(log_slot, creatureOrGO_idx) + add_count);
}

void Player::SendQuestUpdateAddPlayer(Quest const* quest, uint16 old_count,
		uint16 add_count)
{
	...
	sLog->outDebug(LOG_FILTER_NETWORKIO,
			"WORLD: Sent SMSG_QUESTUPDATE_ADD_PVP_KILL");
	...
	if (log_slot < MAX_QUEST_LOG_SIZE) SetQuestSlotCounter(log_slot,
			QUEST_PVP_KILL_SLOT,
			GetQuestSlotCounter(log_slot, QUEST_PVP_KILL_SLOT) + add_count);
}
...
void Player::_LoadArenaStatsInfo(PreparedQueryResult result)
{
	...
	if (result)
	{
		do
		{
			...
			personalRatingCache [fields [0].GetUInt8()] =
					fields [1].GetUInt16();
		}
		while (result->NextRow());
	}
		...
		WorldSession * session)
{
	...
	if (player && player) if (player->_LoadFromDB(guid, holder, result)) return player;
		...
		PreparedQueryResult & result)
{
	...
	if (money > MAX_MONEY_AMOUNT) money = MAX_MONEY_AMOUNT;
	SetMoney(money);
...
if(	!_LoadHomeBind(
	...
	if (raidDiff >= MAX_RAID_DIFFICULTY) raidDiff =
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
	...
	if (arena_currency > sWorld->getIntConfig(CONFIG_MAX_JUSTICE_POINTS)) arena_currency =
			sWorld->getIntConfig(CONFIG_MAX_JUSTICE_POINTS);

	// check arena teams integrity
	for (uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT; ++arena_slot)
	{
		uint32 arena_team_id = GetArenaTeamId(arena_slot);
		if (!arena_team_id) continue;

		if (ArenaTeam * at = sObjectMgr->GetArenaTeamById(arena_team_id)) if (at->IsMember(
				GetGUID())) continue;
	...
	else if (mapEntry && mapEntry->IsBattlegroundOrArena())
	{
		...
		{
			// leave bg
			if (player_at_bg) currentBg->RemovePlayerAtLeave(GetGUID(), false,
					true);
	...
	else if (!taxi_nodes.empty())
	{
		...
		else if (!m_taxi.LoadTaxiDestinationsFromString(taxi_nodes, GetTeam()))
		{
			...
			if (uint32 node_id = m_taxi.GetTaxiSource()) nodeEntry =
					sTaxiNodesStore.LookupEntry(node_id);
	...
	if (mapEntry)
	{
		...
		if (instanceId) if (InstanceSave * save = GetInstanceSave(mapId, mapEntry->IsRaid())) if (save->GetInstanceId()
	...
	if (instanceId && !sInstanceSaveMgr->GetInstanceSave(instanceId))
	{
		...
		if (at) Relocate(at->target_X, at->target_Y, at->target_Z,
	...
	float soberFactor;
	if (time_diff > 15 * MINUTE) soberFactor = 0;
	else soberFactor = 1 - time_diff / (15.0f * MINUTE);
	...
	if (m_deathExpireTime > now + MAX_DEATH_COUNT * DEATH_EXPIRE_STEP) m_deathExpireTime =
			now + MAX_DEATH_COUNT * DEATH_EXPIRE_STEP - 1;
	...
	if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GHOST)) m_deathState = DEAD;
	...
	if (curTitle && !HasTitle(curTitle)) curTitle = 0;
	...
	if (!isAlive()) RemoveAllAurasOnDeath();
	else RemoveAllAurasRequiringDeadTarget();
	...
	if (GetSession()->GetSecurity() > SEC_PLAYER)
	{
		switch (sWorld->getIntConfig(CONFIG_GM_LOGIN_STATE))
		{
			...
			case 1:
				...
				break; // enable
			case 2: // save state
				if (extraflags & PLAYER_EXTRA_GM_ON) SetGameMaster(true);
				break;
		}
		...
		switch (sWorld->getIntConfig(CONFIG_GM_VISIBLE_STATE))
		{
			...
			case 1:
				break; // visible
			case 2: // save state
				if (extraflags & PLAYER_EXTRA_GM_INVISIBLE) SetGMVisible(false);
				break;
		}
		...
		switch (sWorld->getIntConfig(CONFIG_GM_CHAT))
		{
			...
			case 1:
				...
				break; // enable
			case 2: // save state
				if (extraflags & PLAYER_EXTRA_GM_CHAT) SetGMChat(true);
				break;
		}
		...
		switch (sWorld->getIntConfig(CONFIG_GM_WHISPERING_TO))
		{
			...
			case 1:
				...
				break; // enable
			case 2: // save state
				if (extraflags & PLAYER_EXTRA_ACCEPT_WHISPERS) SetAcceptWhispers(
						true);
				break;
		}
	}
...
{
	if (!creature->isDead() || !creature->IsDamageEnoughForLootingAndReward()) return false;
	...
	if (!thisGroup) return this == creature->GetLootRecipient();
	else if (thisGroup != creature->GetLootRecipientGroup()) return false;
	...
	switch (thisGroup->GetLootMethod())
	{
		...
		case MASTER_LOOT:
			...
			if (loot->roundRobinPlayer == 0
					|| loot->roundRobinPlayer == GetGUID()) return true;
		...
		case NEED_BEFORE_GREED:
			...
			if (loot->hasOverThresholdItem()) return true;
...
void Player::_LoadActions(PreparedQueryResult result)
{
	...
	if (result)
	{
		do
		{
			...
			if (ActionButton* ab = addActionButton(button, action, type)) ab->uState =
...
{
	if (isAlive()) sObjectAccessor->ConvertCorpseForPlayer(GetGUID());
	else
	{
		if (Corpse *corpse = GetCorpse()) ApplyModFlag(
				PLAYER_FIELD_BYTES,
			...
			if (!bag_guid)
			{
				...
				else if (IsEquipmentPos(INVENTORY_SLOT_BAG_0, slot))
				{
					...
					if (CanEquipItem(slot, dest, item, false, false)
							== EQUIP_ERR_OK) QuickEquipItem(dest, item);
					else success = false;
				}
				else if (IsBankPos(INVENTORY_SLOT_BAG_0, slot))
				{
					...
					if (CanBankItem(INVENTORY_SLOT_BAG_0, slot, dest, item,
							false, false) == EQUIP_ERR_OK) item = BankItem(dest,
							item, true);
					else success = false;
				}
				...
				if (success)
				{
					...
					if (item->IsBag() && IsBagPos(item->GetPos())) bagMap [item_guid] =
							(Bag*) item;
				}
			}
			else
			{
				...
				if (itr != bagMap.end())
				{
					...
					uint8 result = CanStoreItem(itr->second->GetSlot(), slot,
							dest, item);
					if (result == EQUIP_ERR_OK) itr->second->StoreItem(slot,
							item, true);
					else
					{
						sLog->outError(
		...
		PreparedQueryResult resultDelivery)
{
	...
	if (resultUnread) unReadMails = (*resultUnread) [0].GetUInt8();
	...
	if (resultDelivery) m_nextMailDelivereTime =
...
void Player::_LoadMail()
{
	...
	if (result)
	{
		do
		{
			...
			if (has_items) _LoadMailedItems(m);
...
void Player::LoadPet()
{
	...
	if (IsInWorld())
	{
		...
		if (!pet->LoadPetFromDB(this, 0, 0, true, PET_SLOT_ACTUAL_PET_SLOT)) delete pet;
	}
}
	...
	if (result)
	{
		do
		{
			...
			if (pQuest)
			{
				...
				if (qstatus < MAX_QUEST_STATUS) questStatusData.m_status =
						QuestStatus(qstatus);
				else
				{
					questStatusData.m_status = QUEST_STATUS_INCOMPLETE;
						...
						&& !GetQuestRewardStatus(quest_id))
				{
					...
					if (quest_time <= sWorld->GetGameTime()) questStatusData.m_timer =
							1;
					else questStatusData.m_timer = uint32(
							(quest_time - sWorld->GetGameTime())
									* IN_MILLISECONDS);
				}
				else quest_time = 0;

				questStatusData.m_creatureOrGOcount [0] =
						fields [4].GetUInt16();
				questStatusData.m_creatureOrGOcount [1] =
						fields [5].GetUInt16();
				questStatusData.m_creatureOrGOcount [2] =
						fields [6].GetUInt16();
				questStatusData.m_creatureOrGOcount [3] =
						fields [7].GetUInt16();
						...
						&& questStatusData.m_status != QUEST_STATUS_NONE)
				{
					...
					if (questStatusData.m_status == QUEST_STATUS_COMPLETE) SetQuestSlotState(
							slot, QUEST_STATE_COMPLETE);
					else if (questStatusData.m_status == QUEST_STATUS_FAILED) SetQuestSlotState(
							slot, QUEST_STATE_FAIL);

					for (uint8 idx = 0; idx < QUEST_OBJECTIVES_COUNT; ++idx)
						if (questStatusData.m_creatureOrGOcount [idx]) SetQuestSlotCounter(
								slot, idx,
	...
	if (result)
	{
		do
		{
			...
			if (pQuest)
			{
				...
				if (pQuest->GetBonusTalents()) m_questRewardTalentCount +=
						pQuest->GetBonusTalents();
			}
...
void Player::_LoadCurrency(PreparedQueryResult result)
{
	...
	if (result)
	{
		do
		{
			...
			else cur.totalCount = totalCount > totalCap ? totalCap : totalCount;

			if (weekCap == 0) cur.weekCount = weekCount;
			else cur.weekCount = weekCount > weekCap ? weekCap : weekCount;
...
void Player::_LoadConquestPointsWeekCap(PreparedQueryResult result)
{
	...
	if (result)
	{
		do
		{
			...
			if (source != CP_SOURCE_ARENA && source != CP_SOURCE_RATED_BG) continue;
	...
	if (result)
	{
		do
		{
			...
			if (InstanceSave * save = sInstanceSaveMgr->AddInstanceSave(mapId,
					instanceId, Difficulty(difficulty), resetTime, !perm, true)) BindToInstance(
					save, perm, true);
		}
		while (result->NextRow());
	}
		...
		Difficulty difficulty)
{
	...
	MapDifficulty const* mapDiff = GetDownscaledMapDifficultyData(mapid,
			difficulty);
	if (!mapDiff) return NULL;

	BoundInstancesMap::iterator itr = m_boundInstances [difficulty].find(mapid);
	if (itr != m_boundInstances [difficulty].end()) return &itr->second;
	else return NULL;
}
...
InstanceSave * Player::GetInstanceSave(uint32 mapid, bool raid)
{
	...
	if (!pBind || !pBind->perm) if (Group *group = GetGroup()) if (InstanceGroupBind *groupBind = group->GetBoundInstance(this)) pSave =
			groupBind->save;
		...
		Difficulty difficulty, bool unload)
{
	...
	{
		if (!unload) CharacterDatabase.PExecute(
		...
		bool load)
{
	if (save)
	{
		...
		if (bind.save)
		{
			...
			if (permanent != bind.perm || save != bind.save) if (!load) CharacterDatabase.PExecute(
		...
		else if (!load) CharacterDatabase.PExecute(
...
void Player::BindToInstance()
{
	...
	if (_pendingBind->GetInstanceId() != GetInstanceId()) return;
		...
		uint64 player_guid)
{
	...
	if (player)
	{
		for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
		{
			...
			{
				has_binds = true;
				if (group) group->BindToInstance(itr->second.save,
						itr->second.perm, true);
		...
		bool report)
{
	if (!isGameMaster() && ar)
	{
		...
		{
			if (ar->levelMin && getLevel() < ar->levelMin) LevelMin =
					ar->levelMin;
			if (ar->levelMax && getLevel() > ar->levelMax) LevelMax =
					ar->levelMax;
		}
		...
		if (ar->item)
		{
			if (!HasItemCount(ar->item, 1)
					&& (!ar->item2 || !HasItemCount(ar->item2, 1))) missingItem =
					ar->item;
		}
		else if (ar->item2 && !HasItemCount(ar->item2, 1)) missingItem =
				ar->item2;
		...
		if (GetTeam() == ALLIANCE && ar->quest_A
				&& !GetQuestRewardStatus(ar->quest_A)) missingQuest =
				ar->quest_A;
		else if (GetTeam() == HORDE && ar->quest_H
				&& !GetQuestRewardStatus(ar->quest_H)) missingQuest =
				ar->quest_H;

		uint32 missingAchievement = 0;
		if (ar->achievement
				&& !GetAchievementMgr().HasAchieved(
						sAchievementStore.LookupEntry(ar->achievement))) missingAchievement =
				ar->achievement;
				...
				|| missingAchievement)
		{
			if (report)
			{
				if (missingQuest && !ar->questFailedText.empty()) ChatHandler(
						GetSession()).PSendSysMessage("%s",
						ar->questFailedText.c_str());
				else if (mapDiff->hasErrorMessage) // if (missingAchievement) covered by this case
				SendTransferAborted(target_map, TRANSFER_ABORT_DIFFICULTY,
						target_difficulty);
				else if (missingItem) GetSession()->SendAreaTriggerMessage(
						GetSession()->GetArkCoreString(
								LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin,
						ObjectMgr::GetItemPrototype(missingItem)->Name1);
				else if (LevelMin) GetSession()->SendAreaTriggerMessage(
						GetSession()->GetArkCoreString(LANG_LEVEL_MINREQUIRED),
						LevelMin);
			}
			return false;
		}
...
bool Player::CheckInstanceLoginValid()
{
	...
	if (!GetMap()->IsDungeon() || isGameMaster()) return true;
	...
	else
	{
		...
		if (!GetGroup() && GetMap()->GetPlayersCountExceptGMs() > 1) return false;
	}
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
	...
	if (result)
	{
		...
		if (MapManager::IsValidMapCoord(m_homebindMapId, m_homebindX,
				...
				&& GetSession()->Expansion() >= bindMapEntry->Expansion()) ok =
				true;
		else CharacterDatabase.PExecute(
...
void Player::SaveToDB()
{
	...
	if (m_transport) ss << m_transport->GetGUIDLow();
	else ss << "0";
	ss << ", ";
	...
	if (m_session->isLogingOut() || !sWorld->getBoolConfig(
			CONFIG_STATS_SAVE_ONLY_ON_LOGOUT)) _SaveStats(trans);

	CharacterDatabase.CommitTransaction(trans);

	// save pet (hunter pet level and experience and all type pets health/mana).
	if (Pet* pet = GetPet()) pet->SavePetToDB(PET_SLOT_ACTUAL_PET_SLOT);
}
...
void Player::_SaveAuras(SQLTransaction& trans)
{
	...
	{
		if (!itr->second->CanBeSaved()) continue;
		...
		for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
		{
			if (AuraEffect const* effect = aura->GetEffect(i))
			{
				...
				effMask |= 1 << i;
				if (effect->CanBeRecalculated()) recalculateMask |= 1 << i;
			}
			else
			{
				baseDamage [i] = 0;
	...
	for (size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
	{
		...
		if (item->GetState() != ITEM_REMOVED)
		{
			...
			if (test == NULL)
			{
				...
				if (Item* test2 = GetItemByPos(INVENTORY_SLOT_BAG_0, item->GetBagSlot())) bagTestGUID =
						test2->GetGUIDLow();
				sLog->outError(
...
void Player::_SaveDailyQuestStatus(SQLTransaction& trans)
{
	...
	for (uint32 quest_daily_idx = 0; quest_daily_idx < PLAYER_MAX_DAILY_QUESTS;
			++quest_daily_idx)
		if (GetUInt32Value(PLAYER_FIELD_DAILY_QUESTS_1 + quest_daily_idx)) trans->PAppend(
...
{
	if (!m_WeeklyQuestChanged || m_weeklyquests.empty()) return;
...
void Player::_SaveSpells(SQLTransaction& trans)
{
	...
	{
		if (itr->second->state == PLAYERSPELL_REMOVED
				|| itr->second->state == PLAYERSPELL_CHANGED) trans->PAppend(
...
void Player::_SaveCurrency()
{
	...
	{
		if (itr->second.state == PLAYERCURRENCY_CHANGED) CharacterDatabase.PExecute(
				...
				itr->first);
		else if (itr->second.state == PLAYERCURRENCY_NEW) CharacterDatabase.PExecute(
		...
		if (itr->second.state == PLAYERCURRENCY_REMOVED) m_currencies.erase(
				itr++);
		else
		{
...
void Player::_SaveStats(SQLTransaction& trans)
{
	...
	if (!sWorld->getIntConfig(CONFIG_MIN_LEVEL_STAT_SAVE) || getLevel() < sWorld->getIntConfig(
			CONFIG_MIN_LEVEL_STAT_SAVE)) return;
...
void Player::UpdateSpeakTime()
{
	...
	if (GetSession()->GetSecurity() > SEC_PLAYER) return;
	...
	if (m_speakTime > current)
	{
		...
		if (m_speakCount >= max_count)
		{
			...
			if (GetSession()->m_muteTime < new_mute) GetSession()->m_muteTime =
					new_mute;
...
void Player::ResetInstances(uint8 method, bool isRaid)
{
			...
			itr != m_boundInstances [diff].end();)
	{
		...
		if (map && map->IsDungeon()) if (!((InstanceMap*) map)->Reset(method))
		{
			++itr;
			continue;
		}
		...
		if (method == INSTANCE_RESET_ALL
				|| method == INSTANCE_RESET_CHANGE_DIFFICULTY) SendResetInstanceSuccess(
				p->GetMapId());
...
{
	if (!m_contestedPvPTimer || isInCombat()) return;
	if (m_contestedPvPTimer <= diff)
	{
		ResetContestedPvP();
	}
	else m_contestedPvPTimer -= diff;
}

void Player::UpdatePvPFlag(time_t currTime)
{
	if (!IsPvP() || InBattleground() || InArena()) return;
	if (pvpInfo.endTimer == 0 || currTime < (pvpInfo.endTimer + 300)) return;
...
{
	if (!duel || duel->startTimer == 0 || currTime < duel->startTimer + 3) return;
...
Pet* Player::GetPet() const
{
	...
	{
		if (!IS_PET_GUID(pet_guid)) return NULL;

		Pet* pet = ObjectAccessor::GetPet(*this, pet_guid);

		if (!pet) return NULL;

		if (IsInWorld() && pet) return pet;
...
void Player::RemovePet(Pet* pet, PetSlot mode, bool returnreagent)
{
	...
	if (pet)
	{
		...
		if (pet->m_removed) return;
	}

	if (mode == PET_SLOT_ACTUAL_PET_SLOT) mode = m_currentPetSlot;

	if (!pet || pet->GetOwnerGUID() != GetGUID()) return;

	if (mode >= PET_SLOT_HUNTER_FIRST && mode <= PET_SLOT_HUNTER_LAST
			&& pet->getPetType() != HUNTER_PET) assert(false); //debug code.
	...
	if (pet->getPetType() != HUNTER_PET) SetMinion(pet, false,
			PET_SLOT_UNK_SLOT);
	else SetMinion(pet, false, PET_SLOT_ACTUAL_PET_SLOT);
	...
	if (pet->isControlled())
	{
		...
		if (GetGroup()) SetGroupUpdateFlag(GROUP_UPDATE_PET);
	}
}
...
void Player::StopCastingCharm()
{
	...
	{
		if (charm->ToCreature()->HasUnitTypeMask(UNIT_MASK_PUPPET)) ((Puppet*) charm)->UnSummon();
		else if (charm->IsVehicle()) ExitVehicle();
	}
	if (GetCharmGUID()) charm->RemoveCharmAuras();
...
void Player::Whisper(const std::string& text, uint32 language, uint64 receiver)
{
	...
	if (rPlayer->isAFK()) ChatHandler(this).PSendSysMessage(LANG_PLAYER_AFK,
			rPlayer->GetName(), rPlayer->afkMsg.c_str());

	// if player whisper someone, auto turn of dnd to be able to receive an answer
	if (isDND() && !rPlayer->isGameMaster()) ToggleDND();
}
...
void Player::VehicleSpellInitialize()
{
	...
	for (uint32 i = 0; i < CREATURE_MAX_SPELLS; ++i)
	{
		...
		else data << uint32(MAKE_UNIT_ACTION_BUTTON(spellId, i+8));
	}
...
void Player::CharmSpellInitialize()
{
	...
	if (charm->GetTypeId() != TYPEID_PLAYER)
	{
		...
		//if (cinfo && cinfo->type == CREATURE_TYPE_DEMON && getClass() == CLASS_WARLOCK)
		{
			for (uint32 i = 0; i < MAX_SPELL_CHARM; ++i)
				if (charmInfo->GetCharmSpell(i)->GetAction()) ++addlist;
		}
	}
	...
	if (charm->GetTypeId() != TYPEID_PLAYER) data
			<< uint8(charm->ToCreature()->GetReactState())
			<< uint8(charmInfo->GetCommandState()) << uint16(0);
	else data << uint8(0) << uint8(0) << uint16(0);
	...
	if (addlist)
	{
		for (uint32 i = 0; i < MAX_SPELL_CHARM; ++i)
		{
			...
			if (cspell->GetAction()) data << uint32(cspell->packedData);
		}
	}
...
{
	if (!mod || !spellInfo) return false;

	// Mod out of charges
	if (spell && mod->charges == -1
			&& spell->m_appliedMods.find(mod->ownerAura)
					== spell->m_appliedMods.end()) return false;

	// +duration to infinite duration spells making them limited
	if (mod->op == SPELLMOD_DURATION && GetSpellDuration(spellInfo) == -1) return false;
...
void Player::AddSpellMod(SpellModifier* mod, bool apply)
{
	...
	for (int eff = 0; eff < 96; ++eff)
	{
		...
		if (mod->mask & _mask)
		{
			...
			{
				if ((*itr)->type == mod->type && (*itr)->mask & _mask) val +=
						(*itr)->value;
			}
			val += apply ? mod->value : -(mod->value);
			data << uint8(eff);
			if (isFlat) data << int32(val);
			else data << float(val);
			count2++;
		}
	}
	...
	if (apply) m_spellMods [mod->op].push_back(mod);
	else
	{
		m_spellMods [mod->op].remove(mod);
		// mods bound to aura will be removed in AuraEffect::~AuraEffect
		if (!mod->ownerAura) delete mod;
	}
}
...
{
	if (!spell || spell->m_appliedMods.empty()) return;
	...
	for (uint8 i = 0; i < MAX_SPELLMOD; ++i)
	{
				...
				itr != m_spellMods [i].end(); ++itr)
		{
			...
			if (!mod->ownerAura || !mod->ownerAura->GetCharges()) continue;

			// Restore only specific owner aura mods
			if (ownerAuraId
					&& (ownerAuraId != mod->ownerAura->GetSpellProto()->Id)) continue;

			// check if mod affected this spell
			Spell::UsedSpellMods::iterator iterMod = spell->m_appliedMods.find(
					mod->ownerAura);
			if (iterMod == spell->m_appliedMods.end()) continue;

			// remove from list
			spell->m_appliedMods.erase(iterMod);

			// add mod charges back to mod
			if (mod->charges == -1) mod->charges = 1;
			else mod->charges++;

			// Do not set more spellmods than avalible
			if (mod->ownerAura->GetCharges() < mod->charges) mod->charges =
...
void Player::RemoveSpellMods(Spell * spell)
{
	...
	if (spell->m_appliedMods.empty()) return;
	...
	for (uint8 i = 0; i < MAX_SPELLMOD; ++i)
	{
				...
				itr != m_spellMods [i].end();)
		{
			...
			if (mod->ownerAura->DropCharge()) itr = m_spellMods [i].begin();
		}
	}
}
...
{
	if (!spell || (m_spellModTakingSpell && m_spellModTakingSpell != spell)) return;

	if (apply && spell->getState() == SPELL_STATE_FINISHED) return;
...
void Player::RemovePetitionsAndSigns(uint64 guid, uint32 type)
{
	...
	if (result)
	{
		...
		{ // and SendPetitionQueryOpcode reads data from the DB
			...
			if (owner) owner->GetSession()->SendPetitionQueryOpcode(
					petitionguid);
		}
		while (result->NextRow());

		if (type == 10) CharacterDatabase.PExecute(
...
void Player::LeaveAllArenaTeams(uint64 guid)
{
	...
	do
	{
		...
		{
			ArenaTeam* arenaTeam = sObjectMgr->GetArenaTeamById(arenaTeamId);
			if (arenaTeam) arenaTeam->DelMember(guid, true);
		}
	}
	while (result->NextRow());
}
...
void Player::SetRestBonus(float rest_bonus_new)
{
	...
	if (getLevel() >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) rest_bonus_new =
			0;
	...
	if (rest_bonus_new > rest_bonus_max) m_rest_bonus = rest_bonus_max;
	else m_rest_bonus = rest_bonus_new;

	// update data for client
	if (m_rest_bonus > 10) SetByteValue(PLAYER_BYTES_2, 3, 0x01); // Set Reststate = Rested
	else if (m_rest_bonus <= 1) SetByteValue(PLAYER_BYTES_2, 3, 0x02); // Set Reststate = Normal
		...
		Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
	...
	if (GetSession()->isLogingOut() || isInCombat()
			|| HasUnitState(UNIT_STAT_STUNNED) || HasUnitState(UNIT_STAT_ROOT))
	{
	...
	if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE)) return false;
	...
	else
	{
		...
		if (Spell* spell = GetCurrentSpell(CURRENT_GENERIC_SPELL)) if (spell->m_spellInfo->Id
				!= spellid) InterruptSpell(CURRENT_GENERIC_SPELL, false);

		InterruptSpell(CURRENT_AUTOREPEAT_SPELL, false);

		if (Spell* spell = GetCurrentSpell(CURRENT_CHANNELED_SPELL)) if (spell->m_spellInfo->Id
				!= spellid) InterruptSpell(CURRENT_CHANNELED_SPELL, true);
	}
	...
	for (uint32 i = 1; i < nodes.size(); ++i)
	{
		...
		if (prevnode == sourcenode) sourcepath = path;
	...
	if (npc) totalcost = (uint32) ceil(
			totalcost * GetReputationPriceDiscount(npc));
...
void Player::ContinueTaxiFlight()
{
	...
	for (uint32 i = 1; i < nodeList.size(); ++i)
	{
		...
		if (node.mapid != GetMapId()) continue;
...
void Player::ProhibitSpellScholl(SpellSchoolMask idSchoolMask, uint32 unTimeMs)
{
			...
			itr != m_spells.end(); ++itr)
	{
		...
		if (spellInfo->Attributes & SPELL_ATTR0_DISABLED_WHILE_ACTIVE) continue;

		if (spellInfo->PreventionType != SPELL_PREVENTION_TYPE_SILENCE) continue;
	...
	switch (form)
	{
		...
		default: // 0, for example
		{
			...
			if (cEntry && cEntry->powerType < MAX_POWERS
					&& uint32(getPowerType()) != cEntry->powerType) setPowerType(
					Powers(cEntry->powerType));
			break;
		}
	}

	// update auras at form change, ignore this at mods reapply (.reset stats/etc) when form not change.
	if (!reapplyMods) UpdateEquipSpellsAtFormChange();
		...
		VendorItem const* crItem, bool bStore)
{
	...
	if (crItem->ExtendedCost) // case for new honor system
	{
		...
		{
			if (iece->RequiredItem [i]) DestroyItemCount(iece->RequiredItem [i],
	...
	if (it)
	{
		...
		if (!bStore) AutoUnequipOffhandIfNeed();
		...
		uint32 item, uint8 count, uint8 bag, uint8 slot)
{
	...
	// cheating attempt
	if (slot > MAX_BAG_SIZE && slot != NULL_SLOT) return false;
	...
	if (count == pProto->BuyCount) count = 1;
	...
	// reputation discount
	if (price) price = uint32(
			floor(price * GetReputationPriceDiscount(pCreature)));
	...
	if ((bag == NULL_BAG && slot == NULL_SLOT) || IsInventoryPos(bag, slot))
	{
		if (!_StoreOrEquipNewItem(vendorslot, item, count, bag, slot, price,
				pProto, pCreature, crItem, true)) return false;
	}
	else if (IsEquipmentPos(bag, slot))
	{
		...
		if (!_StoreOrEquipNewItem(vendorslot, item, count, bag, slot, price,
				pProto, pCreature, crItem, false)) return false;
	}
	else
	{
...
uint32 Player::GetMaxPersonalArenaRatingRequirement(uint32 minarenaslot)
{
	...
	for (uint8 i = minarenaslot; i < MAX_ARENA_SLOT; ++i)
	{
		if (ArenaTeam * at = sObjectMgr->GetArenaTeamById(GetArenaTeamId(i)))
		{
			...
			if (max_personal_rating < p_rating) max_personal_rating = p_rating;
		}
	}
...
void Player::UpdatePvPState(bool onlyFFA)
{
	...
	{
		if (!IsPvP() || pvpInfo.endTimer != 0) UpdatePvP(true, true);
	}
	else // in friendly area
	{
		if (IsPvP() && !HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP)
				&& pvpInfo.endTimer == 0) pvpInfo.endTimer = time(0); // start toggle-off
	}
}
...
bool Player::ReduceSpellCooldown(uint32 spell_id, uint32 seconds)
{
	if (HasSpellCooldown(spell_id))
	{
		...
		this->AddSpellCooldown(spell_id, 0,
		...
		uint32 itemId, Spell* spell, bool infinityCooldown)
{
		...
		if (rec <= 0 && catrec <= 0
				&& (cat == 76
						|| (IsAutoRepeatRangedSpell(spellInfo)
								&& spellInfo->Id != 75))) rec = GetAttackTime(
				RANGED_ATTACK);

		// Now we have cooldown data (if found any), time to apply mods
		if (rec > 0) ApplySpellMod(spellInfo->Id, SPELLMOD_COOLDOWN, rec,
				spell);

		if (catrec > 0) ApplySpellMod(spellInfo->Id, SPELLMOD_COOLDOWN, catrec,
				spell);
	...
	// self spell cooldown
	if (recTime > 0) AddSpellCooldown(spellInfo->Id, itemId, recTime);
...
void Player::UpdatePotionCooldown(Spell* spell)
{
	...
	if (!m_lastPotionId || isInCombat()) return;

	// Call not from spell cast, send cooldown event for item spells if no in combat
	if (!spell)
	{
		// spell/item pair let set proper cooldown (except not existed charged spell cooldown spellmods for potions)
		if (ItemPrototype const* proto = ObjectMgr::GetItemPrototype(m_lastPotionId)) for (uint8 idx =
				0; idx < MAX_ITEM_SPELLS; ++idx)
			if (proto->Spells [idx].SpellId
					&& proto->Spells [idx].SpellTrigger
							== ITEM_SPELLTRIGGER_ON_USE) if (SpellEntry const* spellInfo = sSpellStore.LookupEntry(proto->Spells[idx].SpellId)) SendCooldownEvent(
					spellInfo, m_lastPotionId);
	}
	// from spell cases (m_lastPotionId set in Spell::SendSpellCooldown)
	else SendCooldownEvent(spell->m_spellInfo, m_lastPotionId, spell);
...
{
	if (!enchantmentcondition) return true;
	...
	for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
	{
				...
				&& pItem2->GetProto()->Socket [0].Color)
		{
					...
					enchant_slot < SOCK_ENCHANTMENT_SLOT + 3; ++enchant_slot)
			{
				...
				if (!enchant_id) continue;

				SpellItemEnchantmentEntry const* enchantEntry =
						sSpellItemEnchantmentStore.LookupEntry(enchant_id);
				if (!enchantEntry) continue;
				...
				for (uint8 b = 0, tmpcolormask = 1; b < 4; b++, tmpcolormask <<=
						1)
				{
					if (tmpcolormask & GemColor) ++curcount [b];
				}
			}
		}
	}
...
void Player::ToggleMetaGemsActive(uint8 exceptslot, bool apply)
{
	...
	for (int slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
	{
				...
				enchant_slot < SOCK_ENCHANTMENT_SLOT + 3; ++enchant_slot)
		{
			...
			uint32 condition = enchantEntry->EnchantmentCondition;
			if (condition) ApplyEnchantment(pItem,
					EnchantmentSlot(enchant_slot), apply);
		}
	}
}
	...
	else
	{
		...
		if (IsMounted())
		{
			AuraEffectList const& auras = GetAuraEffectsByType(
					SPELL_AURA_MOUNTED);
			if (!auras.empty()) m_bgData.mountSpell = (*auras.begin())->GetId();
		}
		else m_bgData.mountSpell = 0;
...
void Player::LeaveBattleground(bool teleportToEntryPoint)
{
	if (Battleground *bg = GetBattleground())
	{
		...
		if (bg->isBattleground() && !isGameMaster() && sWorld->getBoolConfig(
...
bool Player::CanReportAfkDueToLimit()
{
	...
	if (m_bgData.bgAfkReportedCount++ >= 15) return false;
...
void Player::ReportedAfkBy(Player* reporter)
{
	...
	if (!bg || bg != reporter->GetBattleground()
			|| GetTeam() != reporter->GetTeam()) return;
...
WorldLocation Player::GetStartPosition() const
{
	...
	if (getClass() == CLASS_DEATH_KNIGHT && HasSpell(50977)) mapId = 0;
...
bool Player::isValid() const
{
	...
	if (GetSession()->PlayerLogout() || GetSession()->PlayerLoading()) return false;
...
{
	if (Unit::canSeeAlways(obj)) return true;

	// Always can see self
	if (m_mover == obj) return true;

	if (uint64 guid = GetUInt64Value(PLAYER_FARSIGHT)) if (obj->GetGUID()
			== guid) return true;
...
{
	if (Unit::isAlwaysDetectableFor(seer)) return true;

	if (const Player* seerPlayer = seer->ToPlayer()) if (IsGroupVisibleFor(
			seerPlayer)) return true;
...
bool Player::IsVisibleGloballyFor(Player* u) const
{
	...
	if (u->GetSession()->GetSecurity() > SEC_PLAYER) return GetSession()->GetSecurity()
			<= u->GetSession()->GetSecurity();
...
template <>
inline void UpdateVisibilityOf_helper(std::set <uint64>& s64,
		GameObject* target, std::set <Unit*>& /*v*/)
{
	if (!target->IsTransport()) s64.insert(target->GetGUID());
}
...
{
	if (p->GetPetGUID() == t->GetGUID() && t->ToCreature()->isPet()) ((Pet*) t)->Remove(
			PET_SLOT_ACTUAL_PET_SLOT, true);
}
...
void Player::UpdateVisibilityOf(WorldObject* target)
{
	if (HaveAtClient(target))
	{
		...
		{
			if (target->GetTypeId() == TYPEID_UNIT) BeforeVisibilityDestroy
					< Creature > (target->ToCreature(), this);
	...
	else
	{
		if (canSeeOrDetect(target, false, true))
		{
			...
			if (target->isType(TYPEMASK_UNIT)) SendInitialVisiblePackets(
					(Unit*) target);
		}
	}
}
...
void Player::SendInitialVisiblePackets(Unit* target)
{
	...
	if (target->isAlive())
	{
		if (target->GetMotionMaster()->GetCurrentMovementGeneratorType()
				!= IDLE_MOTION_TYPE) target->SendMonsterMoveWithSpeedToCurrentDestination(
				this);
		if (target->HasUnitState(UNIT_STAT_MELEE_ATTACKING)
				&& target->getVictim()) target->SendMeleeAttackStart(
				target->getVictim());
	}
}
...
void Player::ModifyMoney(int32 d)
{
	...
	if (d < 0) SetMoney(GetMoney() > uint64(-d) ? GetMoney() + d : 0);
	...
	else
	{
		...
		if (GetMoney() < uint32(MAX_MONEY_AMOUNT - d))
		{
			...
			if (Guild * pGuild = sObjectMgr->GetGuildById(GetGuildId()))
			{
				if (pGuild)
				{
					if (!this->HasAura(83940) && !this->HasAura(83941)) SetGuildMoneyModifier(
							10);

					if (this->HasAura(83940) && !this->HasAura(83941)) SetGuildMoneyModifier(
							15);

					if (this->HasAura(83941) && this->HasAura(83940)) SetGuildMoneyModifier(
							25);
		...
		else
		{
			...
			if (d) SendEquipError(EQUIP_ERR_TOO_MUCH_GOLD, NULL, NULL);
		}
		SetMoney(newAmount);
	}
}
...
{
	if (m_curSelection) return ObjectAccessor::GetUnit(*this, m_curSelection);
	return NULL;
}

Player * Player::GetSelectedPlayer() const
{
	if (m_curSelection) return ObjectAccessor::GetPlayer(*this, m_curSelection);
	return NULL;
}
...
void Player::SendComboPoints()
{
	...
	if (combotarget)
	{
		...
		else data.Initialize(SMSG_UPDATE_COMBO_POINTS,
				combotarget->GetPackGUID().size() + 1);
...
void Player::AddComboPoints(Unit* target, int8 count, Spell * spell)
{
	...
	if (target->GetGUID() == m_comboTarget) *comboPoints += count;
	else
	{
		if (m_comboTarget) if (Unit* target2 = ObjectAccessor::GetUnit(*this, m_comboTarget)) target2->RemoveComboPointHolder(
				GetGUIDLow());

		// Spells will always add value to m_comboPoints eventualy, so it must be cleared first
		if (spell) m_comboPoints = 0;
...
void Player::ClearComboPoints()
{
	...
	if (Unit* target = ObjectAccessor::GetUnit(*this, m_comboTarget)) target->RemoveComboPointHolder(
			GetGUIDLow());
...
void Player::SendInitialPacketsAfterAddToMap()
{
	...
	{ SPELL_AURA_MOD_FEAR, SPELL_AURA_TRANSFORM, SPELL_AURA_WATER_WALK,
			...
			SPELL_AURA_NONE };
	for (AuraType const* itr = &auratypes [0];
			itr && itr [0] != SPELL_AURA_NONE; ++itr)
	{
		Unit::AuraEffectList const& auraList = GetAuraEffectsByType(*itr);
		if (!auraList.empty()) auraList.front()->HandleEffect(this,
				AURA_EFFECT_HANDLE_SEND_FOR_CLIENT, true);
	}

	if (HasAuraType(SPELL_AURA_MOD_STUN)) SetMovement(MOVE_ROOT);
	...
	else if (GetRaidDifficulty() != GetStoredRaidDifficulty()) SendRaidDifficulty(
			GetGroup() != NULL);
}

void Player::SendUpdateToOutOfRangeGroupMembers()
{
	if (m_groupUpdateMask == GROUP_UPDATE_FLAG_NONE) return;
	if (Group* group = GetGroup()) group->UpdatePlayerOutOfRange(this);

	m_groupUpdateMask = GROUP_UPDATE_FLAG_NONE;
	m_auraRaidUpdateMask = 0;
	if (Pet *pet = GetPet()) pet->ResetAuraUpdateMaskForRaid();
}
		...
		uint32 time)
{
	...
	uint32 type;
	if (time > 3600) type = RAID_INSTANCE_WELCOME;
	else if (time > 900 && time <= 3600) type = RAID_INSTANCE_WARNING_HOURS;
	else if (time > 300 && time <= 900) type = RAID_INSTANCE_WARNING_MIN;
...
{
	if (pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_PROTO_FLAG_NO_EQUIP_COOLDOWN)) return;
...
{
	// not need after this call
	if (HasAtLoginFlag(AT_LOGIN_RESET_SPELLS)) RemoveAtLoginFlag(
			AT_LOGIN_RESET_SPELLS, true);
	...
	if (myClassOnly)
	{
				...
				iter != smap.end(); ++iter)
		{
			...
			if (!IsSpellFitByClassAndRace(spellInfo->Id)) continue;
			...
			if (GetTalentSpellCost(first_rank) > 0) continue;

			// skip broken spells
			if (!SpellMgr::IsSpellValid(spellInfo, this, false)) continue;
		}
	}
	else for (PlayerSpellMap::const_iterator iter = smap.begin();
			iter != smap.end(); ++iter)
...
void Player::learnQuestRewardedSpells(Quest const* quest)
{
	...
	if (sSpellMgr->GetSpellRank(learned_0) > 1 && !HasSpell(learned_0))
	{
		...
		if (!HasSpell(first_spell)) return;
					...
					&& learnedInfo->Effect [1] == 0 && profSpell)
			{
						...
						itr != m_spells.end(); ++itr)
				{
					...
					if (itrInfo->Effect [0] != SPELL_EFFECT_TRADE_SKILL
							|| itrInfo->Effect [1] != 0) continue;

					// compare same chain spells
					if (sSpellMgr->IsSpellRequiringSpell(itr->first, profSpell)) return;
				}
			}
		}
	}
	...
	for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
	{
		...
		if (!pAbility || pAbility->skillId != skill_id
				|| pAbility->learnOnGetSkill
						!= ABILITY_LEARNED_ON_GET_PROFESSION_SKILL) continue;
		// Check race if set
		if (pAbility->racemask && !(pAbility->racemask & raceMask)) continue;
		// Check class if set
		if (pAbility->classmask && !(pAbility->classmask & classMask)) continue;

		if (sSpellStore.LookupEntry(pAbility->spellId))
		{
			// need unlearn spell
			if (skill_value < pAbility->req_skill_value) removeSpell(
					pAbility->spellId);
			// need learn
			else if (!IsInWorld()) addSpell(pAbility->spellId, true, true, true,
					false);
			else learnSpell(pAbility->spellId, true);
		}
	}
}
...
void Player::SendAurasForTarget(Unit *target)
{
			...
			itr != visibleAuras->end(); ++itr)
	{
		...
		if (aura->GetMaxDuration() > 0) flags |= AFLAG_DURATION;
		data << uint8(flags);
		// level
		...
		if (!(flags & AFLAG_CASTER)) data.appendPackGUID(aura->GetCasterGUID());
...
{
	if (m_weeklyquests.empty()) return;
...
void Player::ResetCurrencyWeekCap()
{
	...
	if (m_maxWeekRating [CP_SOURCE_ARENA] <= 1500) m_conquestPointsWeekCap [CP_SOURCE_ARENA] =
			PLAYER_DEFAULT_CONQUEST_POINTS_WEEK_CAP;
	else if (m_maxWeekRating [CP_SOURCE_ARENA] > 3000) m_conquestPointsWeekCap [CP_SOURCE_ARENA] =
			3000;
	else m_conquestPointsWeekCap [CP_SOURCE_ARENA] =
			uint16(
					1.4326
							* (1511.26
									/ (1
...
{
	if (GetBattlegroundId() == 0) return NULL;
...
bool Player::GetBGAccessByLevel(BattlegroundTypeId bgTypeId) const
{
	...
	if (level > DEFAULT_MAX_LEVEL) level = DEFAULT_MAX_LEVEL;

	if (level < bg->GetMinLevel() || level > bg->GetMaxLevel()) return false;
...
float Player::GetReputationPriceDiscount(Creature const* pCreature) const
{
	...
	if (!vendor_faction || !vendor_faction->faction) return 1.0f;

	ReputationRank rank = GetReputationRank(vendor_faction->faction);
	if (rank <= REP_NEUTRAL) return 1.0f;
...
bool Player::IsSpellFitByClassAndRace(uint32 spell_id) const
{
			...
			_spell_idx != bounds.second; ++_spell_idx)
	{
		...
		if (_spell_idx->second->racemask
				&& (_spell_idx->second->racemask & racemask) == 0) continue;

		// skip wrong class skills
		if (_spell_idx->second->classmask
				&& (_spell_idx->second->classmask & classmask) == 0) continue;
...
bool Player::HasQuestForGO(int32 GOId) const
{
	for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
	{
		...
		if (qs.m_status == QUEST_STATUS_INCOMPLETE)
		{
			...
			if (GetGroup() && GetGroup()->isRaidGroup()
					&& !qinfo->IsAllowedInRaid()) continue;
			...
			for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
			{
				...
				if ((-1) * GOId == qinfo->ReqCreatureOrGOId [j]
						&& qs.m_creatureOrGOcount [j]
								< qinfo->ReqCreatureOrGOCount [j]) return true;
			}
		}
	}
	return false;
...
void Player::UpdateForQuestWorldObjects()
{
			...
			itr != m_clientGUIDs.end(); ++itr)
	{
		...
		{
			if (GameObject * obj = HashMapHolder < GameObject > ::Find(*itr)) obj->BuildValuesUpdateBlockForPlayer(
					&udata, this);
		}
		else if (IS_CRE_OR_VEH_GUID(*itr))
		{
			...
			if (!obj->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK)) continue;
...
void Player::SummonIfPossible(bool agree)
{
	...
	if (m_summon_expire < time(NULL)) return;
	...
	if (Battleground *bg = GetBattleground()) bg->EventPlayerDroppedFlag(this);
...
void Player::AutoUnequipOffhandIfNeed(bool force /*= false*/)
{
	...
	if (!CanDualWield()
			&& (offItem->GetProto()->InventoryType == INVTYPE_WEAPONOFFHAND
					|| offItem->GetProto()->InventoryType == INVTYPE_WEAPON)) force =
			true;
...
{
	if (spellInfo->EquippedItemClass < 0) return true;
	...
	switch (spellInfo->EquippedItemClass)
	{
		...
		{
			for (uint8 i = EQUIPMENT_SLOT_MAINHAND; i < EQUIPMENT_SLOT_TABARD;
					++i)
				if (Item *item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, i)) if (item
						!= ignoreItem
						&& item->IsFitToSpellRequirements(spellInfo)) return true;
			break;
		}
		case ITEM_CLASS_ARMOR:
		{
			// tabard not have dependent spells
			for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_MAINHAND;
					++i)
				if (Item *item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, i)) if (item
						!= ignoreItem
						&& item->IsFitToSpellRequirements(spellInfo)) return true;
			...
			if (Item *item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND)) if (item
					!= ignoreItem && item->IsFitToSpellRequirements(spellInfo)) return true;

			// ranged slot can have some armor subclasses
			if (Item *item = GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED)) if (item
					!= ignoreItem && item->IsFitToSpellRequirements(spellInfo)) return true;
...
bool Player::CanNoReagentCast(SpellEntry const* spellInfo) const
{
	...
	if (spellInfo->AttributesEx5 & SPELL_ATTR5_NO_REAGENT_WHILE_PREP
			&& HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PREPARATION)) return true;
	...
	if (spellInfo->SpellFamilyFlags & noReagentMask) return true;
...
void Player::RemoveItemDependentAurasAndCasts(Item * pItem)
{
	...
	for (uint32 i = 0; i < CURRENT_MAX_SPELL; ++i)
		if (Spell* spell = GetCurrentSpell(CurrentSpellTypes(i))) if (spell->getState()
				!= SPELL_STATE_DELAYED
				&& !HasItemFitToSpellRequirements(spell->m_spellInfo, pItem)) InterruptSpell(
				CurrentSpellTypes(i));
}
...
uint32 Player::GetResurrectionSpellId()
{
	...
	if (prio < 1 && HasSpell(20608) && !HasSpellCooldown(21169)
			&& (HasAura(58059) || HasItemCount(17030, 1))) spell_id = 21169;
	...
	if (pVictim->GetTypeId() == TYPEID_UNIT)
	{
		if (pVictim->ToCreature()->isTotem() || pVictim->ToCreature()->isPet()
				|| pVictim->ToCreature()->GetCreatureInfo()->flags_extra
						& CREATURE_FLAG_EXTRA_NO_XP_AT_KILL) return false;
	}
	return true;
}
...
bool Player::GetsRecruitAFriendBonus(bool forXP)
{
			...
			CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL) || !forXP)
	{
		if (Group *group = this->GetGroup())
		{
			...
			{
				Player* pGroupGuy = itr->getSource();
				if (!pGroupGuy) continue;

				if (!pGroupGuy->IsAtRecruitAFriendDistance(this)) continue; // member (alive or dead) or his corpse at req. distance

				if (forXP)
				{
					// level must be allowed to get RaF bonus
					if (pGroupGuy->getLevel() > sWorld->getIntConfig(
							CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL)) continue;

					// level difference must be small enough to get RaF bonus, UNLESS we are lower level
					if (pGroupGuy->getLevel() < getLevel()) if (uint8(
							getLevel() - pGroupGuy->getLevel()) > sWorld->getIntConfig(
							CONFIG_MAX_RECRUIT_A_FRIEND_BONUS_PLAYER_LEVEL_DIFFERENCE)) continue;
				}
		...
		WorldObject* pRewardSource)
{
	...
	if (Group *group = GetGroup())
	{
				...
				itr->next())
		{
			...
			if (!player->IsAtGroupRewardDistance(pRewardSource)) continue; // member (alive or dead) or his corpse at req. distance

			// quest objectives updated only for alive group member or dead but with not released body
			if (player->isAlive() || !player->GetCorpse()) player->KilledMonsterCredit(
...
{
	if (!pRewardSource) return false;
	const WorldObject* player = GetCorpse();
	if (!player || isAlive()) player = this;

	if (player->GetMapId() != pRewardSource->GetMapId()
			|| player->GetInstanceId() != pRewardSource->GetInstanceId()) return false;

	return pRewardSource->GetDistance(player) <= sWorld->getFloatConfig(
...
bool Player::IsAtRecruitAFriendDistance(WorldObject const* pOther) const
{
	...
	if (player->GetMapId() != pOther->GetMapId()
			|| player->GetInstanceId() != pOther->GetInstanceId()) return false;

	return pOther->GetDistance(player) <= sWorld->getFloatConfig(
...
void Player::ResurectUsingRequestData()
{
	...
	if (IS_PLAYER_GUID(m_resurrectGUID)) TeleportTo(m_resurrectMap,
			m_resurrectX, m_resurrectY, m_resurrectZ, GetOrientation());
...
void Player::UpdateZoneDependentAuras(uint32 newZone)
{
	...
	for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
			itr != saBounds.second; ++itr)
		if (itr->second->autocast
				&& itr->second->IsFitToRequirements(this, newZone, 0)) if (!HasAura(
				itr->second->spellId)) CastSpell(this, itr->second->spellId,
				true);
}
...
void Player::UpdateAreaDependentAuras(uint32 newArea)
{
			...
			iter != m_ownedAuras.end();)
	{
		...
		if (sSpellMgr->GetSpellAllowedInLocationError(
				...
				newArea, this) != SPELL_CAST_OK) RemoveOwnedAura(iter);
		else ++iter;
	}
	...
	for (SpellAreaForAreaMap::const_iterator itr = saBounds.first;
		...
		if (itr->second->autocast
				&& itr->second->IsFitToRequirements(this, m_zoneUpdateId,
						newArea)) if (!HasAura(itr->second->spellId)) CastSpell(
...
uint32 Player::GetCorpseReclaimDelay(bool pvp) const
{
	if (pvp)
	{
		if (!sWorld->getBoolConfig(CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVP)) return copseReclaimDelay [0];
	}
	else if (!sWorld->getBoolConfig(CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVE)) return 0;
...
void Player::UpdateCorpseReclaimDelay()
{
	...
	if ((pvp && !sWorld->getBoolConfig(CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVP))
			|| (!pvp && !sWorld->getBoolConfig(
					CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVE))) return;
	...
	if (now < m_deathExpireTime)
	{
		...
		if (count < MAX_DEATH_COUNT) m_deathExpireTime = now
				+ (count + 1) * DEATH_EXPIRE_STEP;
		else m_deathExpireTime = now + MAX_DEATH_COUNT * DEATH_EXPIRE_STEP;
	}
	else m_deathExpireTime = now + DEATH_EXPIRE_STEP;
}

void Player::SendCorpseReclaimDelay(bool load)
{
	Corpse* corpse = GetCorpse();
	if (load && !corpse) return;

	bool pvp;
	if (corpse) pvp = (corpse->GetType() == CORPSE_RESURRECTABLE_PVP);
	else pvp = (m_ExtraFlags & PLAYER_EXTRA_PVP_DEATH);

	time_t delay;
	if (load)
	{
		if (corpse->GetGhostTime() > m_deathExpireTime) return;
						...
						CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVE)))
		{
			count = (m_deathExpireTime - corpse->GetGhostTime())
					/ DEATH_EXPIRE_STEP;
			if (count >= MAX_DEATH_COUNT) count = MAX_DEATH_COUNT - 1;
		}
		else count = 0;
		...
		time_t now = time(NULL);
		if (now >= expected_time) return;

		delay = expected_time - now;
	}
	else delay = GetCorpseReclaimDelay(pvp);
...
Player* Player::GetNextRandomRaidMember(float radius)
{
			...
			itr = itr->next())
	{
		...
		if (Target && Target != this && IsWithinDistInMap(Target, radius)
				&& !Target->HasInvisibilityAura() && !IsHostileTo(Target)) nearMembers.push_back(
				Target);
	}
...
PartyResult Player::CanUninviteFromGroup() const
{
	...
	if (!grp) return ERR_NOT_IN_GROUP;

	if (grp->isLFGGroup())
	{
		uint64 gguid = grp->GetGUID();
		if (!sLFGMgr->GetKicksLeft(gguid)) return ERR_PARTY_LFG_BOOT_LIMIT;

		LfgState state = sLFGMgr->GetState(gguid);
		if (state == LFG_STATE_BOOT) return ERR_PARTY_LFG_BOOT_IN_PROGRESS;

		if (grp->GetMembersCount() <= sLFGMgr->GetVotesNeeded(gguid)) return ERR_PARTY_LFG_BOOT_TOO_FEW_PLAYERS;

		if (state == LFG_STATE_FINISHED_DUNGEON) return ERR_PARTY_LFG_BOOT_DUNGEON_COMPLETE;

		if (grp->isRollLootActive()) return ERR_PARTY_LFG_BOOT_LOOT_ROLLS;
	...
	{
		if (!grp->IsLeader(GetGUID()) && !grp->IsAssistant(GetGUID())) return ERR_NOT_LEADER;

		if (InBattleground()) return ERR_INVITE_RESTRICTED;
	}
...
{
	if (group == NULL) m_originalGroup.unlink();
	else
	{
...
void Player::UpdateUnderwaterState(Map* m, float x, float y, float z)
{
	...
	{
		if (res & LIQUID_MAP_UNDER_WATER) m_MirrorTimerFlags |=
				UNDERWATER_INWATER;
		else m_MirrorTimerFlags &= ~UNDERWATER_INWATER;
	}

	// Allow travel in dark water on taxi or transport
	if ((liquid_status.type & MAP_LIQUID_TYPE_DARK_WATER) && !isInFlight()
			&& !GetTransport()) m_MirrorTimerFlags |= UNDERWARER_INDARKWATER;
	else m_MirrorTimerFlags &= ~UNDERWARER_INDARKWATER;
	...
	if (liquid_status.type & MAP_LIQUID_TYPE_MAGMA)
	{
		if (res
				& (LIQUID_MAP_UNDER_WATER | LIQUID_MAP_IN_WATER
						| LIQUID_MAP_WATER_WALK)) m_MirrorTimerFlags |=
				UNDERWATER_INLAVA;
		else m_MirrorTimerFlags &= ~UNDERWATER_INLAVA;
	}
	...
	if (liquid_status.type & MAP_LIQUID_TYPE_SLIME)
	{
		if (res
				& (LIQUID_MAP_UNDER_WATER | LIQUID_MAP_IN_WATER
						| LIQUID_MAP_WATER_WALK)) m_MirrorTimerFlags |=
				UNDERWATER_INSLIME;
		else m_MirrorTimerFlags &= ~UNDERWATER_INSLIME;
	}
}

void Player::SetCanParry(bool value)
{
	if (m_canParry == value) return;
...
void Player::SetViewpoint(WorldObject* target, bool apply)
{
	if (apply)
	{
		...
		if (target->isType(TYPEMASK_UNIT) && !GetVehicle()) ((Unit*) target)->AddPlayerToVision(
				this);
	}
	else
	{
		...
		if (target->isType(TYPEMASK_UNIT) && !GetVehicle()) ((Unit*) target)->RemovePlayerFromVision(
				this);
...
{
	if (uint64 guid = GetUInt64Value(PLAYER_FARSIGHT)) return (WorldObject*) ObjectAccessor::GetObjectByTypeMask(
			*this, guid, TYPEMASK_SEER);
	return NULL;
}
...

{
	...
	if (level > GT_MAX_LEVEL) level = GT_MAX_LEVEL; // max level in this dbc
	...
	if ((hairstyle == newhairstyle) && (haircolor == newhaircolor)
			...
			&& (!newSkin || (newSkin->hair_id == skincolor))) return 0;
	...
	if ((haircolor != newhaircolor) && (hairstyle == newhairstyle)) cost +=
			bsc->cost * 0.5f; // +1/2 of price

	if (facialhair != newfacialhair) cost += bsc->cost * 0.75f; // +3/4 of price

	if (newSkin && skincolor != newSkin->hair_id) cost += bsc->cost * 0.75f; // +5/6 of price
...
void Player::InitGlyphsForLevel()
{
	...
	for (uint32 i = 0; i < sGlyphSlotStore.GetNumRows(); ++i)
		...
		{
			if (gs->LearningOrder <= order) glyphMask |= (1 << uint8(counter)); // Set the appropriate bit
	...
	if (level >= 81 && !HasSpell(90647)) learnSpell(90647, false);
	else if (!HasSpell(89964)) learnSpell(89964, false);
}
...
{
	if (bitIndex > MAX_TITLE_INDEX) return false;
...
void Player::SetTitle(CharTitlesEntry const* title, bool lost)
{
	...
	if (lost)
	{
		if (!HasFlag(PLAYER__FIELD_KNOWN_TITLES + fieldIndexOffset, flag)) return;
...
void Player::UpdateCharmedAI()
{
	...
	if (!charmer->isInCombat()) GetMotionMaster()->MoveFollow(charmer,
...
{
	if (!spellInfo || !spellInfo->StartRecoveryTime) return;

	float cdTime = float(spellInfo->StartRecoveryTime);

	if (!(spellInfo->Attributes & (SPELL_ATTR0_UNK4 | SPELL_ATTR0_PASSIVE))) cdTime *=
			GetFloatValue(UNIT_MOD_CAST_SPEED);
	else if (IsRangedWeaponSpell(spellInfo) && !spell->IsAutoRepeat()) cdTime *=
			m_modAttackSpeedPct [RANGED_ATTACK];

	if (cdTime > 1500.0f) cdTime = 1500.0f;

	ApplySpellMod(spellInfo->Id, SPELLMOD_GLOBAL_COOLDOWN, cdTime, spell);
	if (cdTime > 0) m_globalCooldowns [spellInfo->StartRecoveryCategory] =
			uint32(cdTime);
}
...
uint32 Player::GetRuneBaseCooldown(uint8 index)
{
	...
	for (AuraEffectList::const_iterator i = regenAura.begin();
			i != regenAura.end(); ++i)
	{
		if ((*i)->GetMiscValue() == POWER_RUNE && (*i)->GetMiscValueB() == rune) cooldown =
...
void Player::RestoreBaseRune(uint8 index)
{
	...
	if (aura && !(aura->GetSpellProto()->Attributes & SPELL_ATTR0_PASSIVE)) return;

	ConvertRune(index, GetBaseRune(index));
	SetRuneConvertAura(index, NULL);

	// Don't drop passive talents providing rune convertion
	if (!aura || aura->GetAuraType() != SPELL_AURA_CONVERT_RUNE) return;

	for (uint8 i = 0; i < MAX_RUNES; ++i)
	{
		if (aura == m_runes->runes [i].ConvertAura) return;
	}
...
{
	if (getClass() != CLASS_DEATH_KNIGHT) return;
...
{
	for (uint32 i = 0; i < MAX_RUNES; ++i)
		if (GetBaseRune(i) == runeType && GetRuneCooldown(i) == 0) return false;
		...
		LootStore const& store, bool broadcast)
{
	...
	for (uint32 i = 0; i < max_slot; ++i)
	{
		...
		uint8 msg = CanStoreNewItem(bag, slot, dest, lootItem->itemid,
				lootItem->count);
		if (msg != EQUIP_ERR_OK && slot != NULL_SLOT) msg = CanStoreNewItem(bag,
				NULL_SLOT, dest, lootItem->itemid, lootItem->count);
		if (msg != EQUIP_ERR_OK && bag != NULL_BAG) msg = CanStoreNewItem(
...
void Player::StoreLootItem(uint8 lootSlot, Loot* loot)
{
	...
	{
		SendLootRelease(GetLootGUID());return;
	}
	...
	if (msg == EQUIP_ERR_OK)
	{
		...
		if (qitem)
		{
			...
			if (item->freeforall || loot->GetPlayerQuestItems().size() == 1) SendNotifyLootItemRemoved(
					lootSlot);
			else loot->NotifyQuestItemRemoved(qitem->index);
		}
		else
		{
			if (ffaitem)
	...
	else SendEquipError(msg, NULL, NULL, item->itemid);
}

uint32 Player::CalculateTalentsPoints() const
{
	uint32 base_talent = 0;
	if (getLevel() >= 10 && getLevel() <= 81) base_talent = (((getLevel() - 10
			+ 1) - (((getLevel() - 10 + 1) % 2) == 1 ? 1 : 0)) / 2) + 1;
	else base_talent = getLevel() - 44;

	if (getClass() != CLASS_DEATH_KNIGHT || GetMapId() != 609) return uint32(
			base_talent * sWorld->getRate(RATE_TALENT));

	uint32 talentPointsForLevel = getLevel() < 56 ? 0 : getLevel() - 55;
	talentPointsForLevel += m_questRewardTalentCount;

	if (talentPointsForLevel > base_talent) talentPointsForLevel = base_talent;
...
void Player::_LoadSkills(PreparedQueryResult result)
{
	...
	if (result)
	{
		do
		{
			...
			if (pSkill->categoryId == SKILL_CATEGORY_PROFESSION) SetUInt32Value(
					PLAYER_SKILL_INDEX(count), MAKE_PAIR32(skill, 1));
			else SetUInt32Value(PLAYER_SKILL_INDEX(count),
					MAKE_PAIR32(skill, 0));
	...
	if (getClass() == CLASS_DEATH_KNIGHT)
	{
		...
		if (base_skill < 1) base_skill = 1; // skill mast be known and then > 0 in any case

		if (GetPureSkillValue(SKILL_FIRST_AID) < base_skill) SetSkill(
				SKILL_FIRST_AID, 0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_AXES) < base_skill) SetSkill(SKILL_AXES, 0,
				base_skill, base_skill);
		if (GetPureSkillValue(SKILL_DEFENSE) < base_skill) SetSkill(
				SKILL_DEFENSE, 0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_POLEARMS) < base_skill) SetSkill(
				SKILL_POLEARMS, 0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_SWORDS) < base_skill) SetSkill(SKILL_SWORDS,
				0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_2H_AXES) < base_skill) SetSkill(
				SKILL_2H_AXES, 0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_2H_SWORDS) < base_skill) SetSkill(
				SKILL_2H_SWORDS, 0, base_skill, base_skill);
		if (GetPureSkillValue(SKILL_UNARMED) < base_skill) SetSkill(
				SKILL_UNARMED, 0, base_skill, base_skill);
	}
}
...
uint32 Player::GetPhaseMaskForSpawn() const
{
	...
	if (!isGameMaster()) phase = GetPhaseMask();
	else
	{
		AuraEffectList const& phases = GetAuraEffectsByType(SPELL_AURA_PHASE);
		if (!phases.empty()) phase = phases.front()->GetMiscValue();
	}

	// some aura phases include 1 normal map in addition to phase itself
	if (uint32 n_phase = phase & ~PHASEMASK_NORMAL) return n_phase;
		...
		uint32 limit_count) const
{
	...
	if (uint8 res = CanEquipUniqueItem(pProto, eslot, limit_count)) return res;
			...
			enchant_slot < SOCK_ENCHANTMENT_SLOT + 3; ++enchant_slot)
	{
		...
		if (uint8 res = CanEquipUniqueItem(pGem, eslot, gem_limit_count)) return res;
	}
		...
		uint8 except_slot, uint32 limit_count) const
{
	...
	if (itemProto->Flags & ITEM_PROTO_FLAG_UNIQUE_EQUIPPED)
	{
		...
		if (HasItemOrGemWithIdEquipped(itemProto->ItemId, 1, except_slot)) return EQUIP_ERR_ITEM_UNIQUE_EQUIPABLE;
	}
	...
	if (itemProto->ItemLimitCategory)
	{
		...
		if (!limitEntry) return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;

		// NOTE: limitEntry->mode not checked because if item have have-limit then it applied and to equip case

		if (limit_count > limitEntry->maxCount) return EQUIP_ERR_ITEM_MAX_LIMIT_CATEGORY_EQUIPPED_EXCEEDED;

		// there is an equip limit on this item
		if (HasItemOrGemWithLimitCategoryEquipped(itemProto->ItemLimitCategory,
				limitEntry->maxCount - limit_count + 1, except_slot)) return EQUIP_ERR_ITEM_MAX_COUNT_EQUIPPED_SOCKETED;
	}
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
			...
			&& !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
	{
		...
		if (damageperc > 0)
		{
			uint32 damage = (uint32)(damageperc * GetMaxHealth() * sWorld
					->getRate(RATE_DAMAGE_FALL));
			...
			if (damage > 0)
			{
				...
				if (damage > GetMaxHealth()) damage = GetMaxHealth();

				// Gust of Wind
				if (HasAura(43621)) damage = GetMaxHealth() / 2;

				uint32 original_health = GetHealth();
				uint32 final_damage = EnvironmentalDamage(DAMAGE_FALL, damage);

				// recheck alive, might have died of EnvironmentalDamage, avoid cases when player die in fact like Spirit of Redemption case
				if (isAlive() && final_damage < original_health) GetAchievementMgr().UpdateAchievementCriteria(
...
{
	if (AchievementEntry const *achievement = sAchievementStore.LookupEntry(entry)) return GetAchievementMgr().HasAchieved(
			achievement);
	return false;
}
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank, bool one)
{
	...
	if (one && talentTabInfo->TalentTabID != GetTalentBranchSpec(m_activeSpec))
	{
					...
					if (thisrank != -1)
					{
								...
								== GetTalentBranchSpec(m_activeSpec))
						{
							...
							if (curtalent_maxrank != -1
									&& thisrank == curtalent_maxrank) pointInBranchSpec +=
									curtalent_maxrank + 1;
						}
					}
				}
		...
		if (pointInBranchSpec < 31) return;
	}

	// prevent learn talent for different class (cheating)
	if ((getClassMask() & talentTabInfo->ClassMask) == 0) return;
	...
	if (curtalent_maxrank >= (talentRank + 1)) return;

	// check if we have enough talent points
	if (CurTalentPoints < (talentRank - curtalent_maxrank + 1)) return;
	...
	if (talentInfo->DependsOn > 0)
	{
		if (TalentEntry const *depTalentInfo = sTalentStore.LookupEntry(talentInfo->DependsOn))
		{
			...
			for (uint8 rank = talentInfo->DependsOnRank; rank < MAX_TALENT_RANK;
					rank++)
			{
				if (depTalentInfo->RankID [rank] != 0) if (HasSpell(
						depTalentInfo->RankID [rank])) hasEnoughRank = true;
			}
			if (!hasEnoughRank) return;
		}
	}
	...
	if (spentPoints < (talentInfo->Row * MAX_TALENT_RANK)) return;
...
void Player::LearnPetTalent(uint64 petGuid, uint32 talentId, uint32 talentRank)
{
	...
	if (petGuid != pet->GetGUID()) return;

	uint32 CurTalentPoints = pet->GetFreeTalentPoints();

	if (CurTalentPoints == 0) return;

	if (talentRank >= MAX_PET_TALENT_RANK) return;
	...
	if (!pet_family) return;

	if (pet_family->petTalentType == PET_TALENT_TYPE_NOT_HUNTER_PET) // not hunter pet
	return;

	// prevent learn talent for different family (cheating)
	if (!((1 << pet_family->petTalentType) & talentTabInfo->petTalentMask)) return;
	...
	if (talentInfo->DependsOn > 0)
	{
		if (TalentEntry const *depTalentInfo = sTalentStore.LookupEntry(talentInfo->DependsOn))
		{
			...
			for (uint8 rank = talentInfo->DependsOnRank; rank < MAX_TALENT_RANK;
					rank++)
			{
				if (depTalentInfo->RankID [rank] != 0) if (pet->HasSpell(
						depTalentInfo->RankID [rank])) hasEnoughRank = true;
			}
			if (!hasEnoughRank) return;
		}
	...
	// not have required min points spent in talent tree
	if (spentPoints < (talentInfo->Row * MAX_PET_TALENT_RANK)) return;
	...
	// already known
	if (pet->HasSpell(spellid)) return;
		...
		uint32 opcode)
{
	if (m_lastFallTime >= minfo.fallTime
			...
			|| opcode == MSG_MOVE_FALL_LAND) SetFallInformation(minfo.fallTime,
			minfo.pos.GetPositionZ());
}
...
{
	if (!m_temporaryUnsummonedPetNumber) return;

	// not resummon in not appropriate state
	if (IsPetNeedBeTemporaryUnsummoned()) return;

	if (GetPetGUID()) return;

	Pet* NewPet = new Pet(this);
	if (!NewPet->LoadPetFromDB(this, 0, m_temporaryUnsummonedPetNumber, true)) delete NewPet;
...
{
	if (!c->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK)) return false;

	SpellClickInfoMapBounds clickPair = sObjectMgr->GetSpellClickInfoMapBounds(
			c->GetEntry());
	if (clickPair.first == clickPair.second) return true;

	for (SpellClickInfoMap::const_iterator itr = clickPair.first;
			itr != clickPair.second; ++itr)
		if (itr->second.IsFitToRequirements(this, c)) return true;
...
void Player::BuildPlayerTalentsInfoData(WorldPacket *data)
{
	...
	*data << uint8(m_activeSpec); // talent group index (0 or 1)
	...
	if (m_specsCount)
	{
		if (m_specsCount > MAX_TALENT_SPECS) m_specsCount = MAX_TALENT_SPECS;
			...
			*data << uint8(talentIdCount); // [PH], talentIdCount
			...
			for (uint8 i = 0; i < MAX_TALENT_TABS; ++i)
			{
						...
						++talentId)
				{
					...
					if (talentInfo->TalentTab != talentTabId) continue;
					...
					// not learned talent
					if (curtalent_maxrank < 0) continue;
...
void Player::BuildPetTalentsInfoData(WorldPacket *data)
{
	...
	*data << uint8(talentIdCount); // [PH], talentIdCount
	...
	CreatureFamilyEntry const *pet_family = sCreatureFamilyStore.LookupEntry(
			ci->family);
	if (!pet_family
			|| pet_family->petTalentType == PET_TALENT_TYPE_NOT_HUNTER_PET) return;
...
void Player::SendTalentsInfoData(bool pet)
{
	...
	data << uint8(pet ? 1 : 0);
	if (pet) BuildPetTalentsInfoData(&data);
	else BuildPlayerTalentsInfoData(&data);
	GetSession()->SendPacket(&data);
}
...
void Player::SendEquipmentSetList()
{
	...
	{
		if (itr->second.state == EQUIPMENT_SET_DELETED) continue;
...
void Player::DeleteEquipmentSet(uint64 setGuid)
{
			...
			itr != m_EquipmentSets.end(); ++itr)
	{
		...
		{
			if (itr->second.state == EQUIPMENT_SET_NEW) m_EquipmentSets.erase(
					itr);
			else itr->second.state = EQUIPMENT_SET_DELETED;
			break;
		}
	}
}
...
void Player::RemoveAtLoginFlag(AtLoginFlags f, bool in_db_also /*= false*/)
{
	...
	if (in_db_also) CharacterDatabase.PExecute(
			"UPDATE characters set at_login = at_login & ~ %u WHERE guid ='%u'",
			uint32(f), GetGUIDLow());
}
...
void Player::UpdateSpecCount(uint8 count)
{
	...
	if (m_activeSpec >= count) ActivateSpec(0);
...
{
	if (GetActiveSpec() == spec) return;

	if (spec > GetSpecsCount()) return;
	...
	if (Pet* pet = GetPet()) RemovePet(pet, PET_SLOT_ACTUAL_PET_SLOT, true);
	...
	for (uint8 slot = 0; slot < MAX_GLYPH_SLOT_INDEX; ++slot)
		...
		if (uint32 oldglyph = m_Glyphs[m_activeSpec][slot]) if (GlyphPropertiesEntry const *old_gp = sGlyphPropertiesStore.LookupEntry(oldglyph)) RemoveAurasDueToSpell(
				old_gp->SpellId);
	...
	for (uint32 i = 0; i < sTalentTreePrimarySpellsStore.GetNumRows(); ++i)
	{
		...
		if (!talentInfo || talentInfo->TalentTabID != GetTalentBranchSpec(spec)) continue;
	...
	for (uint8 slot = 0; slot < MAX_GLYPH_SLOT_INDEX; ++slot)
	{
		...
		// apply primary glyph
		if (glyph) if (GlyphPropertiesEntry const *gp = sGlyphPropertiesStore.LookupEntry(glyph)) CastSpell(
				this, gp->SpellId, true);
...

	{
		...
		if (PreparedQueryResult result = CharacterDatabase.Query(stmt)) _LoadActions(
				result);
	}
	...
	if (pw != POWER_MANA) SetPower(POWER_MANA, 0); // Mana must be 0 even if it isn't the active power type.
...
bool Player::AddItem(uint32 itemId, uint32 count)
{
	...
	uint8 msg = CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, count,
			&noSpaceForCount);
	if (msg != EQUIP_ERR_OK) count = noSpaceForCount;
	...
	Item* item = StoreNewItem(dest, itemId, true,
			Item::GenerateItemRandomPropertyId(itemId));
	if (item) SendNewItem(item, count, true, false);
	else return false;
	return true;
}
...
void Player::RefundItem(Item *item)
{
	...
	// Grant back money
	if (moneyRefund) ModifyMoney(moneyRefund);
}

void Player::SetRandomWinner(bool isWinner)
{
	m_IsBGRandomWinner = isWinner;
	if (m_IsBGRandomWinner) CharacterDatabase.PExecute(
...
void Player::_LoadRandomBGStatus(PreparedQueryResult result)
{
	...
	if (result) m_IsBGRandomWinner = true;
}
...
float Player::GetAverageItemLevel()
{
	...
	for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
	{
		...
		if (i == EQUIPMENT_SLOT_TABARD || i == EQUIPMENT_SLOT_RANGED
				|| i == EQUIPMENT_SLOT_OFFHAND || i == EQUIPMENT_SLOT_BODY) continue;

		if (m_items [i] && m_items [i]->GetProto()) sum +=
				m_items [i]->GetProto()->GetItemLevelIncludingQuality();
...
{
	if (_instanceResetTimes.empty()) return;
...
void Player::RemoveOrAddMasterySpells()
{
	...
	{
		if (HasAura(77514)) RemoveAurasDueToSpell(77514);

		if (HasAura(77515)) RemoveAurasDueToSpell(77515);

		if (HasAura(77493)) RemoveAurasDueToSpell(77493);

		if (HasAura(76658)) RemoveAurasDueToSpell(76658);

		if (HasAura(76657)) RemoveAurasDueToSpell(76657);

		if (HasAura(76595)) RemoveAurasDueToSpell(76595);

		if (HasAura(76671)) RemoveAurasDueToSpell(76671);

		if (HasAura(77220)) RemoveAurasDueToSpell(77220);

		if (HasAura(76857)) RemoveAurasDueToSpell(76857);
	}
	else if (HasAuraType(SPELL_AURA_MASTERY))
	{
		if (GetTalentBranchSpec(GetActiveSpec()) == BS_DEATH_KNIGHT_FROST) if (!HasAura(
				77514)) AddAura(77514, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_DEATH_KNIGHT_UNHOLY) if (!HasAura(
				77515)) AddAura(77515, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_DRUID_FERAL_COMBAT) if (!HasAura(
				77493)) AddAura(77493, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_HUNTER_SURVIVAL) if (!HasAura(
				76658)) AddAura(76658, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_HUNTER_BEAST_MASTERY) if (!HasAura(
				76657)) AddAura(76657, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_MAGE_FIRE) if (!HasAura(
				76595)) AddAura(76595, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_PALADIN_PROTECTION) if (!HasAura(
				76671)) AddAura(76671, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_WARLOCK_DESTRUCTION) if (!HasAura(
				77220)) AddAura(77220, this);

		if (GetTalentBranchSpec(GetActiveSpec()) == BS_WARRIOR_PROTECTION) if (!HasAura(
				76857)) AddAura(76857, this);
	}
}
